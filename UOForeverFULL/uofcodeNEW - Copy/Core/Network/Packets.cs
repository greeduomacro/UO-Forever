...
namespace Server.Network
{
	...
	public sealed class DisplayEquipmentInfo : Packet
	{
		public DisplayEquipmentInfo( Item item, EquipmentInfo info ) : base( 0xBF )
		{
			...
			if ( info.Crafter != null )
			{
                ...
                if (info.Crafter.Fame > 9999)
                {
                    if (info.Crafter.Female)
                        name = "Lady " + name;
                    else
                        name = "Lord " + name;
                }
	...
	{
		public WorldItem( Item item ) : this( item, item.ItemID )
		{
		}

		public WorldItem( Item item, int itemID ) : base( 0x1A )
		{
	...
	{
		public WorldItemSA( Item item ) : this( item, item.ItemID )
		{
		}

		public WorldItemSA( Item item, int itemID ) : base( 0xF3, 24 )
		{
	...
	{
		public WorldItemHS( Item item ) : this( item, item.ItemID )
		{
		}
    ...
    public sealed class MobileDirectionToOthersOnly : Packet
    {
	...
	{
		public static FeatureFlags AdditionalFlags { get; set; }
		public static FeatureFlags RemovedFlags { get; set; }

		public static SupportedFeatures Instantiate( NetState ns, Mobile m )
		{
			return new SupportedFeatures( ns, m );
		}

		public SupportedFeatures( NetState ns, Mobile m ) : base( 0xB9, ns.ExtendedSupportedFeatures ? 5 : 3 )
		{
			FeatureFlags flags = ExpansionInfo.GetFeatures(m != null ? m.Expansion : Core.Expansion);

			if (AdditionalFlags != FeatureFlags.None)
			{
				flags |= AdditionalFlags;
			}

			if (RemovedFlags != FeatureFlags.None)
			{
				flags &= ~RemovedFlags;
			}
    ...
    public sealed class MobileStatusExtended : Packet
    {
            ...
            : base(0x11)
        {
            ...
            if (m.EraHS && ns != null && ns.ExtendedStatus)
            {
                type = 6;
                EnsureCapacity(121);
            }
            else if (m.EraML && ns != null && ns.SupportsExpansion(Expansion.ML))
            {
                type = 5;
                EnsureCapacity(91);
            }
            ...
            if (type >= 4)
            {
                m_Stream.Write(0); // Fire
                m_Stream.Write(0); // Cold
                m_Stream.Write(0); // Poison
                m_Stream.Write(0); // Energy
                m_Stream.Write(0); // Luck
    ...
    public sealed class MobileStatus : Packet
    {
            ...
            : base(0x11)
        {
            ...
            else if (beholder.EraHS && ns != null && ns.ExtendedStatus)
            {
                type = 6;
                EnsureCapacity(121);
            }
			else if (beholder.EraML && ns != null && ns.SupportsExpansion(Expansion.ML))
            {
                type = 5;
                EnsureCapacity(91);
            }
