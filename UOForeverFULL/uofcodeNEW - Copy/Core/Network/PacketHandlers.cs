...
namespace Server.Network
{
	...
	public static class PacketHandlers
	{
		...
		static PacketHandlers()
		{
            ...
            RegisterExtended(0xF0, true, new OnPacketReceive(Custom));
...
//					EventSink.InvokeInscribeMenuRequest( new InvokeInscribeMenuRequest( m, World.FindItem( serial ), loc ) );
		...
		public static void LookReq( NetState state, PacketReader pvSrc )
		{
			...
			if ( s.IsMobile )
			{
				...
				{
					if ( SingleClickProps && from.EraAOS )
					{
			...
			else if ( s.IsItem )
			{
				...
				if ( item != null && !item.Deleted && from.CanSee( item ) && Utility.InUpdateRange( from.Location, item.GetWorldLocation( from ) ) )
				{
					if ( SingleClickProps && from.EraAOS )
					{
...
//				m.ClearFastwalkStack();
        ...
        public static void Custom(NetState state, PacketReader pvSrc)
        {
            if (state.Mobile == null)
                return;
		...
		public static void DoLogin( NetState state, Mobile m )
		{
			...
			state.Send( SupportedFeatures.Instantiate( state, m ) );
