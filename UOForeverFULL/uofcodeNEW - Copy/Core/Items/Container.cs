...
namespace Server.Items
{
	...
	public class Container : Item
	{
		...
		{
			if (item != this && from.AccessLevel < AccessLevel.GameMaster)
			{
				if (item != null && item.IsPhased)
				{
					return false;
		...
		public virtual bool CheckHold( Mobile m, Item item, bool message, bool checkItems, int plusItems, int plusWeight )
		{
			if ( m != null && m.AccessLevel < AccessLevel.GameMaster )
			{
				...
				if ( checkItems && maxItems != 0 && (this.TotalItems + plusItems + item.TotalItems + (item.IsVirtualItem || item.IsPhased ? 0 : 1)) > maxItems )
				{
					if ( message )
				...
				else if ( MaxWeight != 0 && (this.TotalWeight + plusWeight + item.TotalWeight + item.PileWeight) > MaxWeight )
				{
					if ( message )
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            SetSaveFlag(ref flags, SaveFlag.MovableContents, m_MovableContents);
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
                case 4:
                    {
                        flags = (SaveFlag)reader.ReadByte();
                        if (GetSaveFlag(flags, SaveFlag.MovableContents))
                            m_MovableContents = true; // default is false
                        //skip case 3!!!
                        goto case 2;
                    }
		...
		{
			if ( from.AccessLevel > AccessLevel.Player || from.InRange( this.GetWorldLocation(), 2 ) && Expansion == from.Expansion)
				DisplayTo( from );
			else
				from.SendLocalizedMessage( 500446 ); // That is too far away.
		}
