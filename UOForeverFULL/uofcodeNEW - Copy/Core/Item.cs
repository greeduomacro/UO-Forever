...
*/
	...
	public class Item : IEntity, IHued, IPoint3D, IComparable<Item>, ISerializable, ISpawnable
	{
		#region Expansion
		private Expansion _Expansion = Expansion.None;
		...
		public Expansion Expansion
		{
			...
			set
			{
				if (_Expansion == value || value == Expansion.None || (_Expansion != Expansion.None && !ExpansionChangeAllowed))
				{
					return;
				}
				...
				if (_Expansion == Expansion.None && Parent is Item)
				{
					Item p = (Item)Parent;

					if (p.Expansion != Expansion.None && p.Expansion != value)
					{
						value = p.Expansion;
					}
				}
		...
		[CommandProperty(AccessLevel.Counselor)]
		public virtual bool ExpansionChangeAllowed
		{
			get
			{
				if (Deleted)
				...
				return parent is Item && ((Item)parent).ExpansionChangeAllowed;
				*/
				return false;
			}
		}

		public bool EraT2A { get { return Expansion >= Expansion.T2A; } }
		public bool EraUOR { get { return Expansion >= Expansion.UOR; } }
		public bool EraUOTD { get { return Expansion >= Expansion.UOTD; } }
		public bool EraLBR { get { return Expansion >= Expansion.LBR; } }
		public bool EraAOS { get { return Expansion >= Expansion.AOS; } }
		public bool EraSE { get { return Expansion >= Expansion.SE; } }
		public bool EraML { get { return Expansion >= Expansion.ML; } }
		public bool EraSA { get { return Expansion >= Expansion.SA; } }
		public bool EraHS { get { return Expansion >= Expansion.HS; } }

		protected virtual void OnExpansionChanged(Expansion old)
		{ }

		public virtual bool ShowExpansion { get { return IsPhased; } }

		protected virtual void LabelToExpansion(Mobile m)
		{
			if (!Deleted && ShowExpansion && Expansion != Expansion.None && m != null && m.AccessLevel > AccessLevel.Counselor)
			{
				LabelTo(m, IsPhased ? 0x22 : 0x55, "[{0}]", Expansion);
			}
		}

		[CommandProperty(AccessLevel.Counselor)]
		public virtual bool IsPhased
		{
			get
			{
				if (Deleted)// || HeldBy != null)
				{
					return false;
				}

				if (m_Map != null && m_Map != Map.Internal && m_Map.Expansion != Expansion)
				{
					var parent = Parent;

					/*bool result = false;

					if (parent is Mobile && ((Mobile)parent).Expansion != Expansion)
					{
						result = true;
					}
					else if (parent is Item && ((Item)parent).Expansion != Expansion)
					{
						result = true;
					}
					...
					switch (Layer)
					{
						...
						case Layer.Bank:
							{
								if (parent is Mobile && ((Mobile)parent).BankBox == this)
								{
									return false;
								}
							}
						...
						case Layer.Backpack:
							{
								if (parent is Mobile && ((Mobile)parent).Backpack == this)
								{
									return false;
								}
							}
				...
				return Parent is Item && ((Item)Parent).IsPhased;
			}
		}
		#endregion
		...
		{
			return other != null ? m_Serial.CompareTo(other.Serial) : -1;
		}
		...
		private int m_Hue;
		private int m_HSHue; //Used in conjunction with UO:HS multis
		...
		public virtual List<Item> LookupItems()
		{
		...
		public virtual List<Item> AcquireItems()
		{
			if ( this is Container )
			{
		...
		public virtual void AddNameProperties( ObjectPropertyList list )
		{
			...
			if ( GetFlag( ImplFlag.Dyable ) && DisplayDyable )
				list.Add( 1050045, "\tDyable\t\t" ); //~1_PREFIX~~2_NAME~~3_SUFFIX~
		...
		public void Bounce( Mobile from )
		{
			...
			if ( bounce != null )
			{
				...
				if ( parent is Item && !((Item)parent).Deleted )
				{
					...
					if ( (p.IsAccessibleTo( from ) || (Nontransferable && p is BankBox)) && ( !(root is Mobile) || ((Mobile)root).CheckNonlocalDrop( from, this, p ) ) )
					{
					...
					{
                        if (p is BankBox && p.RootParentEntity is Mobile) // tyring to bounce into bank box and failing because it is no longer accessible
                        {
                            Mobile owningMob = (Mobile)p.RootParentEntity;
                            if (owningMob.Backpack != null) // put it in their backpack
                            {
                                owningMob.Backpack.AddItem(this);
                            }
                            else
                            {
		...
		public virtual DeathMoveResult OnParentDeath( Mobile parent )
		{
			if ( !Movable )
				...
				return DeathMoveResult.MoveToBackpack;
			else if ( CheckNewbied( parent ) && parent.Kills < 5 )
				return DeathMoveResult.MoveToBackpack;
		...
		{
			StringEntry entry = to.GetStringEntry( number );
			if ( entry != null )
			{
		...
		public void MoveToWorldAndStack( Point3D location, Map map )
		{
			...
			IPooledEnumerable eable = m_Map.GetItemsInRange( m_Location, 0 );

			foreach ( Item item in eable )
				if ( item.StackWith( null, this, false ) )
					break;
		...
		[CommandProperty(AccessLevel.Counselor, AccessLevel.Administrator)]
		public bool CommandDelete { get; set; }
        ...
        private int m_Association = 0; // for grouping items together so you can easily [area ___ where item association = __
        [CommandProperty(AccessLevel.GameMaster)]
        public int Association { get { return m_Association; } set { m_Association = value; } }
		...
		{
			return ( Decays && Spawner == null && Parent == null && Map != Map.Internal && Region.Find( Location, Map ).OnDecay( this ) );
		}
		...
		{
			if ( !ObjectPropertyList.Enabled || !EraAOS )
				return;
		...
		private enum SaveFlag : ulong
		{
			...
			HSHue			= 0x08000000,
            GhostVisible    = 0x10000000,
            Breakable       = 0x20000000,
            DoesNotDecay    = 0x40000000
		}

        [Flags]
        private enum ExtSaveFlag : short
        {
            None            = 0x00000000,
            Association     = 0x00000001,
			CmdDelete		= 0x00000002,
		...
		public virtual void Serialize( GenericWriter writer )
		{
			...
			writer.Write( version = 15 ); // version
            ...
            if ( !Breakable )
                flags |= SaveFlag.Breakable;
            if ( DoesNotDecay )
                flags |= SaveFlag.DoesNotDecay;
			...
			ImplFlag implFlags = (m_Flags &
								  (ImplFlag.Visible | ImplFlag.Movable | ImplFlag.Stackable | ImplFlag.Insured | ImplFlag.PaidInsurance |
								   ImplFlag.QuestItem | ImplFlag.Dyable));
           
			if (implFlags != (ImplFlag.Visible | ImplFlag.Movable))
                flags |= SaveFlag.ImplFlags;
            
			// Extended save flags ->
            ExtSaveFlag extSaveFlags = ExtSaveFlag.None;

			SetExtSaveFlag(ref extSaveFlags, ExtSaveFlag.Association, m_Association > 0);
			SetExtSaveFlag(ref extSaveFlags, ExtSaveFlag.CmdDelete, CommandDelete);
			SetExtSaveFlag(ref extSaveFlags, ExtSaveFlag.Expansion, Expansion != Expansion.None);
			
			SetSaveFlag(ref flags, SaveFlag.ExtSaveFlags, extSaveFlags != ExtSaveFlag.None);
			// <- Extended save flags

			// version 13 changes int to ulong
			if (version >= 13)
			{
			...
			// version 12
            if (GetSaveFlag(flags, SaveFlag.ExtSaveFlags))
			{
				// version 13 changes short to ulong
				if (version >= 13)
				{
					writer.Write((ulong)extSaveFlags);
				}
				else
				{
					writer.Write((short)extSaveFlags);
				}
			}

			// can use version switching here
			switch (version)
			{
                case 15:
                    {
                        writer.Write(m_Transport);
                    }
                    goto case 14;
				case 14:
					{
						if (GetExtSaveFlag(extSaveFlags, ExtSaveFlag.Expansion))
						{
							writer.Write((int)_Expansion);
						}
					}
					goto case 13;
				case 13:
				...
				case 12:
					{
						if (GetExtSaveFlag(extSaveFlags, ExtSaveFlag.Association))
						{
							writer.Write(m_Association);
						}
					}
					break;
			}
			...
			if ( GetSaveFlag( flags, SaveFlag.HSHue ) )
				writer.WriteEncodedInt( m_HSHue );
		...
		public IEnumerable<TEntity> GetObjectsInRange<TEntity>(int range) where TEntity : IEntity
		{
			var pool = GetObjectsInRange(range);

			foreach (var o in pool.OfType<TEntity>())
			{
				yield return o;
			}
		...
		public IEnumerable<TItem> GetItemsInRange<TItem>(int range) where TItem : Item
		{
			var pool = GetItemsInRange(range);

			foreach (var o in pool.OfType<TItem>())
			{
				yield return o;
			}
		...
		public IEnumerable<TMobile> GetMobilesInRange<TMobile>(int range) where TMobile : Mobile
		{
			var pool = GetMobilesInRange(range);

			foreach (var o in pool.OfType<TMobile>())
			{
				yield return o;
			}
		...
		public IEnumerable<TState> GetClientsInRange<TState>(int range) where TState : NetState
		{
			var pool = GetClientsInRange(range);

			foreach (var o in pool.OfType<TState>())
			{
				yield return o;
			}
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			SaveFlag flags = version > 10
								 ? (SaveFlag)(version < 13 ? (ulong)reader.ReadInt() : reader.ReadULong())
								 : SaveFlag.None;
            
            ExtSaveFlag extFlags = ExtSaveFlag.None;

			if (GetSaveFlag(flags, SaveFlag.ExtSaveFlags))
			{
				extFlags = (ExtSaveFlag)(version < 13 ? (ulong)reader.ReadShort() : reader.ReadULong());
			}
			...
			switch ( version )
			{
				...
				case 14:
					{
						if (GetExtSaveFlag(extFlags, ExtSaveFlag.Expansion))
						{
							_Expansion = (Expansion)reader.ReadInt();
						}
					}
					goto case 13;
				// 13 also changed ExtSaveFlag from short to ulong
				case 13:
					CommandDelete = GetExtSaveFlag(extFlags, ExtSaveFlag.CmdDelete);
					goto case 12;
				case 12:
					{
						if (GetExtSaveFlag(extFlags, ExtSaveFlag.Association))
						{
							m_Association = reader.ReadInt();
						}
					}
					goto case 11;
				case 11:
					{
						if (GetSaveFlag(flags, SaveFlag.GhostVisible))
							m_GhostVisible = false; // default is true
						
						if (GetSaveFlag(flags, SaveFlag.Breakable))
							m_Breakable = false; // default is true
						
						if (GetSaveFlag(flags, SaveFlag.DoesNotDecay))
							m_DoesNotDecay = true; // default is false
						//skip version 10!
					}
					goto case 9;
				case 10:
					{
						// haven't read flags yet
		...
		public void SendInfoTo( NetState state ) {
			SendInfoTo( state, ObjectPropertyList.Enabled && EraAOS );
		}

		public virtual void SendInfoTo( NetState state, bool sendOplPacket )
		{
			if (state == null || state.Mobile == null /*||
				(state.Mobile.AccessLevel < AccessLevel.GameMaster && IsPhased(state.Mobile))*/)
			{
				return;
			}
		...
		[Hue, CommandProperty( AccessLevel.EventMaster )]
		...
		public virtual int HSHue
		{
			get
			{
				return (QuestItem ? QuestItemHue : m_HSHue);
			}
			set
			{
				if ( m_HSHue != value )
		...
		{
			get { return QuestItem || Association > 0; }
		}
		...
		public void ProcessDelta()
		{
			...
			if ( map != null && !Deleted )
			{
				bool sendOPLUpdate = ObjectPropertyList.Enabled && EraAOS && (flags & ItemDelta.Properties) != 0;
						...
						if ( rootParent != null )
						{
							...
							if ( ns != null )
							{
								if ( rootParent.CanSee( this ) && rootParent.InRange( worldLoc, GetUpdateRange( rootParent ) ) )
								{
									...
									if (ObjectPropertyList.Enabled && rootParent.EraAOS)
										ns.Send( OPLPacket );
								}
							}
						}
								...
								if ( tradeRecip != null )
								{
									...
									if ( ns != null )
									{
										if ( tradeRecip.CanSee( this ) && tradeRecip.InRange( worldLoc, GetUpdateRange( tradeRecip ) ) )
										{
											...
											if (ObjectPropertyList.Enabled && tradeRecip.EraAOS)
												ns.Send( OPLPacket );
										}
									}
								}
								...
								else
								{
									...
									if ( ns != null )
									{
										if ( mob.CanSee( this ) )
										{
											...
											if ( ObjectPropertyList.Enabled && mob.EraAOS )
												ns.Send( OPLPacket );
										}
									}
								}
				...
				if ( (flags & ItemDelta.Update) != 0 )
				{
					...
					foreach ( NetState state in eable ) {
						...
						if ( m.CanSee( this ) && m.InRange( worldLoc, GetUpdateRange( m ) ) ) {
							if ( m_Parent == null ) {
								SendInfoTo( state, ObjectPropertyList.Enabled && m.EraAOS );
							} else {
								if ( p == null ) {
									if ( m_Parent is Item ) {
								...
								if ( ObjectPropertyList.Enabled && m.EraAOS ) {
									state.Send( OPLPacket );
								}
							}
						}
		...
		public void PublicOverheadMessage( MessageType type, int hue, int number, string args )
		{
			if ( m_Map != null )
			{
				...
				foreach ( NetState state in eable )
				{
					...
					if ( m.CanSee( this ) && m.InRange( worldLoc, GetUpdateRange( m ) ) )
					{
						StringEntry entry = m.GetStringEntry( number );
						if ( entry != null )
						{
							if ( entry.State == LState.Original ) //Client Localized
							{
		...
		public void PrivateOverheadMessage( MessageType type, int hue, bool ascii, string text, NetState state )
		{
			if( state != null )
			{
				...
				if ( state.Mobile != null )
					lang = state.Mobile.Language;

				if( ascii )
					state.Send( new AsciiMessage( m_Serial, m_ItemID, type, hue, 3, Name, text ) );
				else
					state.Send( new UnicodeMessage( m_Serial, m_ItemID, type, hue, 3, lang, Name, text ) );
			}
		}
		...
		public void PrivateOverheadMessage( MessageType type, int hue, int number, string args, NetState state )
		{
			if( state != null )
			{
				if ( state.LTable != null )
				{
					StringEntry entry = state.LTable[number];
					...
					if ( entry != null )
					{
						...
						else
						{
							...
							state.Send( new UnicodeMessage( m_Serial, m_ItemID, type, hue, 3, lang, Name, entry.CombinedWithArguments( args ) ) );
						}
					}
				}
			}
		...
		public string GetDisplayName( NetState state )
		{
			return GetDisplayName( state, true );
		}

		public virtual string GetDisplayName( NetState state, bool article )
		{
			string name = Name;

			if ( String.IsNullOrEmpty( name ) )
				name = GetStringFromLabel( state );

			if ( name.ToLower().StartsWith( "an " ) )
		...
		public string GetStringFromLabel()
		{
			return GetStringFromLabel( null );
		}

		public virtual string GetStringFromLabel( NetState state )
		{
			StringList stringlist = StringList.Default;

			if ( state != null && state.LTable != null )
				stringlist = state.LTable;

			if ( stringlist[LabelNumber] != null )
				return stringlist[LabelNumber].Text;
		...
		{
			if( Nontransferable && from.AccessLevel <= AccessLevel.GameMaster )
			{
		...
		public virtual bool OnDroppedInto( Mobile from, Container target, Point3D p )
		{
			if( !from.OnDroppedItemInto( this, target, p ) )
				return false;
			else if( Nontransferable && target != from.Backpack && ( QuestItem || target != from.BankBox ) && from.AccessLevel <= AccessLevel.GameMaster )
			{
				HandleInvalidTransfer( from );
				return false;
			}
		...
		public virtual bool OnDroppedOnto( Mobile from, Item target )
		{
			if ( Deleted || from.Deleted || target.Deleted || from.Map != target.Map || from.Map == null || target.Map == null )
				...
				return false;
			else if ( Nontransferable && from.AccessLevel <= AccessLevel.GameMaster && target != from.Backpack)
			{
                HandleInvalidTransfer( from );
				return false;
			}
		...
		//Used for virtual locations of items with OnSingleClick (LookReq)
 		public virtual Point3D GetWorldLocation( Mobile from )
		{
			...
			if ( root == null )
				...
				return ((IEntity)root).GetVirtualLocation( from );

			//return root == null ? m_Location : new Point3D( (IPoint3D) root );
		}

		public virtual Point3D GetVirtualLocation( Mobile from )
		{
			return m_Location;
		}
        ...
        private ItemData? _InternalItemData;

        [CommandProperty(AccessLevel.Counselor, AccessLevel.GameMaster)]
        public ItemData ItemData
        {
            //
            get { return _InternalItemData != null ? _InternalItemData.Value : TileData.ItemTable[m_ItemID & TileData.MaxItemValue]; }
            set { _InternalItemData = value; }
        }
		...
		public virtual bool CheckLift( Mobile from, Item item, ref LRReason reject )
		{
			...
			else if ( from != null && !from.CheckLiftTrigger( item, ref reject))
                return false;
            else
				return true;
		}
		...
		public virtual void OnSingleClick( Mobile from )
		{
			...
			if ( ns != null )
			{
				if ( this.Name == null )
				{
					if ( m_Amount <= 1 || m_ItemID == 8198)
						ns.Send( new MessageLocalized( m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, LabelNumber, "", "" ) );
					else
						ns.Send( new MessageLocalizedAffix( m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, LabelNumber, "", AffixType.Append, String.Format( " : {0}", Utility.MoneyFormat(m_Amount, from)), "" ) );
				}
				else
				{
                    if (m_Amount <= 1 || m_ItemID == 8198)
                        ns.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, from.Language, "", this.Name ) );
                    else
                        ns.Send(new UnicodeMessage(m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, from.Language, "", this.Name + String.Format(" : {0}", Utility.MoneyFormat(m_Amount, from)) ));
				}
			}
		}
		...
		{
			return IsPhased || CheckBlessed(obj as Mobile);
		}

		public virtual bool CheckBlessed( Mobile m )
		{
			if ( IsPhased || m_LootType == LootType.Blessed || (Mobile.InsuranceEnabled && EraAOS && Insured) )
				return true;

			return ( m != null && m == BlessedFor );
		}

		public virtual bool CheckNewbied( Mobile from )
		{
			return ( m_LootType == LootType.Newbied && from.Kills < Mobile.MurderCount );
		}
		...
		public virtual bool IsStandardLoot()
		{
			...
			if ( Mobile.InsuranceEnabled && EraAOS && Insured )
				return false;
        ...
        public static void ForceSerialChange(Item item, int newSerial)
        {
            if (World.FindEntity(newSerial) == null) // only allow if it doesn't exist already in the world records
            {
                item.m_Serial = newSerial;
            }
        }
