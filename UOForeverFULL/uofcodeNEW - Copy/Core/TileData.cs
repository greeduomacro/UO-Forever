...
namespace Server
{
	...
	public static class TileData
	{
        ...
        public static int[] NoLOSOverrides = new int[12] // int array that contains itemids of tiles that should be NoLOS but aren't
        {
            // add Teleport tiles here
            6173,
            6174,
            6175,
            6176,
        ...
        public static int[] PassableOverrides = new int[1] // int array that contains itemids of tiles that should be passable but aren't
        {
            37324
        };
				...
				using ( FileStream fs = new FileStream( filePath, FileMode.Open, FileAccess.Read, FileShare.Read ) )
				{
					...
					if ( fs.Length == 3188736 ) { // 7.0.9.0
						...
						for ( int i = 0; i < 0x10000; ++i )
						{
                            ...
                            long flagInt = bin.ReadInt64();
                            for (int ix = 0; ix < PassableOverrides.Length; ix++)
                            {
                                if (PassableOverrides[ix] == i)
                                {
                                    if ((flagInt & (long)TileFlag.Impassable) > 0) { flagInt -= (long)TileFlag.Impassable; } // take out the Impassable flag 
                                }
                            }
                            for (int ix = 0; ix < NoLOSOverrides.Length; ix++)
                            {
                                if (NoLOSOverrides[ix] == i)
                                {
                                    flagInt |= (long)TileFlag.NoShoot; // add the NoShoot flag
                                }
                            }

                            TileFlag flags = (TileFlag)flagInt;
