 ...
 *
 *   $Id: Mobile.cs 867 2012-04-22 05:37:16Z mark $
 *
 ***************************************************************************/
...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
	{
		...
		public virtual Expansion Expansion
		{
			get
			{
				if (m_Map != null && m_Map != Map.Internal)
				{
					return m_Map.Expansion;
				}

				if (m_LogoutMap != null && m_LogoutMap != Map.Internal)
				{
					return m_LogoutMap.Expansion;
				}
        ...
        public bool CustomTeam = false; // Alan Mod: optimization to check for custom XmlTeams (since it is looked up so much for notoriety). Not serialized, set by XmlTeam attachments
		#endregion

		//Do not check bankbox
		public virtual bool HasItemsNoBankBox()
		{
			...
			return (m_Backpack != null && m_Backpack.Items.Count > 0);
		}

		public virtual bool HasItems()
		{
			return HasItemsNoBankBox() || ( m_BankBox != null && m_BankBox.Items.Count > 0 );
		}
		...
		public StringEntry GetStringEntry( int number )
		{
			if ( LTable != null )
				return LTable[number];

			return null;
		}

		[CommandProperty( AccessLevel.Lead )]
		public string Address{ get{ return NetState != null ? Utility.FormattedIPAddress( NetState.Address ) : null; } }

        // Alan Mod
        private int m_Association = 0; // for grouping mobs together so you can easily [area ___ where mobile association = __
        [CommandProperty(AccessLevel.GameMaster)]
        public int Association { get { return m_Association; } set { m_Association = value; } }

        private int m_HitsRegenPerTick = 1;
        [CommandProperty(AccessLevel.GameMaster)]
        public int HitsRegenPerTick { get { return m_HitsRegenPerTick; } set { m_HitsRegenPerTick = value; } }
		...
		public string PreferredLanguage
		{
			...
			set
			{
				...
				if ( m_NetState != null )
				{
					m_NetState.LTable = StringList.GetList( Language );
		...
		public DateTime NextMovementTime { get; set; }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public DateTime NextArcadeSwingTime { get; set; }
		...
		private static int m_MaxPlayerResistance = 0;
		...
		private static TimeSpan WarmodeSpamCatch(Mobile m)
		{
			return TimeSpan.FromSeconds(m.EraSE ? 1.0 : 0.5);
		}

		private static TimeSpan WarmodeSpamDelay(Mobile m)
		{
			return TimeSpan.FromSeconds(m.EraSE ? 4.0 : 2.0);
		}
		...
		public void DelayChangeWarmode( bool value )
		{
			...
			{
				m_WarmodeChanges = 1;
				m_NextWarmodeChange = now + WarmodeSpamCatch(this);
			}
		...
		private class HitsTimer : Timer
		{
			...
			protected override void OnTick()
			{
				if( m_Owner.CanRegenHits )// m_Owner.Alive && !m_Owner.Poisoned )
					m_Owner.Hits+= m_Owner.HitsRegenPerTick;
		...
		{
			if (delta == 0 || sender == null || sender.IsVirtualItem)
			{
				return;
			}
		...
		public virtual bool CheckShove( Mobile shoved )
		{
			if( (m_Map.Rules & MapRules.FreeMovement) == 0 )
			{
                if (!shoved.Alive || !Alive || shoved.IsDeadBondedPet || IsDeadBondedPet)
                    ...
                    return true;
                else if (m_IgnoreMobiles || shoved.IgnoreMobiles)
                    return true;
		...
		public virtual void Kill()
		{
			...
			if( pack != null )
			{
				List<Item> packCopy = new List<Item>();
				...
				if ( pack.Items != null && pack.Items.Count > 0 )
				{
					...
					for( int i = 0; i < packCopy.Count; ++i )
					{
						...
						if( res == DeathMoveResult.MoveToCorpse )
						{
							...
							{
								Item subItem = subItems[j];
								if ( !subItem.Deleted && ( subItem.LootType == LootType.Blessed || subItem.Insured ) )
									moveToPack.Add( subItem );
							}
						}
						else
							moveToPack.Add( item );
		...
		public virtual void Lift( Item item, int amount, out bool rejected, out LRReason reject )
		{
			...
			if( rejected && state != null )
			{
				...
				if (ObjectPropertyList.Enabled && EraAOS && item.Parent != null)
		...
		public static Item LiftItemDupe( Item oldItem, int amount )
		{
            ...
            item.Association = oldItem.Association;
        ...
        /* DropReq6017 handles this, if you use resynch like this, then it removes items
         * from corpses on the client, which is also annoying.
        public void DropResynch()
        {
            // COPY OF RESYNCH PACKETHANDLER due to the issue of > 3 tiles trying to pick up something
            if (this.Deleted || this.NetState == null) return;
            NetState state = this.NetState;
            if (state.StygianAbyss)
            {
                state.Send(new MobileUpdate(this));
                state.Send(new MobileIncoming(this, this));
            }
            else
            {
                state.Send(new MobileUpdateOld(this));
                state.Send(new MobileIncomingOld(this, this));
            }
			...
			if( m_Map != null )
			{
				...
				for( int i = 0; i < hears.Count; ++i )
				{
					...
					if( mutatedArgs == null || !CheckHearsMutatedSpeech( heard, mutateContext ) )
					{
						...
						{
							if( regp == null )
								regp = Packet.Acquire( new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, text ) );
					...
					else
					{
						...
						{
							if( mutp == null )
								mutp = Packet.Acquire( new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, mutatedText ) );
		...
		public void Heal( int amount, Mobile from, bool message )
		{
			...
			if( message && amount > 0 && m_NetState != null && m_NetState.Mobile != null )
			{
				
				StringEntry entry = m_NetState.Mobile.GetStringEntry( 1008158 );
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			switch( version )
			{
                ...
                case 38:
                    m_HitsRegenPerTick = (int)reader.ReadByte();
                    goto case 37;
                case 37:
                    if (GetSaveFlag(flags, SaveFlag.Association))
                        m_Association = reader.ReadInt();
                    goto case 36;
                case 36:
                    if (GetSaveFlag(flags, SaveFlag.IgnoreMobiles))
                        m_IgnoreMobiles = true;
                    if (GetSaveFlag(flags, SaveFlag.GhostVisible))
                        m_GhostVisible = false; // default is true
                    if (GetSaveFlag(flags, SaveFlag.GuardImmune))
                        m_GuardImmune = true; // default is false
                    // skip to 33!
                    goto case 33;
                case 35:
                ...
                case 34:
                    ...
                    goto case 33;
                case 33: //Removed m_Language, worthless
				case 32:
				{
		...
		public virtual void Serialize( GenericWriter writer )
		{
            ...
            // version 32 and above WRITE THE flags FIRST!
			SaveFlag flags = SaveFlag.None;
			...
			SetSaveFlag(ref flags, SaveFlag.IgnoreMobiles, m_IgnoreMobiles);
			SetSaveFlag(ref flags, SaveFlag.GuardImmune, m_GuardImmune);
			SetSaveFlag(ref flags, SaveFlag.GhostVisible, !m_GhostVisible);
			SetSaveFlag(ref flags, SaveFlag.Association, m_Association > 0);

			writer.WriteEncodedInt( (int)flags );
            //
            writer.Write((Item)m_Transport);
            // version 38
            byte hitsRegenPerTick = m_HitsRegenPerTick < 0 ? (byte)0 : (m_HitsRegenPerTick > 250 ? (byte)250 : (byte)m_HitsRegenPerTick);
            writer.Write(hitsRegenPerTick);

            // version 37
            if (GetSaveFlag(flags, SaveFlag.Association))
                writer.Write((int)m_Association);

            // don't do 35 or 34 anymore!
            //version 35
            //writer.Write((bool)m_GhostVisible);

            //version 34
            //writer.Write((bool)m_GuardImmune);
            // less than version 34
		...
		public Skills Skills
		{
			get
			{
				return m_Skills ?? (m_Skills = new Skills(this));
			}
			set
			{
			}
        ...
        public Map Map
        {
            ...
            set
            {
                ...
                if (m_Map != value)
                {
	                ...
	                if (value != null && value.Expansion != exp)
	                {
		                DropHolding();
	                }
                    ...
                    if (ns != null)
                    {
                        ...
                        if (ns.StygianAbyss)
                        {
							ns.Send(SupportedFeatures.Instantiate(ns, this));
                            ns.Send(new MobileUpdate(this));
        ...
        // Alan Mod: make it so we can send frozen packets
        public static bool FrozenPacketFlagsEnabled = false;
		...
		public virtual int GetPacketFlags()
		{
            ...
            if (FrozenPacketFlagsEnabled && (m_Paralyzed || m_Frozen)) // Alan Mod
                flags |= 0x01;
		...
		public virtual bool CanSee( Item item )
		{
			...
			if (!item.GhostVisible && !Alive && AccessLevel < AccessLevel.GameMaster)
			{
				return false;
			}
			...
			if( item is BankBox )
			{
				...
				if( m_AccessLevel <= AccessLevel.Counselor && (box.Owner != this || !box.Opened) )
					return false;
			}
			...
			if (item.Deleted || item.Map != m_Map)
			{
				return false;
			}

			return m_AccessLevel >= AccessLevel.EventMaster || (item.Visible && !item.IsPhased);
		}
		...
		{
			if( m_Deleted || m == null || m.m_Deleted || m_Map == Map.Internal || m.m_Map == Map.Internal )
				return false;

            if (!m.GhostVisible && !Alive && m.AccessLevel < AccessLevel.GameMaster)
                return false;

            return this == m || (
                m.m_Map == m_Map &&
                (!m.Hidden || (m_AccessLevel != AccessLevel.Player && (m_AccessLevel >= m.AccessLevel || m_AccessLevel >= AccessLevel.EventMaster))) &&
                ((m.Alive || (EraSE && Skills.SpiritSpeak.Value >= 100.0)) || !Alive || m_AccessLevel > AccessLevel.Player || m.Warmode));
		...
		public CultureInfo Culture{ get{ return m_Culture; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public string Language{ get{ return ( String.IsNullOrEmpty( m_PreferredLanguage ) ? m_Language : m_PreferredLanguage ); } set{ m_Language = value; UpdateCulture(); } }

		public void UpdateCulture()
		{
			m_Culture = Utility.GetCultureByLanguage( String.IsNullOrEmpty( m_PreferredLanguage ) ? m_Language : m_PreferredLanguage );
		}
		...
		public int BodyModValue
		{
			get
			{
				return m_BodyMod.BodyID;
			}
			set
			{
				m_BodyMod = value;
		...
		[CommandProperty( AccessLevel.Seer, AccessLevel.Lead )]
        ...
        public virtual void SetLocation(Point3D newLocation, bool isTeleport)
        {
            ...
            if (oldLocation != newLocation)
            {
                ...
                if (map != null)
                {
                    ...
                    if (ourState != null)
                    {
                        IPooledEnumerable eeable = map.GetObjectsInRange(newLocation, Core.GlobalMaxUpdateRange);
                    ...
                    else
                    {
                        ...
                        foreach (NetState ns in eable)
                        {
                            if (((isTeleport && (!ns.HighSeas || !m_NoMoveHS)) || !Utility.InUpdateRange(oldLocation, ns.Mobile.Location)) && ns.Mobile.CanSee(this))
                            {
                                ...
                                if (ObjectPropertyList.Enabled && ns.Mobile.EraAOS)
                                {
                                    ns.Send(OPLPacket);
		...
		public BankBox BankBox
		{
			get
			{
				...
				if (m_BankBox == null)
					AddItem(m_BankBox = new BankBox(this, Expansion));
		...
		public virtual BankBox FindBank(Expansion e)
		{
			if (!Player)
			{
				return FindBankNoCreate();
			}
			
			var bank = BankBox;

			return bank != null && bank.Expansion == e ? bank : null;
		}
            ...
            if (m_Map != null)
            {
                ...
                foreach (NetState state in eable)
                {
                    if (state.Mobile.CanSee(this))
                    {
                        ...
                        if (ObjectPropertyList.Enabled && state.Mobile.EraAOS)
                        {
                            state.Send(OPLPacket);
        ...
        public int TypeRef { get { return m_TypeRef; } }
...
//			m_LTable = StringList.Default;
        ...
        public virtual void ProcessDelta()
        {
            ...
            bool sendOPLUpdate = ObjectPropertyList.Enabled && m.EraAOS && (delta & MobileDelta.Properties) != 0;
		...
		public void PublicExtendedOverheadMessage( MessageType type, int hue, bool ascii, int number, bool noLineOfSight )
		{
			if( m_Map != null )
			{
				...
				foreach( NetState state in eable )
				{
					...
					{
						StringEntry entry = state.Mobile.GetStringEntry( number );
						if ( entry != null )
						{
							Packet p = null;
			...
			if( state != null )
			{
				...
				if ( entry != null )
				{
					if( ascii )
						...
						state.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, entry.Text ) );
				}
			}
		}
		...
		public void LocalExtendedOverheadMessage( MessageType type, int hue, bool ascii, int number )
		{
			if( m_NetState != null )
			{
				StringEntry entry = GetStringEntry( number );
				if ( entry != null )
				{
					if( ascii )
						m_NetState.Send( new AsciiMessage( m_Serial, Body, type, hue, 3, Name, entry.Text ) );
					else
						m_NetState.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, entry.Text ) );
				}
			}
		}
		...
		public void PrivateOverheadMessage( MessageType type, int hue, bool ascii, string text, NetState state )
		{
			...
			if( ascii )
				...
				state.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, text ) );
		}
		...
		public void LocalOverheadMessage( MessageType type, int hue, bool ascii, string text )
		{
			...
			if( ns != null )
			{
				if( ascii )
					...
					ns.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, text ) );
			}
		}
			...
			if( m_NetState != null )
			{
				...
				if ( entry != null )
				{
					if ( entry.State == LState.Original ) //Client Localized
						...
						m_NetState.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, entry.CombinedWithArguments( args ) ) );
				}
			}
		}
		...
		{
			SendLocalizedMessage( number, 0x3B2 );
		}
			...
			if( m_NetState != null )
			{
				...
				if ( entry != null )
				{
					if ( entry.State == LState.Original ) //Client Localized
					{
						...
						m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, 3, Language, "System", entry.CombinedWithArguments( args ) ) );
				}
			}
		}
			...
			if( m_NetState != null )
			{
				...
				if ( entry != null )
				{
					if ( entry.State == LState.Original ) //Client Localized
						...
						m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, 3, Language, "System", append ? entry.AppendArguments( args ) : entry.PrependArguments( args ) ) );
				}
			}
		}
...
*/
		...
		{
			if( m_NetState != null )
				m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, 3, Language, "System", text ) );
		}
		...
		{
			if( m_NetState != null )
				m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, font, Language, "System", text ) );
		}
        ...
        private bool m_IsStealthing = false;
        [CommandProperty(AccessLevel.GameMaster)]
        ...
        private bool m_IgnoreMobiles = false;
        [CommandProperty(AccessLevel.GameMaster)]
        ...
        public static TimeSpan ServerWideObjectDelay = TimeSpan.FromSeconds(0.5);
		    ...
		    if (!m_Player)
		    {
		        if (m_GuildClickMessage)
		        {
		            ...
		            if (guild != null && (m_DisplayGuildTitle || (m_Player && guild.Type != GuildType.Regular)))
		            {
		                ...
		                string text = String.Format(title.Length <= 0 ? "[{1}]{2}" : "[{0}, {1}]{2}", title,
		        ...
		        if ((m_Body.IsHuman) && m_Fame >= 10000)
		            prefix = (m_Female ? "Lady" : "Lord");
