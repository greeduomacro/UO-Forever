...
namespace Server
{
	public sealed class DynamicSaveStrategy : SaveStrategy
	{
		...
		private SequentialFileWriter _mapData, _mapIndex;

		private ConcurrentBag<Item> _decayBag;
		private ConcurrentBag<BaseInstanceMap> _decayMapBag;
		...
		private BlockingCollection<QueuedMemoryWriter> _mapThreadWriters;

		public DynamicSaveStrategy()
		{
			_decayBag = new ConcurrentBag<Item>();
			_decayMapBag = new ConcurrentBag<BaseInstanceMap>();
			...
			_mapThreadWriters = new BlockingCollection<QueuedMemoryWriter>();
		}
		...
		public override void ProcessDecay()
		{
			...
			while( _decayMapBag.TryTake( out map ) )
				map.Delete();
		}
		...
		private void OpenFiles()
		{
			...
			_mapData = new SequentialFileWriter(World.MapDataPath, _metrics);
			_mapIndex = new SequentialFileWriter(World.MapIndexPath, _metrics);
			...
			WriteCount(_mapIndex, World.Maps.Count);
		}
