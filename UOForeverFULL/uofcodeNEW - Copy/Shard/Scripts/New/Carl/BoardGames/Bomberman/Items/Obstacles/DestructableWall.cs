...
namespace Solaris.BoardGames
{
    ...
    public class DestructableWall : BombermanObstacle
    {
        ...
        public DestructableWall(BombermanStyle style)
            : base(BombermanSettings.GetDestructableWallID(style), "Bombable Wall")
        {
        ...
        public override void Destroy()
        {
            ...
            if (Utility.RandomDouble() < BombermanSettings.UPGRADE_SPAWN_CHANCE)
            {
                if (BoardGameControlItem.State == BoardGameState.Active)
                {
                    ...
                    upgrade.RegisterToBoardGameControlItem(BoardGameControlItem,
                        new Point3D(Offset.X, Offset.Y, Offset.Z + 3));
                }
            }
