...
namespace Solaris.BoardGames
{
    ...
    public class GamePiece : Item
    {
        ...
        public GamePiece(int itemidmin, int itemidmax, string name)
            : this(Utility.RandomMinMax(itemidmin, itemidmax), name)
        {}

        //randomize itemid constructor
        public GamePiece(int itemidmin, int itemidmax, string name, bool blocklos)
            : this(Utility.RandomMinMax(itemidmin, itemidmax), name, blocklos)
        {}
        ...
        public void UpdatePosition()
        {
            ...
            {
                MoveToWorld(
                    new Point3D(BoardGameControlItem.X + BoardGameControlItem.BoardOffset.X + Offset.X,
                        BoardGameControlItem.Y + BoardGameControlItem.BoardOffset.Y + Offset.Y,
                        BoardGameControlItem.Z + BoardGameControlItem.BoardOffset.Z + Offset.Z),
                    BoardGameControlItem.BoardMap);
            }
            else
            {
                Delete();
        ...
        public override void OnLocationChange(Point3D old)
        {
            if (BoardGameControlItem != null)
            {
                BoardGameControlItem.Location = new Point3D(X - BoardGameControlItem.BoardOffset.X - Offset.X,
                    Y - BoardGameControlItem.BoardOffset.Y - Offset.Y, Z - BoardGameControlItem.BoardOffset.Z - Offset.Z);
            }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(_Blocker);

            writer.Write(BoardGameControlItem);
