...
{
	public class BattleBornBeardDye : Item
	{
		[Constructable]
		public BattleBornBeardDye() : base( 6191 )
		{
            Name = "Battleborn Beard Dye";
            LootType = LootType.Blessed;
		}

        public BattleBornBeardDye(Serial serial)
            : base(serial)
        {
        }
		...
		private class ConfirmGump : BaseConfirmGump
		{
			...
			public override void Confirm( Mobile from )
			{
				if ( m_Item != null && !m_Item.Deleted && m_Item.IsChildOf( from.Backpack ) )
				{
					...
					{
						from.FacialHairHue = 1157;
						from.PlaySound( 0x240 );
						from.SendLocalizedMessage( 502618 ); // You dye your hair.
						m_Item.Delete();
					}
					else
						from.SendLocalizedMessage( 502619 ); // You have no hair to dye and you cannot use this.
				}
				else
