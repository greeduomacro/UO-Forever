...
{
	[Flags]
	public enum CartographySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class CartographySkillScroll : Item
	{
		private int m_SkillBonus;
		private CartographySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public CartographySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( CartographySkillScrollFlags.Newbie ); } set{ SetFlag( CartographySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( CartographySkillScrollFlags.Expires ); } set{ SetFlag( CartographySkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( CartographySkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( CartographySkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public CartographySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public CartographySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public CartographySkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public CartographySkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public CartographySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Cartography Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Cartography skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Cartography Skill Scroll";
		}
		...
		{
			from.SendGump( new CartographySkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(CartographySkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (CartographySkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class CartographySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private CartographySkillScroll m_CartographySkillScroll;
		private Mobile m_Target;

		public CartographySkillScrollGump ( Mobile from, Mobile target, CartographySkillScroll ball ) : base ( 20, 20 )
		{
			m_CartographySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_CartographySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_CartographySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_CartographySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_CartographySkillScroll.Deleted )
				return;

			if ( m_CartographySkillScroll.Expires && DateTime.UtcNow >= m_CartographySkillScroll.ExpireDate )
				m_CartographySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_CartographySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_CartographySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_CartographySkillScroll.SkillBonus;

				List<Skill> decreased = m_CartographySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_CartographySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_CartographySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_CartographySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_CartographySkillScroll.Rechargable )
							m_CartographySkillScroll.Delete();
					}
					else
