...
{
	[Flags]
	public enum HealingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class HealingSkillScroll : Item
	{
		private int m_SkillBonus;
		private HealingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public HealingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( HealingSkillScrollFlags.Newbie ); } set{ SetFlag( HealingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( HealingSkillScrollFlags.Expires ); } set{ SetFlag( HealingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( HealingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( HealingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public HealingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public HealingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public HealingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public HealingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public HealingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Healing Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Healing Skill Scroll";
		}
		...
		{
			from.SendGump( new HealingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(HealingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (HealingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class HealingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private HealingSkillScroll m_HealingSkillScroll;
		private Mobile m_Target;

		public HealingSkillScrollGump ( Mobile from, Mobile target, HealingSkillScroll ball ) : base ( 20, 20 )
		{
			m_HealingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_HealingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_HealingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_HealingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_HealingSkillScroll.Deleted )
				return;

			if ( m_HealingSkillScroll.Expires && DateTime.UtcNow >= m_HealingSkillScroll.ExpireDate )
				m_HealingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_HealingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_HealingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_HealingSkillScroll.SkillBonus;

				List<Skill> decreased = m_HealingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_HealingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_HealingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_HealingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_HealingSkillScroll.Rechargable )
							m_HealingSkillScroll.Delete();
					}
					else
