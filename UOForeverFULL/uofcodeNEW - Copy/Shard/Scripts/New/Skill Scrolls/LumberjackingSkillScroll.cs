...
{
	[Flags]
	public enum LumberjackingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class LumberjackingSkillScroll : Item
	{
		private int m_SkillBonus;
		private LumberjackingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public LumberjackingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( LumberjackingSkillScrollFlags.Newbie ); } set{ SetFlag( LumberjackingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( LumberjackingSkillScrollFlags.Expires ); } set{ SetFlag( LumberjackingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( LumberjackingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( LumberjackingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public LumberjackingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public LumberjackingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public LumberjackingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public LumberjackingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public LumberjackingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Lumberjacking Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Lumberjacking skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Lumberjacking Skill Scroll";
		}
		...
		{
			from.SendGump( new LumberjackingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(LumberjackingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (LumberjackingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class LumberjackingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private LumberjackingSkillScroll m_LumberjackingSkillScroll;
		private Mobile m_Target;

		public LumberjackingSkillScrollGump ( Mobile from, Mobile target, LumberjackingSkillScroll ball ) : base ( 20, 20 )
		{
			m_LumberjackingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_LumberjackingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_LumberjackingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_LumberjackingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_LumberjackingSkillScroll.Deleted )
				return;

			if ( m_LumberjackingSkillScroll.Expires && DateTime.UtcNow >= m_LumberjackingSkillScroll.ExpireDate )
				m_LumberjackingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_LumberjackingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_LumberjackingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_LumberjackingSkillScroll.SkillBonus;

				List<Skill> decreased = m_LumberjackingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_LumberjackingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_LumberjackingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_LumberjackingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_LumberjackingSkillScroll.Rechargable )
							m_LumberjackingSkillScroll.Delete();
					}
					else
