...
{
	[Flags]
	public enum ParrySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class ParrySkillScroll : Item
	{
		private int m_SkillBonus;
		private ParrySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public ParrySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( ParrySkillScrollFlags.Newbie ); } set{ SetFlag( ParrySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( ParrySkillScrollFlags.Expires ); } set{ SetFlag( ParrySkillScrollFlags.Expires, value ); InvalidateProperties(); } }
		...
		[Constructable]
		public ParrySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public ParrySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public ParrySkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public ParrySkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public ParrySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Parry Skill Scroll"; } }
		...
		{
			from.SendGump( new ParrySkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(ParrySkillScroll) ) )
				SendGump( from );
			else
...
{
	public class ParrySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private ParrySkillScroll m_ParrySkillScroll;
		private Mobile m_Target;

		public ParrySkillScrollGump ( Mobile from, Mobile target, ParrySkillScroll ball ) : base ( 20, 20 )
		{
			m_ParrySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_ParrySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_ParrySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_ParrySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_ParrySkillScroll.Deleted )
				return;

			if ( m_ParrySkillScroll.Expires && DateTime.UtcNow >= m_ParrySkillScroll.ExpireDate )
				m_ParrySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_ParrySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_ParrySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_ParrySkillScroll.SkillBonus;

				List<Skill> decreased = m_ParrySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_ParrySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_ParrySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_ParrySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_ParrySkillScroll.Rechargable )
							m_ParrySkillScroll.Delete();
					}
					else
