...
{
	[Flags]
	public enum LockpickingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class LockpickingSkillScroll : Item
	{
		private int m_SkillBonus;
		private LockpickingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public LockpickingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( LockpickingSkillScrollFlags.Newbie ); } set{ SetFlag( LockpickingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( LockpickingSkillScrollFlags.Expires ); } set{ SetFlag( LockpickingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( LockpickingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( LockpickingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public LockpickingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public LockpickingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public LockpickingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public LockpickingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public LockpickingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Lockpicking Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Lockpicking skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Lockpicking Skill Scroll";
		}
		...
		{
			from.SendGump( new LockpickingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(LockpickingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (LockpickingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class LockpickingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private LockpickingSkillScroll m_LockpickingSkillScroll;
		private Mobile m_Target;

		public LockpickingSkillScrollGump ( Mobile from, Mobile target, LockpickingSkillScroll ball ) : base ( 20, 20 )
		{
			m_LockpickingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_LockpickingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_LockpickingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_LockpickingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_LockpickingSkillScroll.Deleted )
				return;

			if ( m_LockpickingSkillScroll.Expires && DateTime.UtcNow >= m_LockpickingSkillScroll.ExpireDate )
				m_LockpickingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_LockpickingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_LockpickingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_LockpickingSkillScroll.SkillBonus;

				List<Skill> decreased = m_LockpickingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_LockpickingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_LockpickingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_LockpickingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_LockpickingSkillScroll.Rechargable )
							m_LockpickingSkillScroll.Delete();
					}
					else
