...
{
	[Flags]
	public enum ArcherySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class ArcherySkillScroll : Item
	{
		private int m_SkillBonus;
		private ArcherySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public ArcherySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( ArcherySkillScrollFlags.Newbie ); } set{ SetFlag( ArcherySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( ArcherySkillScrollFlags.Expires ); } set{ SetFlag( ArcherySkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( ArcherySkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( ArcherySkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public ArcherySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public ArcherySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public ArcherySkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public ArcherySkillScroll() : this( 1 )
		{
		}

		public ArcherySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Archery Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Archery Skill Scroll";
		}
		...
		{
			from.SendGump( new ArcherySkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(ArcherySkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (ArcherySkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class ArcherySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private ArcherySkillScroll m_ArcherySkillScroll;
		private Mobile m_Target;

		public ArcherySkillScrollGump ( Mobile from, Mobile target, ArcherySkillScroll ball ) : base ( 20, 20 )
		{
			m_ArcherySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_ArcherySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_ArcherySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_ArcherySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_ArcherySkillScroll.Deleted )
				return;

			if ( m_ArcherySkillScroll.Expires && DateTime.UtcNow >= m_ArcherySkillScroll.ExpireDate )
				m_ArcherySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_ArcherySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_ArcherySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_ArcherySkillScroll.SkillBonus;

				List<Skill> decreased = m_ArcherySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_ArcherySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_ArcherySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_ArcherySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_ArcherySkillScroll.Rechargable )
							m_ArcherySkillScroll.Delete();
					}
					else
