...
{
	[Flags]
	public enum EvalIntSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class EvalIntSkillScroll : Item
	{
		private int m_SkillBonus;
		private EvalIntSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public EvalIntSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( EvalIntSkillScrollFlags.Newbie ); } set{ SetFlag( EvalIntSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( EvalIntSkillScrollFlags.Expires ); } set{ SetFlag( EvalIntSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( EvalIntSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( EvalIntSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public EvalIntSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public EvalIntSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public EvalIntSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public EvalIntSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public EvalIntSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a EvalInt Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} EvalInt Skill Scroll";
		}
		...
		{
			from.SendGump( new EvalIntSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(EvalIntSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (EvalIntSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class EvalIntSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private EvalIntSkillScroll m_EvalIntSkillScroll;
		private Mobile m_Target;

		public EvalIntSkillScrollGump ( Mobile from, Mobile target, EvalIntSkillScroll ball ) : base ( 20, 20 )
		{
			m_EvalIntSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_EvalIntSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_EvalIntSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_EvalIntSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_EvalIntSkillScroll.Deleted )
				return;

			if ( m_EvalIntSkillScroll.Expires && DateTime.UtcNow >= m_EvalIntSkillScroll.ExpireDate )
				m_EvalIntSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_EvalIntSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_EvalIntSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_EvalIntSkillScroll.SkillBonus;

				List<Skill> decreased = m_EvalIntSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_EvalIntSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_EvalIntSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_EvalIntSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_EvalIntSkillScroll.Rechargable )
							m_EvalIntSkillScroll.Delete();
					}
					else
