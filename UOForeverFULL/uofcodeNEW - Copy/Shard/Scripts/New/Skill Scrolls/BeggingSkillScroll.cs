...
{
	[Flags]
	public enum BeggingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class BeggingSkillScroll : Item
	{
		private int m_SkillBonus;
		private BeggingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public BeggingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( BeggingSkillScrollFlags.Newbie ); } set{ SetFlag( BeggingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( BeggingSkillScrollFlags.Expires ); } set{ SetFlag( BeggingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( BeggingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( BeggingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public BeggingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public BeggingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public BeggingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public BeggingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public BeggingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Begging Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Begging Skill Scroll";
		}
		...
		{
			from.SendGump( new BeggingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(BeggingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (BeggingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class BeggingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private BeggingSkillScroll m_BeggingSkillScroll;
		private Mobile m_Target;

		public BeggingSkillScrollGump ( Mobile from, Mobile target, BeggingSkillScroll ball ) : base ( 20, 20 )
		{
			m_BeggingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_BeggingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_BeggingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_BeggingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_BeggingSkillScroll.Deleted )
				return;

			if ( m_BeggingSkillScroll.Expires && DateTime.UtcNow >= m_BeggingSkillScroll.ExpireDate )
				m_BeggingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_BeggingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_BeggingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_BeggingSkillScroll.SkillBonus;

				List<Skill> decreased = m_BeggingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_BeggingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_BeggingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_BeggingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_BeggingSkillScroll.Rechargable )
							m_BeggingSkillScroll.Delete();
					}
					else
