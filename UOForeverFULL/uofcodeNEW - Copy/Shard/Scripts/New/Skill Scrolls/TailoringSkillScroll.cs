...
{
	[Flags]
	public enum TailoringSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class TailoringSkillScroll : Item
	{
		private int m_SkillBonus;
		private TailoringSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public TailoringSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( TailoringSkillScrollFlags.Newbie ); } set{ SetFlag( TailoringSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( TailoringSkillScrollFlags.Expires ); } set{ SetFlag( TailoringSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( TailoringSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( TailoringSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public TailoringSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public TailoringSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public TailoringSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public TailoringSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public TailoringSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Tailoring Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Tailoring skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Tailoring Skill Scroll";
		}
		...
		{
			from.SendGump( new TailoringSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(TailoringSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (TailoringSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class TailoringSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private TailoringSkillScroll m_TailoringSkillScroll;
		private Mobile m_Target;

		public TailoringSkillScrollGump ( Mobile from, Mobile target, TailoringSkillScroll ball ) : base ( 20, 20 )
		{
			m_TailoringSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_TailoringSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_TailoringSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_TailoringSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_TailoringSkillScroll.Deleted )
				return;

			if ( m_TailoringSkillScroll.Expires && DateTime.UtcNow >= m_TailoringSkillScroll.ExpireDate )
				m_TailoringSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_TailoringSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_TailoringSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_TailoringSkillScroll.SkillBonus;

				List<Skill> decreased = m_TailoringSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_TailoringSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_TailoringSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_TailoringSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_TailoringSkillScroll.Rechargable )
							m_TailoringSkillScroll.Delete();
					}
					else
