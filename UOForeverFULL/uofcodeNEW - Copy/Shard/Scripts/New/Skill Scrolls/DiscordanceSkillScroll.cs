...
{
	[Flags]
	public enum DiscordanceSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class DiscordanceSkillScroll : Item
	{
		private int m_SkillBonus;
		private DiscordanceSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public DiscordanceSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( DiscordanceSkillScrollFlags.Newbie ); } set{ SetFlag( DiscordanceSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( DiscordanceSkillScrollFlags.Expires ); } set{ SetFlag( DiscordanceSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( DiscordanceSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( DiscordanceSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public DiscordanceSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public DiscordanceSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public DiscordanceSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public DiscordanceSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public DiscordanceSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Discordance Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Discordance skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Discordance Skill Scroll";
		}
		...
		{
			from.SendGump( new DiscordanceSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(DiscordanceSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (DiscordanceSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class DiscordanceSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private DiscordanceSkillScroll m_DiscordanceSkillScroll;
		private Mobile m_Target;

		public DiscordanceSkillScrollGump ( Mobile from, Mobile target, DiscordanceSkillScroll ball ) : base ( 20, 20 )
		{
			m_DiscordanceSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_DiscordanceSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_DiscordanceSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_DiscordanceSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_DiscordanceSkillScroll.Deleted )
				return;

			if ( m_DiscordanceSkillScroll.Expires && DateTime.UtcNow >= m_DiscordanceSkillScroll.ExpireDate )
				m_DiscordanceSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_DiscordanceSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_DiscordanceSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_DiscordanceSkillScroll.SkillBonus;

				List<Skill> decreased = m_DiscordanceSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_DiscordanceSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_DiscordanceSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_DiscordanceSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_DiscordanceSkillScroll.Rechargable )
							m_DiscordanceSkillScroll.Delete();
					}
					else
