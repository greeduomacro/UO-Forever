...
{
	[Flags]
	public enum TrackingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class TrackingSkillScroll : Item
	{
		private int m_SkillBonus;
		private TrackingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public TrackingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( TrackingSkillScrollFlags.Newbie ); } set{ SetFlag( TrackingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( TrackingSkillScrollFlags.Expires ); } set{ SetFlag( TrackingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( TrackingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( TrackingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public TrackingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public TrackingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public TrackingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public TrackingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public TrackingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Tracking Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Tracking skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Tracking Skill Scroll";
		}
		...
		{
			from.SendGump( new TrackingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(TrackingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (TrackingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class TrackingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private TrackingSkillScroll m_TrackingSkillScroll;
		private Mobile m_Target;

		public TrackingSkillScrollGump ( Mobile from, Mobile target, TrackingSkillScroll ball ) : base ( 20, 20 )
		{
			m_TrackingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_TrackingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_TrackingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_TrackingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
