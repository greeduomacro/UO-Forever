...
{
	[Flags]
	public enum SnoopingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class SnoopingSkillScroll : Item
	{
		private int m_SkillBonus;
		private SnoopingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public SnoopingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( SnoopingSkillScrollFlags.Newbie ); } set{ SetFlag( SnoopingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( SnoopingSkillScrollFlags.Expires ); } set{ SetFlag( SnoopingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( SnoopingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( SnoopingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public SnoopingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public SnoopingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public SnoopingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public SnoopingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public SnoopingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Snooping Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Snooping skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Snooping Skill Scroll";
		}
		...
		{
			from.SendGump( new SnoopingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(SnoopingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (SnoopingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class SnoopingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private SnoopingSkillScroll m_SnoopingSkillScroll;
		private Mobile m_Target;

		public SnoopingSkillScrollGump ( Mobile from, Mobile target, SnoopingSkillScroll ball ) : base ( 20, 20 )
		{
			m_SnoopingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_SnoopingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_SnoopingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_SnoopingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_SnoopingSkillScroll.Deleted )
				return;

			if ( m_SnoopingSkillScroll.Expires && DateTime.UtcNow >= m_SnoopingSkillScroll.ExpireDate )
				m_SnoopingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_SnoopingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_SnoopingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_SnoopingSkillScroll.SkillBonus;

				List<Skill> decreased = m_SnoopingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_SnoopingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_SnoopingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_SnoopingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_SnoopingSkillScroll.Rechargable )
							m_SnoopingSkillScroll.Delete();
					}
					else
