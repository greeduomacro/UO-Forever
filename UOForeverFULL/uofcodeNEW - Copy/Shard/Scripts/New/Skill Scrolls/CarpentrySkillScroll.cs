...
{
	[Flags]
	public enum CarpentrySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class CarpentrySkillScroll : Item
	{
		private int m_SkillBonus;
		private CarpentrySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public CarpentrySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( CarpentrySkillScrollFlags.Newbie ); } set{ SetFlag( CarpentrySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( CarpentrySkillScrollFlags.Expires ); } set{ SetFlag( CarpentrySkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( CarpentrySkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( CarpentrySkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public CarpentrySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public CarpentrySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public CarpentrySkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public CarpentrySkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public CarpentrySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Carpentry Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Carpentry skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Carpentry Skill Scroll";
		}
		...
		{
			from.SendGump( new CarpentrySkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(CarpentrySkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (CarpentrySkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class CarpentrySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private CarpentrySkillScroll m_CarpentrySkillScroll;
		private Mobile m_Target;

		public CarpentrySkillScrollGump ( Mobile from, Mobile target, CarpentrySkillScroll ball ) : base ( 20, 20 )
		{
			m_CarpentrySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_CarpentrySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_CarpentrySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_CarpentrySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_CarpentrySkillScroll.Deleted )
				return;

			if ( m_CarpentrySkillScroll.Expires && DateTime.UtcNow >= m_CarpentrySkillScroll.ExpireDate )
				m_CarpentrySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_CarpentrySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_CarpentrySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_CarpentrySkillScroll.SkillBonus;

				List<Skill> decreased = m_CarpentrySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_CarpentrySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_CarpentrySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_CarpentrySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_CarpentrySkillScroll.Rechargable )
							m_CarpentrySkillScroll.Delete();
					}
					else
