...
{
	[Flags]
	public enum RemoveTrapSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class RemoveTrapSkillScroll : Item
	{
		private int m_SkillBonus;
		private RemoveTrapSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public RemoveTrapSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( RemoveTrapSkillScrollFlags.Newbie ); } set{ SetFlag( RemoveTrapSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( RemoveTrapSkillScrollFlags.Expires ); } set{ SetFlag( RemoveTrapSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( RemoveTrapSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( RemoveTrapSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public RemoveTrapSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public RemoveTrapSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public RemoveTrapSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public RemoveTrapSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public RemoveTrapSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a RemoveTrap Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 RemoveTrap skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} RemoveTrap Skill Scroll";
		}
		...
		{
			from.SendGump( new RemoveTrapSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(RemoveTrapSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (RemoveTrapSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class RemoveTrapSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private RemoveTrapSkillScroll m_RemoveTrapSkillScroll;
		private Mobile m_Target;

		public RemoveTrapSkillScrollGump ( Mobile from, Mobile target, RemoveTrapSkillScroll ball ) : base ( 20, 20 )
		{
			m_RemoveTrapSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_RemoveTrapSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_RemoveTrapSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_RemoveTrapSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_RemoveTrapSkillScroll.Deleted )
				return;

			if ( m_RemoveTrapSkillScroll.Expires && DateTime.UtcNow >= m_RemoveTrapSkillScroll.ExpireDate )
				m_RemoveTrapSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_RemoveTrapSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_RemoveTrapSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_RemoveTrapSkillScroll.SkillBonus;

				List<Skill> decreased = m_RemoveTrapSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_RemoveTrapSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_RemoveTrapSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_RemoveTrapSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_RemoveTrapSkillScroll.Rechargable )
							m_RemoveTrapSkillScroll.Delete();
					}
					else
