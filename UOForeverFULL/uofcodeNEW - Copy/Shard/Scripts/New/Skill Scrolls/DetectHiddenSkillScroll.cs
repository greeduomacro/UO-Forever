...
{
	[Flags]
	public enum DetectHiddenSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class DetectHiddenSkillScroll : Item
	{
		private int m_SkillBonus;
		private DetectHiddenSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public DetectHiddenSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( DetectHiddenSkillScrollFlags.Newbie ); } set{ SetFlag( DetectHiddenSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( DetectHiddenSkillScrollFlags.Expires ); } set{ SetFlag( DetectHiddenSkillScrollFlags.Expires, value ); InvalidateProperties(); } }
		...
		[Constructable]
		public DetectHiddenSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public DetectHiddenSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public DetectHiddenSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public DetectHiddenSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public DetectHiddenSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a DetectHidden Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 DetectHidden skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} DetectHidden Skill Scroll";
		}
		...
		{
			from.SendGump( new DetectHiddenSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(DetectHiddenSkillScroll) ) )
				SendGump( from );
			else
...
{
	public class DetectHiddenSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private DetectHiddenSkillScroll m_DetectHiddenSkillScroll;
		private Mobile m_Target;

		public DetectHiddenSkillScrollGump ( Mobile from, Mobile target, DetectHiddenSkillScroll ball ) : base ( 20, 20 )
		{
			m_DetectHiddenSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_DetectHiddenSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_DetectHiddenSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_DetectHiddenSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_DetectHiddenSkillScroll.Deleted )
				return;

			if ( m_DetectHiddenSkillScroll.Expires && DateTime.UtcNow >= m_DetectHiddenSkillScroll.ExpireDate )
				m_DetectHiddenSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_DetectHiddenSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_DetectHiddenSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_DetectHiddenSkillScroll.SkillBonus;

				List<Skill> decreased = m_DetectHiddenSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_DetectHiddenSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_DetectHiddenSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_DetectHiddenSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_DetectHiddenSkillScroll.Rechargable )
							m_DetectHiddenSkillScroll.Delete();
					}
					else
