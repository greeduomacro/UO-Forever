...
{
	[Flags]
	public enum CampingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class CampingSkillScroll : Item
	{
		private int m_SkillBonus;
		private CampingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public CampingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( CampingSkillScrollFlags.Newbie ); } set{ SetFlag( CampingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( CampingSkillScrollFlags.Expires ); } set{ SetFlag( CampingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( CampingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( CampingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public CampingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public CampingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public CampingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public CampingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public CampingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Camping Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Camping Skill Scroll";
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(CampingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (CampingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class CampingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private CampingSkillScroll m_CampingSkillScroll;
		private Mobile m_Target;

		public CampingSkillScrollGump ( Mobile from, Mobile target, CampingSkillScroll ball ) : base ( 20, 20 )
		{
			m_CampingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_CampingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_CampingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_CampingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_CampingSkillScroll.Deleted )
				return;

			if ( m_CampingSkillScroll.Expires && DateTime.UtcNow >= m_CampingSkillScroll.ExpireDate )
				m_CampingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_CampingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_CampingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_CampingSkillScroll.SkillBonus;

				List<Skill> decreased = m_CampingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_CampingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_CampingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_CampingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_CampingSkillScroll.Rechargable )
							m_CampingSkillScroll.Delete();
					}
					else
