...
{
	[Flags]
	public enum HerdingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class HerdingSkillScroll : Item
	{
		private int m_SkillBonus;
		private HerdingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public HerdingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( HerdingSkillScrollFlags.Newbie ); } set{ SetFlag( HerdingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( HerdingSkillScrollFlags.Expires ); } set{ SetFlag( HerdingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( HerdingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( HerdingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public HerdingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public HerdingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public HerdingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public HerdingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public HerdingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Herding Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Herding Skill Scroll";
		}
		...
		{
			from.SendGump( new HerdingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(HerdingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (HerdingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class HerdingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private HerdingSkillScroll m_HerdingSkillScroll;
		private Mobile m_Target;

		public HerdingSkillScrollGump ( Mobile from, Mobile target, HerdingSkillScroll ball ) : base ( 20, 20 )
		{
			m_HerdingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_HerdingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_HerdingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_HerdingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_HerdingSkillScroll.Deleted )
				return;

			if ( m_HerdingSkillScroll.Expires && DateTime.UtcNow >= m_HerdingSkillScroll.ExpireDate )
				m_HerdingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_HerdingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_HerdingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_HerdingSkillScroll.SkillBonus;

				List<Skill> decreased = m_HerdingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_HerdingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_HerdingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_HerdingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_HerdingSkillScroll.Rechargable )
							m_HerdingSkillScroll.Delete();
					}
					else
