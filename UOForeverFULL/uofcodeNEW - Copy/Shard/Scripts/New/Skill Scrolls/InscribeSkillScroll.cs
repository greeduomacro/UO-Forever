...
{
	[Flags]
	public enum InscribeSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class InscribeSkillScroll : Item
	{
		private int m_SkillBonus;
		private InscribeSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public InscribeSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( InscribeSkillScrollFlags.Newbie ); } set{ SetFlag( InscribeSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( InscribeSkillScrollFlags.Expires ); } set{ SetFlag( InscribeSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( InscribeSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( InscribeSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public InscribeSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public InscribeSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public InscribeSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public InscribeSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public InscribeSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Inscribe Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Inscribe skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Inscribe Skill Scroll";
		}
		...
		{
			from.SendGump( new InscribeSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(InscribeSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (InscribeSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class InscribeSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private InscribeSkillScroll m_InscribeSkillScroll;
		private Mobile m_Target;

		public InscribeSkillScrollGump ( Mobile from, Mobile target, InscribeSkillScroll ball ) : base ( 20, 20 )
		{
			m_InscribeSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_InscribeSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_InscribeSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_InscribeSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_InscribeSkillScroll.Deleted )
				return;

			if ( m_InscribeSkillScroll.Expires && DateTime.UtcNow >= m_InscribeSkillScroll.ExpireDate )
				m_InscribeSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_InscribeSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_InscribeSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_InscribeSkillScroll.SkillBonus;

				List<Skill> decreased = m_InscribeSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_InscribeSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_InscribeSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_InscribeSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_InscribeSkillScroll.Rechargable )
							m_InscribeSkillScroll.Delete();
					}
					else
