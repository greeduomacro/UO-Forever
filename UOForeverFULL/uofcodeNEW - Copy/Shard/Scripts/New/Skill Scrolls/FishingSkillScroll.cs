...
{
	[Flags]
	public enum FishingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class FishingSkillScroll : Item
	{
		private int m_SkillBonus;
		private FishingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public FishingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( FishingSkillScrollFlags.Newbie ); } set{ SetFlag( FishingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( FishingSkillScrollFlags.Expires ); } set{ SetFlag( FishingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( FishingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( FishingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public FishingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public FishingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public FishingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public FishingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public FishingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Fishing Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Fishing Skill Scroll";
		}
		...
		{
			from.SendGump( new FishingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(FishingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (FishingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class FishingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private FishingSkillScroll m_FishingSkillScroll;
		private Mobile m_Target;

		public FishingSkillScrollGump ( Mobile from, Mobile target, FishingSkillScroll ball ) : base ( 20, 20 )
		{
			m_FishingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_FishingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_FishingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_FishingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_FishingSkillScroll.Deleted )
				return;

			if ( m_FishingSkillScroll.Expires && DateTime.UtcNow >= m_FishingSkillScroll.ExpireDate )
				m_FishingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_FishingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_FishingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_FishingSkillScroll.SkillBonus;

				List<Skill> decreased = m_FishingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_FishingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_FishingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_FishingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_FishingSkillScroll.Rechargable )
							m_FishingSkillScroll.Delete();
					}
					else
