...
{
	[Flags]
	public enum AnimalLoreSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class AnimalLoreSkillScroll : Item
	{
		private int m_SkillBonus;
		private AnimalLoreSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public AnimalLoreSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( AnimalLoreSkillScrollFlags.Newbie ); } set{ SetFlag( AnimalLoreSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( AnimalLoreSkillScrollFlags.Expires ); } set{ SetFlag( AnimalLoreSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( AnimalLoreSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( AnimalLoreSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public AnimalLoreSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public AnimalLoreSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public AnimalLoreSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public AnimalLoreSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public AnimalLoreSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a AnimalLore Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 AnimalLore skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} AnimalLore Skill Scroll";
		}
		...
		{
			from.SendGump( new AnimalLoreSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(AnimalLoreSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (AnimalLoreSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class AnimalLoreSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private AnimalLoreSkillScroll m_AnimalLoreSkillScroll;
		private Mobile m_Target;

		public AnimalLoreSkillScrollGump ( Mobile from, Mobile target, AnimalLoreSkillScroll ball ) : base ( 20, 20 )
		{
			m_AnimalLoreSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_AnimalLoreSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_AnimalLoreSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_AnimalLoreSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_AnimalLoreSkillScroll.Deleted )
				return;

			if ( m_AnimalLoreSkillScroll.Expires && DateTime.UtcNow >= m_AnimalLoreSkillScroll.ExpireDate )
				m_AnimalLoreSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_AnimalLoreSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_AnimalLoreSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_AnimalLoreSkillScroll.SkillBonus;

				List<Skill> decreased = m_AnimalLoreSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_AnimalLoreSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_AnimalLoreSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_AnimalLoreSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_AnimalLoreSkillScroll.Rechargable )
							m_AnimalLoreSkillScroll.Delete();
					}
					else
