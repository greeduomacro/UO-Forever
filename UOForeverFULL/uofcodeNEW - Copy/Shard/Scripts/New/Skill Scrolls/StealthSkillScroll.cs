...
{
	[Flags]
	public enum StealthSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class StealthSkillScroll : Item
	{
		private int m_SkillBonus;
		private StealthSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public StealthSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( StealthSkillScrollFlags.Newbie ); } set{ SetFlag( StealthSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( StealthSkillScrollFlags.Expires ); } set{ SetFlag( StealthSkillScrollFlags.Expires, value ); InvalidateProperties(); } }
		...
		[Constructable]
		public StealthSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public StealthSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public StealthSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public StealthSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public StealthSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Stealth Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Stealth Skill Scroll";
		}
		...
		{
			from.SendGump( new StealthSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(StealthSkillScroll) ) )
				SendGump( from );
			else
...
{
	public class StealthSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private StealthSkillScroll m_StealthSkillScroll;
		private Mobile m_Target;

		public StealthSkillScrollGump ( Mobile from, Mobile target, StealthSkillScroll ball ) : base ( 20, 20 )
		{
			m_StealthSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_StealthSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_StealthSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_StealthSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_StealthSkillScroll.Deleted )
				return;

			if ( m_StealthSkillScroll.Expires && DateTime.UtcNow >= m_StealthSkillScroll.ExpireDate )
				m_StealthSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_StealthSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_StealthSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_StealthSkillScroll.SkillBonus;

				List<Skill> decreased = m_StealthSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_StealthSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_StealthSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_StealthSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_StealthSkillScroll.Rechargable )
							m_StealthSkillScroll.Delete();
					}
					else
