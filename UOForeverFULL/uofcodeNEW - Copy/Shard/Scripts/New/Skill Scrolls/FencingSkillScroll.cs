...
{
	[Flags]
	public enum FencingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class FencingSkillScroll : Item
	{
		private int m_SkillBonus;
		private FencingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public FencingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( FencingSkillScrollFlags.Newbie ); } set{ SetFlag( FencingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( FencingSkillScrollFlags.Expires ); } set{ SetFlag( FencingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( FencingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( FencingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public FencingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public FencingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public FencingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public FencingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public FencingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Fencing Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Fencing Skill Scroll";
		}
		...
		{
			from.SendGump( new FencingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(FencingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (FencingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class FencingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private FencingSkillScroll m_FencingSkillScroll;
		private Mobile m_Target;

		public FencingSkillScrollGump ( Mobile from, Mobile target, FencingSkillScroll ball ) : base ( 20, 20 )
		{
			m_FencingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_FencingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_FencingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_FencingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_FencingSkillScroll.Deleted )
				return;

			if ( m_FencingSkillScroll.Expires && DateTime.UtcNow >= m_FencingSkillScroll.ExpireDate )
				m_FencingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_FencingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_FencingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_FencingSkillScroll.SkillBonus;

				List<Skill> decreased = m_FencingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_FencingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_FencingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_FencingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_FencingSkillScroll.Rechargable )
							m_FencingSkillScroll.Delete();
					}
					else
