...
{
	[Flags]
	public enum ItemIDSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class ItemIDSkillScroll : Item
	{
		private int m_SkillBonus;
		private ItemIDSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public ItemIDSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( ItemIDSkillScrollFlags.Newbie ); } set{ SetFlag( ItemIDSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( ItemIDSkillScrollFlags.Expires ); } set{ SetFlag( ItemIDSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( ItemIDSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( ItemIDSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public ItemIDSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public ItemIDSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public ItemIDSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public ItemIDSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public ItemIDSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a ItemID Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} ItemID Skill Scroll";
		}
		...
		{
			from.SendGump( new ItemIDSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(ItemIDSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (ItemIDSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class ItemIDSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private ItemIDSkillScroll m_ItemIDSkillScroll;
		private Mobile m_Target;

		public ItemIDSkillScrollGump ( Mobile from, Mobile target, ItemIDSkillScroll ball ) : base ( 20, 20 )
		{
			m_ItemIDSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_ItemIDSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_ItemIDSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_ItemIDSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_ItemIDSkillScroll.Deleted )
				return;

			if ( m_ItemIDSkillScroll.Expires && DateTime.UtcNow >= m_ItemIDSkillScroll.ExpireDate )
				m_ItemIDSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_ItemIDSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_ItemIDSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_ItemIDSkillScroll.SkillBonus;

				List<Skill> decreased = m_ItemIDSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_ItemIDSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_ItemIDSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_ItemIDSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_ItemIDSkillScroll.Rechargable )
							m_ItemIDSkillScroll.Delete();
					}
					else
