...
{
	[Flags]
	public enum MeditationSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class MeditationSkillScroll : Item
	{
		private int m_SkillBonus;
		private MeditationSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public MeditationSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( MeditationSkillScrollFlags.Newbie ); } set{ SetFlag( MeditationSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( MeditationSkillScrollFlags.Expires ); } set{ SetFlag( MeditationSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( MeditationSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( MeditationSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public MeditationSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public MeditationSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public MeditationSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public MeditationSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public MeditationSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Meditation Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Meditation skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Meditation Skill Scroll";
		}
		...
		{
			from.SendGump( new MeditationSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(MeditationSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (MeditationSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class MeditationSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private MeditationSkillScroll m_MeditationSkillScroll;
		private Mobile m_Target;

		public MeditationSkillScrollGump ( Mobile from, Mobile target, MeditationSkillScroll ball ) : base ( 20, 20 )
		{
			m_MeditationSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_MeditationSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_MeditationSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_MeditationSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_MeditationSkillScroll.Deleted )
				return;

			if ( m_MeditationSkillScroll.Expires && DateTime.UtcNow >= m_MeditationSkillScroll.ExpireDate )
				m_MeditationSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_MeditationSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_MeditationSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_MeditationSkillScroll.SkillBonus;

				List<Skill> decreased = m_MeditationSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_MeditationSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_MeditationSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_MeditationSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_MeditationSkillScroll.Rechargable )
							m_MeditationSkillScroll.Delete();
					}
					else
