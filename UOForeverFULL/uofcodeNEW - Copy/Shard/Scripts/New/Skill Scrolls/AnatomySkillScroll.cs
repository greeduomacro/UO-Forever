...
{
	[Flags]
	public enum AnatomySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class AnatomySkillScroll : Item
	{
		private int m_SkillBonus;
		private AnatomySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public AnatomySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( AnatomySkillScrollFlags.Newbie ); } set{ SetFlag( AnatomySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( AnatomySkillScrollFlags.Expires ); } set{ SetFlag( AnatomySkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( AnatomySkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( AnatomySkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public AnatomySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public AnatomySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public AnatomySkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public AnatomySkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public AnatomySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Anatomy Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Anatomy Skill Scroll";
		}
		...
		{
			from.SendGump( new AnatomySkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(AnatomySkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (AnatomySkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class AnatomySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private AnatomySkillScroll m_AnatomySkillScroll;
		private Mobile m_Target;

		public AnatomySkillScrollGump ( Mobile from, Mobile target, AnatomySkillScroll ball ) : base ( 20, 20 )
		{
			m_AnatomySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_AnatomySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_AnatomySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_AnatomySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_AnatomySkillScroll.Deleted )
				return;

			if ( m_AnatomySkillScroll.Expires && DateTime.UtcNow >= m_AnatomySkillScroll.ExpireDate )
				m_AnatomySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_AnatomySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_AnatomySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_AnatomySkillScroll.SkillBonus;

				List<Skill> decreased = m_AnatomySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_AnatomySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_AnatomySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_AnatomySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_AnatomySkillScroll.Rechargable )
							m_AnatomySkillScroll.Delete();
					}
					else
