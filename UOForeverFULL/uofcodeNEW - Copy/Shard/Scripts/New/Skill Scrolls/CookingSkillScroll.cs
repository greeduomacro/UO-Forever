...
{
	[Flags]
	public enum CookingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class CookingSkillScroll : Item
	{
		private int m_SkillBonus;
		private CookingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public CookingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( CookingSkillScrollFlags.Newbie ); } set{ SetFlag( CookingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( CookingSkillScrollFlags.Expires ); } set{ SetFlag( CookingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( CookingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( CookingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public CookingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public CookingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public CookingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public CookingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public CookingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Cooking Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Cooking Skill Scroll";
		}
		...
		{
			from.SendGump( new CookingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(CookingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (CookingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class CookingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private CookingSkillScroll m_CookingSkillScroll;
		private Mobile m_Target;

		public CookingSkillScrollGump ( Mobile from, Mobile target, CookingSkillScroll ball ) : base ( 20, 20 )
		{
			m_CookingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_CookingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_CookingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_CookingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_CookingSkillScroll.Deleted )
				return;

			if ( m_CookingSkillScroll.Expires && DateTime.UtcNow >= m_CookingSkillScroll.ExpireDate )
				m_CookingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_CookingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_CookingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_CookingSkillScroll.SkillBonus;

				List<Skill> decreased = m_CookingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_CookingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_CookingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_CookingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_CookingSkillScroll.Rechargable )
							m_CookingSkillScroll.Delete();
					}
					else
