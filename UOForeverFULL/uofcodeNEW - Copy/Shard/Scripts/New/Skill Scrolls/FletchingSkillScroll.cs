...
{
	[Flags]
	public enum FletchingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class FletchingSkillScroll : Item
	{
		private int m_SkillBonus;
		private FletchingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public FletchingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( FletchingSkillScrollFlags.Newbie ); } set{ SetFlag( FletchingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( FletchingSkillScrollFlags.Expires ); } set{ SetFlag( FletchingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( FletchingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( FletchingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public FletchingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public FletchingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public FletchingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public FletchingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public FletchingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Fletching Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Fletching skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Fletching Skill Scroll";
		}
		...
		{
			from.SendGump( new FletchingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(FletchingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (FletchingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class FletchingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private FletchingSkillScroll m_FletchingSkillScroll;
		private Mobile m_Target;

		public FletchingSkillScrollGump ( Mobile from, Mobile target, FletchingSkillScroll ball ) : base ( 20, 20 )
		{
			m_FletchingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_FletchingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_FletchingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_FletchingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_FletchingSkillScroll.Deleted )
				return;

			if ( m_FletchingSkillScroll.Expires && DateTime.UtcNow >= m_FletchingSkillScroll.ExpireDate )
				m_FletchingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_FletchingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_FletchingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_FletchingSkillScroll.SkillBonus;

				List<Skill> decreased = m_FletchingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_FletchingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_FletchingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_FletchingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_FletchingSkillScroll.Rechargable )
							m_FletchingSkillScroll.Delete();
					}
					else
