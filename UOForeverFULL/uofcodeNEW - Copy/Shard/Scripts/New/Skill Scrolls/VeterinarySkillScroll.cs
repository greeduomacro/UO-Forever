...
{
	[Flags]
	public enum VeterinarySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class VeterinarySkillScroll : Item
	{
		private int m_SkillBonus;
		private VeterinarySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public VeterinarySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( VeterinarySkillScrollFlags.Newbie ); } set{ SetFlag( VeterinarySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( VeterinarySkillScrollFlags.Expires ); } set{ SetFlag( VeterinarySkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( VeterinarySkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( VeterinarySkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public VeterinarySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public VeterinarySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public VeterinarySkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public VeterinarySkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public VeterinarySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Veterinary Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Veterinary skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Veterinary Skill Scroll";
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(VeterinarySkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (VeterinarySkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class VeterinarySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private VeterinarySkillScroll m_VeterinarySkillScroll;
		private Mobile m_Target;

		public VeterinarySkillScrollGump ( Mobile from, Mobile target, VeterinarySkillScroll ball ) : base ( 20, 20 )
		{
			m_VeterinarySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_VeterinarySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_VeterinarySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_VeterinarySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_VeterinarySkillScroll.Deleted )
				return;

			if ( m_VeterinarySkillScroll.Expires && DateTime.UtcNow >= m_VeterinarySkillScroll.ExpireDate )
				m_VeterinarySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_VeterinarySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_VeterinarySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_VeterinarySkillScroll.SkillBonus;

				List<Skill> decreased = m_VeterinarySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_VeterinarySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_VeterinarySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_VeterinarySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_VeterinarySkillScroll.Rechargable )
							m_VeterinarySkillScroll.Delete();
					}
					else
