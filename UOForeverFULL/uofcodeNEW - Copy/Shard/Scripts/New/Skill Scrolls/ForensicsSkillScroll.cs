...
{
	[Flags]
	public enum ForensicsSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class ForensicsSkillScroll : Item
	{
		private int m_SkillBonus;
		private ForensicsSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public ForensicsSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( ForensicsSkillScrollFlags.Newbie ); } set{ SetFlag( ForensicsSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( ForensicsSkillScrollFlags.Expires ); } set{ SetFlag( ForensicsSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( ForensicsSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( ForensicsSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public ForensicsSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public ForensicsSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public ForensicsSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public ForensicsSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public ForensicsSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Forensics Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Forensics skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Forensics Skill Scroll";
		}
		...
		{
			from.SendGump( new ForensicsSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(ForensicsSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (ForensicsSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class ForensicsSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private ForensicsSkillScroll m_ForensicsSkillScroll;
		private Mobile m_Target;

		public ForensicsSkillScrollGump ( Mobile from, Mobile target, ForensicsSkillScroll ball ) : base ( 20, 20 )
		{
			m_ForensicsSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_ForensicsSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_ForensicsSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_ForensicsSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_ForensicsSkillScroll.Deleted )
				return;

			if ( m_ForensicsSkillScroll.Expires && DateTime.UtcNow >= m_ForensicsSkillScroll.ExpireDate )
				m_ForensicsSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_ForensicsSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_ForensicsSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_ForensicsSkillScroll.SkillBonus;

				List<Skill> decreased = m_ForensicsSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_ForensicsSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_ForensicsSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_ForensicsSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_ForensicsSkillScroll.Rechargable )
							m_ForensicsSkillScroll.Delete();
					}
					else
