...
{
	[Flags]
	public enum PeacemakingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class PeacemakingSkillScroll : Item
	{
		private int m_SkillBonus;
		private PeacemakingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public PeacemakingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( PeacemakingSkillScrollFlags.Newbie ); } set{ SetFlag( PeacemakingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( PeacemakingSkillScrollFlags.Expires ); } set{ SetFlag( PeacemakingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( PeacemakingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( PeacemakingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public PeacemakingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public PeacemakingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public PeacemakingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public PeacemakingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public PeacemakingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Peacemaking Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Peacemaking skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Peacemaking Skill Scroll";
		}
		...
		{
			from.SendGump( new PeacemakingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(PeacemakingSkillScroll) ) )
				SendGump( from );
			else
...
{
	public class PeacemakingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private PeacemakingSkillScroll m_PeacemakingSkillScroll;
		private Mobile m_Target;

		public PeacemakingSkillScrollGump ( Mobile from, Mobile target, PeacemakingSkillScroll ball ) : base ( 20, 20 )
		{
			m_PeacemakingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_PeacemakingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_PeacemakingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_PeacemakingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_PeacemakingSkillScroll.Deleted )
				return;

			if ( m_PeacemakingSkillScroll.Expires && DateTime.UtcNow >= m_PeacemakingSkillScroll.ExpireDate )
				m_PeacemakingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_PeacemakingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_PeacemakingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_PeacemakingSkillScroll.SkillBonus;

				List<Skill> decreased = m_PeacemakingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_PeacemakingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_PeacemakingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_PeacemakingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_PeacemakingSkillScroll.Rechargable )
							m_PeacemakingSkillScroll.Delete();
					}
					else
