...
{
	[Flags]
	public enum StealingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class StealingSkillScroll : Item
	{
		private int m_SkillBonus;
		private StealingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public StealingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( StealingSkillScrollFlags.Newbie ); } set{ SetFlag( StealingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( StealingSkillScrollFlags.Expires ); } set{ SetFlag( StealingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( StealingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( StealingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public StealingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public StealingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public StealingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public StealingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public StealingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Stealing Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Stealing skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Stealing Skill Scroll";
		}
		...
		{
			from.SendGump( new StealingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(StealingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (StealingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class StealingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private StealingSkillScroll m_StealingSkillScroll;
		private Mobile m_Target;

		public StealingSkillScrollGump ( Mobile from, Mobile target, StealingSkillScroll ball ) : base ( 20, 20 )
		{
			m_StealingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_StealingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_StealingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_StealingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_StealingSkillScroll.Deleted )
				return;

			if ( m_StealingSkillScroll.Expires && DateTime.UtcNow >= m_StealingSkillScroll.ExpireDate )
				m_StealingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_StealingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_StealingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_StealingSkillScroll.SkillBonus;

				List<Skill> decreased = m_StealingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_StealingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_StealingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_StealingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_StealingSkillScroll.Rechargable )
							m_StealingSkillScroll.Delete();
					}
					else
