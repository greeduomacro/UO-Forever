...
{
	[Flags]
	public enum AnimalTamingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class AnimalTamingSkillScroll : Item
	{
		private int m_SkillBonus;
		private AnimalTamingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public AnimalTamingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( AnimalTamingSkillScrollFlags.Newbie ); } set{ SetFlag( AnimalTamingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( AnimalTamingSkillScrollFlags.Expires ); } set{ SetFlag( AnimalTamingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( AnimalTamingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( AnimalTamingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public AnimalTamingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public AnimalTamingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public AnimalTamingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public AnimalTamingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public AnimalTamingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a AnimalTaming Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 AnimalTaming skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} AnimalTaming Skill Scroll";
		}
		...
		{
			from.SendGump( new AnimalTamingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(AnimalTamingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (AnimalTamingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class AnimalTamingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private AnimalTamingSkillScroll m_AnimalTamingSkillScroll;
		private Mobile m_Target;

		public AnimalTamingSkillScrollGump ( Mobile from, Mobile target, AnimalTamingSkillScroll ball ) : base ( 20, 20 )
		{
			m_AnimalTamingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_AnimalTamingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_AnimalTamingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_AnimalTamingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_AnimalTamingSkillScroll.Deleted )
				return;

			if ( m_AnimalTamingSkillScroll.Expires && DateTime.UtcNow >= m_AnimalTamingSkillScroll.ExpireDate )
				m_AnimalTamingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_AnimalTamingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_AnimalTamingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_AnimalTamingSkillScroll.SkillBonus;

				List<Skill> decreased = m_AnimalTamingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_AnimalTamingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_AnimalTamingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_AnimalTamingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_AnimalTamingSkillScroll.Rechargable )
							m_AnimalTamingSkillScroll.Delete();
					}
					else
