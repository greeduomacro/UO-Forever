...
{
	[Flags]
	public enum MacingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class MacingSkillScroll : Item
	{
		private int m_SkillBonus;
		private MacingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public MacingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( MacingSkillScrollFlags.Newbie ); } set{ SetFlag( MacingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( MacingSkillScrollFlags.Expires ); } set{ SetFlag( MacingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( MacingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( MacingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public MacingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public MacingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public MacingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public MacingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public MacingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Macing Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Macing Skill Scroll";
		}
		...
		{
			from.SendGump( new MacingSkillScrollGump( from, from, this ) );
		}
...
{
	public class MacingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private MacingSkillScroll m_MacingSkillScroll;
		private Mobile m_Target;

		public MacingSkillScrollGump ( Mobile from, Mobile target, MacingSkillScroll ball ) : base ( 20, 20 )
		{
			m_MacingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_MacingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_MacingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_MacingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_MacingSkillScroll.Deleted )
				return;

			if ( m_MacingSkillScroll.Expires && DateTime.UtcNow >= m_MacingSkillScroll.ExpireDate )
				m_MacingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_MacingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_MacingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_MacingSkillScroll.SkillBonus;

				List<Skill> decreased = m_MacingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_MacingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_MacingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_MacingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_MacingSkillScroll.Rechargable )
							m_MacingSkillScroll.Delete();
					}
					else
