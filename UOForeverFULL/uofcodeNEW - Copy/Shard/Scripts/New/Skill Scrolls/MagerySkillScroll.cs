...
{
	[Flags]
	public enum MagerySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class MagerySkillScroll : Item
	{
		private int m_SkillBonus;
		private MagerySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public MagerySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( MagerySkillScrollFlags.Newbie ); } set{ SetFlag( MagerySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( MagerySkillScrollFlags.Expires ); } set{ SetFlag( MagerySkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( MagerySkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( MagerySkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public MagerySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public MagerySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public MagerySkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public MagerySkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public MagerySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Magery Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Magery Skill Scroll";
		}
		...
		{
			from.SendGump( new MagerySkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(MagerySkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (MagerySkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class MagerySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private MagerySkillScroll m_MagerySkillScroll;
		private Mobile m_Target;

		public MagerySkillScrollGump ( Mobile from, Mobile target, MagerySkillScroll ball ) : base ( 20, 20 )
		{
			m_MagerySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_MagerySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_MagerySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_MagerySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_MagerySkillScroll.Deleted )
				return;

			if ( m_MagerySkillScroll.Expires && DateTime.UtcNow >= m_MagerySkillScroll.ExpireDate )
				m_MagerySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_MagerySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_MagerySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_MagerySkillScroll.SkillBonus;

				List<Skill> decreased = m_MagerySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_MagerySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_MagerySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_MagerySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_MagerySkillScroll.Rechargable )
							m_MagerySkillScroll.Delete();
					}
					else
