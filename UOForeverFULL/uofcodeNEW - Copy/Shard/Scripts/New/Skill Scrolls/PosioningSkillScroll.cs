...
{
	[Flags]
	public enum PoisoningSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class PoisoningSkillScroll : Item
	{
		private int m_SkillBonus;
		private PoisoningSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public PoisoningSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( PoisoningSkillScrollFlags.Newbie ); } set{ SetFlag( PoisoningSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( PoisoningSkillScrollFlags.Expires ); } set{ SetFlag( PoisoningSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( PoisoningSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( PoisoningSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public PoisoningSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public PoisoningSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public PoisoningSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public PoisoningSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public PoisoningSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Poisoning Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Poisoning skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Poisoning Skill Scroll";
		}
		...
		{
			from.SendGump( new PoisoningSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(PoisoningSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (PoisoningSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class PoisoningSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private PoisoningSkillScroll m_PoisoningSkillScroll;
		private Mobile m_Target;

		public PoisoningSkillScrollGump ( Mobile from, Mobile target, PoisoningSkillScroll ball ) : base ( 20, 20 )
		{
			m_PoisoningSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_PoisoningSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_PoisoningSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_PoisoningSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_PoisoningSkillScroll.Deleted )
				return;

			if ( m_PoisoningSkillScroll.Expires && DateTime.UtcNow >= m_PoisoningSkillScroll.ExpireDate )
				m_PoisoningSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_PoisoningSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_PoisoningSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_PoisoningSkillScroll.SkillBonus;

				List<Skill> decreased = m_PoisoningSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_PoisoningSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_PoisoningSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_PoisoningSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_PoisoningSkillScroll.Rechargable )
							m_PoisoningSkillScroll.Delete();
					}
					else
