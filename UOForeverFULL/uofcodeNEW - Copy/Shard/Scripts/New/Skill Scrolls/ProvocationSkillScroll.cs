...
{
	[Flags]
	public enum ProvocationSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class ProvocationSkillScroll : Item
	{
		private int m_SkillBonus;
		private ProvocationSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public ProvocationSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( ProvocationSkillScrollFlags.Newbie ); } set{ SetFlag( ProvocationSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( ProvocationSkillScrollFlags.Expires ); } set{ SetFlag( ProvocationSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( ProvocationSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( ProvocationSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public ProvocationSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public ProvocationSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public ProvocationSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public ProvocationSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public ProvocationSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Provocation Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Provocation skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Provocation Skill Scroll";
		}
		...
		{
			from.SendGump( new ProvocationSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(ProvocationSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (ProvocationSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class ProvocationSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private ProvocationSkillScroll m_ProvocationSkillScroll;
		private Mobile m_Target;

		public ProvocationSkillScrollGump ( Mobile from, Mobile target, ProvocationSkillScroll ball ) : base ( 20, 20 )
		{
			m_ProvocationSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_ProvocationSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_ProvocationSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_ProvocationSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_ProvocationSkillScroll.Deleted )
				return;

			if ( m_ProvocationSkillScroll.Expires && DateTime.UtcNow >= m_ProvocationSkillScroll.ExpireDate )
				m_ProvocationSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_ProvocationSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_ProvocationSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_ProvocationSkillScroll.SkillBonus;

				List<Skill> decreased = m_ProvocationSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_ProvocationSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_ProvocationSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_ProvocationSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_ProvocationSkillScroll.Rechargable )
							m_ProvocationSkillScroll.Delete();
					}
					else
