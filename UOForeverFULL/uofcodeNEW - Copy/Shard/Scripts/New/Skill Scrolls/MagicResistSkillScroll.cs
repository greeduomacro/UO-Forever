...
{
	[Flags]
	public enum MagicResistSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class MagicResistSkillScroll : Item
	{
		private int m_SkillBonus;
		private MagicResistSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public MagicResistSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( MagicResistSkillScrollFlags.Newbie ); } set{ SetFlag( MagicResistSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( MagicResistSkillScrollFlags.Expires ); } set{ SetFlag( MagicResistSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( MagicResistSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( MagicResistSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public MagicResistSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public MagicResistSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public MagicResistSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public MagicResistSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public MagicResistSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a MagicResist Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 MagicResist skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} MagicResist Skill Scroll";
		}
		...
		{
			from.SendGump( new MagicResistSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(MagicResistSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (MagicResistSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class MagicResistSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private MagicResistSkillScroll m_MagicResistSkillScroll;
		private Mobile m_Target;

		public MagicResistSkillScrollGump ( Mobile from, Mobile target, MagicResistSkillScroll ball ) : base ( 20, 20 )
		{
			m_MagicResistSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_MagicResistSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_MagicResistSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_MagicResistSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_MagicResistSkillScroll.Deleted )
				return;

			if ( m_MagicResistSkillScroll.Expires && DateTime.UtcNow >= m_MagicResistSkillScroll.ExpireDate )
				m_MagicResistSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_MagicResistSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_MagicResistSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_MagicResistSkillScroll.SkillBonus;

				List<Skill> decreased = m_MagicResistSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_MagicResistSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_MagicResistSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_MagicResistSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_MagicResistSkillScroll.Rechargable )
							m_MagicResistSkillScroll.Delete();
					}
					else
