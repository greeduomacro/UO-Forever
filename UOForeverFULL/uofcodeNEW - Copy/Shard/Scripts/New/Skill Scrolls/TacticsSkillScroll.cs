...
{
	[Flags]
	public enum TacticsSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class TacticsSkillScroll : Item
	{
		private int m_SkillBonus;
		private TacticsSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public TacticsSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( TacticsSkillScrollFlags.Newbie ); } set{ SetFlag( TacticsSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( TacticsSkillScrollFlags.Expires ); } set{ SetFlag( TacticsSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( TacticsSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( TacticsSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public TacticsSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public TacticsSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public TacticsSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public TacticsSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public TacticsSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Tactics Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Tactics Skill Scroll";
		}
		...
		{
			from.SendGump( new TacticsSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(TacticsSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (TacticsSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class TacticsSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private TacticsSkillScroll m_TacticsSkillScroll;
		private Mobile m_Target;

		public TacticsSkillScrollGump ( Mobile from, Mobile target, TacticsSkillScroll ball ) : base ( 20, 20 )
		{
			m_TacticsSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_TacticsSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_TacticsSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_TacticsSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_TacticsSkillScroll.Deleted )
				return;

			if ( m_TacticsSkillScroll.Expires && DateTime.UtcNow >= m_TacticsSkillScroll.ExpireDate )
				m_TacticsSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_TacticsSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_TacticsSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_TacticsSkillScroll.SkillBonus;

				List<Skill> decreased = m_TacticsSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_TacticsSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_TacticsSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_TacticsSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_TacticsSkillScroll.Rechargable )
							m_TacticsSkillScroll.Delete();
					}
					else
