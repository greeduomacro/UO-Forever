...
{
	[Flags]
	public enum SwordsSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class SwordsSkillScroll : Item
	{
		private int m_SkillBonus;
		private SwordsSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public SwordsSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( SwordsSkillScrollFlags.Newbie ); } set{ SetFlag( SwordsSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( SwordsSkillScrollFlags.Expires ); } set{ SetFlag( SwordsSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( SwordsSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( SwordsSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public SwordsSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public SwordsSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public SwordsSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public SwordsSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public SwordsSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Swords Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Swords Skill Scroll";
		}
		...
		{
			from.SendGump( new SwordsSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(SwordsSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (SwordsSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class SwordsSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private SwordsSkillScroll m_SwordsSkillScroll;
		private Mobile m_Target;

		public SwordsSkillScrollGump ( Mobile from, Mobile target, SwordsSkillScroll ball ) : base ( 20, 20 )
		{
			m_SwordsSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_SwordsSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_SwordsSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_SwordsSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_SwordsSkillScroll.Deleted )
				return;

			if ( m_SwordsSkillScroll.Expires && DateTime.UtcNow >= m_SwordsSkillScroll.ExpireDate )
				m_SwordsSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_SwordsSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_SwordsSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_SwordsSkillScroll.SkillBonus;

				List<Skill> decreased = m_SwordsSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_SwordsSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_SwordsSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_SwordsSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_SwordsSkillScroll.Rechargable )
							m_SwordsSkillScroll.Delete();
					}
					else
