...
{
	[Flags]
	public enum AlchemySkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class AlchemySkillScroll : Item
	{
		private int m_SkillBonus;
		private AlchemySkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public AlchemySkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( AlchemySkillScrollFlags.Newbie ); } set{ SetFlag( AlchemySkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( AlchemySkillScrollFlags.Expires ); } set{ SetFlag( AlchemySkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( AlchemySkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( AlchemySkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public AlchemySkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public AlchemySkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public AlchemySkillScroll( int bonus ) : this( bonus, false )
		{
            Weight = 1;
		}

		[Constructable]
		public AlchemySkillScroll() : this( 1 )
		{
		}

		public AlchemySkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Alchemy Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Alchemy Skill Scroll";
		}
		...
		{
			from.SendGump( new AlchemySkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(AlchemySkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (AlchemySkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class AlchemySkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private AlchemySkillScroll m_AlchemySkillScroll;
		private Mobile m_Target;

		public AlchemySkillScrollGump ( Mobile from, Mobile target, AlchemySkillScroll ball ) : base ( 20, 20 )
		{
			m_AlchemySkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_AlchemySkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_AlchemySkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_AlchemySkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_AlchemySkillScroll.Deleted )
				return;

			if ( m_AlchemySkillScroll.Expires && DateTime.UtcNow >= m_AlchemySkillScroll.ExpireDate )
				m_AlchemySkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_AlchemySkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_AlchemySkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_AlchemySkillScroll.SkillBonus;

				List<Skill> decreased = m_AlchemySkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_AlchemySkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_AlchemySkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_AlchemySkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_AlchemySkillScroll.Rechargable )
							m_AlchemySkillScroll.Delete();
					}
					else
