...
{
	[Flags]
	public enum HidingSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class HidingSkillScroll : Item
	{
		private int m_SkillBonus;
		private HidingSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public HidingSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( HidingSkillScrollFlags.Newbie ); } set{ SetFlag( HidingSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( HidingSkillScrollFlags.Expires ); } set{ SetFlag( HidingSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( HidingSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( HidingSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public HidingSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public HidingSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public HidingSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public HidingSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public HidingSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Hiding Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Hiding Skill Scroll";
		}
		...
		{
			from.SendGump( new HidingSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(HidingSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (HidingSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class HidingSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private HidingSkillScroll m_HidingSkillScroll;
		private Mobile m_Target;

		public HidingSkillScrollGump ( Mobile from, Mobile target, HidingSkillScroll ball ) : base ( 20, 20 )
		{
			m_HidingSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_HidingSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_HidingSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_HidingSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_HidingSkillScroll.Deleted )
				return;

			if ( m_HidingSkillScroll.Expires && DateTime.UtcNow >= m_HidingSkillScroll.ExpireDate )
				m_HidingSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_HidingSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_HidingSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_HidingSkillScroll.SkillBonus;

				List<Skill> decreased = m_HidingSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_HidingSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_HidingSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_HidingSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_HidingSkillScroll.Rechargable )
							m_HidingSkillScroll.Delete();
					}
					else
