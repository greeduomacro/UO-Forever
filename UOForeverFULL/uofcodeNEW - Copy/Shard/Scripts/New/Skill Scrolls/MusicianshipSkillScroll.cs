...
{
	[Flags]
	public enum MusicianshipSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class MusicianshipSkillScroll : Item
	{
		private int m_SkillBonus;
		private MusicianshipSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public MusicianshipSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( MusicianshipSkillScrollFlags.Newbie ); } set{ SetFlag( MusicianshipSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( MusicianshipSkillScrollFlags.Expires ); } set{ SetFlag( MusicianshipSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( MusicianshipSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( MusicianshipSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public MusicianshipSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public MusicianshipSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public MusicianshipSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public MusicianshipSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public MusicianshipSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Musicianship Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Musicianship skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Musicianship Skill Scroll";
		}
		...
		{
			from.SendGump( new MusicianshipSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(MusicianshipSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (MusicianshipSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class MusicianshipSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private MusicianshipSkillScroll m_MusicianshipSkillScroll;
		private Mobile m_Target;

		public MusicianshipSkillScrollGump ( Mobile from, Mobile target, MusicianshipSkillScroll ball ) : base ( 20, 20 )
		{
			m_MusicianshipSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_MusicianshipSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_MusicianshipSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_MusicianshipSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_MusicianshipSkillScroll.Deleted )
				return;

			if ( m_MusicianshipSkillScroll.Expires && DateTime.UtcNow >= m_MusicianshipSkillScroll.ExpireDate )
				m_MusicianshipSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_MusicianshipSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_MusicianshipSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_MusicianshipSkillScroll.SkillBonus;

				List<Skill> decreased = m_MusicianshipSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_MusicianshipSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_MusicianshipSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_MusicianshipSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_MusicianshipSkillScroll.Rechargable )
							m_MusicianshipSkillScroll.Delete();
					}
					else
