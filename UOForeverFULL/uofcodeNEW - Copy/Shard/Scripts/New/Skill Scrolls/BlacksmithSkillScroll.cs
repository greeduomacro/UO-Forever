...
{
	[Flags]
	public enum BlacksmithSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class BlacksmithSkillScroll : Item
	{
		private int m_SkillBonus;
		private BlacksmithSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public BlacksmithSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( BlacksmithSkillScrollFlags.Newbie ); } set{ SetFlag( BlacksmithSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( BlacksmithSkillScrollFlags.Expires ); } set{ SetFlag( BlacksmithSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( BlacksmithSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( BlacksmithSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public BlacksmithSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public BlacksmithSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public BlacksmithSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public BlacksmithSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public BlacksmithSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Blacksmith Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Blacksmith skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Blacksmith Skill Scroll";
		}
		...
		{
			from.SendGump( new BlacksmithSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(BlacksmithSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (BlacksmithSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class BlacksmithSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private BlacksmithSkillScroll m_BlacksmithSkillScroll;
		private Mobile m_Target;

		public BlacksmithSkillScrollGump ( Mobile from, Mobile target, BlacksmithSkillScroll ball ) : base ( 20, 20 )
		{
			m_BlacksmithSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_BlacksmithSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_BlacksmithSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_BlacksmithSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_BlacksmithSkillScroll.Deleted )
				return;

			if ( m_BlacksmithSkillScroll.Expires && DateTime.UtcNow >= m_BlacksmithSkillScroll.ExpireDate )
				m_BlacksmithSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_BlacksmithSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_BlacksmithSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_BlacksmithSkillScroll.SkillBonus;

				List<Skill> decreased = m_BlacksmithSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_BlacksmithSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_BlacksmithSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_BlacksmithSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_BlacksmithSkillScroll.Rechargable )
							m_BlacksmithSkillScroll.Delete();
					}
					else
