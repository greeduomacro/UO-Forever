...
{
	[Flags]
	public enum TasteIDSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class TasteIDSkillScroll : Item
	{
		private int m_SkillBonus;
		private TasteIDSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public TasteIDSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( TasteIDSkillScrollFlags.Newbie ); } set{ SetFlag( TasteIDSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( TasteIDSkillScrollFlags.Expires ); } set{ SetFlag( TasteIDSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( TasteIDSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( TasteIDSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public TasteIDSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public TasteIDSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public TasteIDSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public TasteIDSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public TasteIDSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a TasteID Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} TasteID Skill Scroll";
		}
		...
		{
			from.SendGump( new TasteIDSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(TasteIDSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (TasteIDSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class TasteIDSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private TasteIDSkillScroll m_TasteIDSkillScroll;
		private Mobile m_Target;

		public TasteIDSkillScrollGump ( Mobile from, Mobile target, TasteIDSkillScroll ball ) : base ( 20, 20 )
		{
			m_TasteIDSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_TasteIDSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_TasteIDSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_TasteIDSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_TasteIDSkillScroll.Deleted )
				return;

			if ( m_TasteIDSkillScroll.Expires && DateTime.UtcNow >= m_TasteIDSkillScroll.ExpireDate )
				m_TasteIDSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_TasteIDSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_TasteIDSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_TasteIDSkillScroll.SkillBonus;

				List<Skill> decreased = m_TasteIDSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_TasteIDSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_TasteIDSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_TasteIDSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_TasteIDSkillScroll.Rechargable )
							m_TasteIDSkillScroll.Delete();
					}
					else
