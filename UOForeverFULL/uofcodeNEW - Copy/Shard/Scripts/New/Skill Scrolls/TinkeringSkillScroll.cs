...
{
	[Flags]
	public enum TinkeringSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class TinkeringSkillScroll : Item
	{
		private int m_SkillBonus;
		private TinkeringSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public TinkeringSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( TinkeringSkillScrollFlags.Newbie ); } set{ SetFlag( TinkeringSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( TinkeringSkillScrollFlags.Expires ); } set{ SetFlag( TinkeringSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( TinkeringSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( TinkeringSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public TinkeringSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public TinkeringSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public TinkeringSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public TinkeringSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public TinkeringSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Tinkering Skill Scroll"; } }
		...
		public virtual void UpdateName()
		{
			if ( Expires )
				...
				Name = "a +1 Tinkering skill scroll";
		}
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Tinkering Skill Scroll";
		}
		...
		{
			from.SendGump( new TinkeringSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(TinkeringSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (TinkeringSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class TinkeringSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private TinkeringSkillScroll m_TinkeringSkillScroll;
		private Mobile m_Target;

		public TinkeringSkillScrollGump ( Mobile from, Mobile target, TinkeringSkillScroll ball ) : base ( 20, 20 )
		{
			m_TinkeringSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_TinkeringSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_TinkeringSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_TinkeringSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_TinkeringSkillScroll.Deleted )
				return;

			if ( m_TinkeringSkillScroll.Expires && DateTime.UtcNow >= m_TinkeringSkillScroll.ExpireDate )
				m_TinkeringSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_TinkeringSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_TinkeringSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_TinkeringSkillScroll.SkillBonus;

				List<Skill> decreased = m_TinkeringSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_TinkeringSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_TinkeringSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_TinkeringSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_TinkeringSkillScroll.Rechargable )
							m_TinkeringSkillScroll.Delete();
					}
					else
