...
{
	[Flags]
	public enum MiningSkillScrollFlags
	{
		None	= 0x00,
		Newbie	= 0x01,
		Expires	= 0x02
	...
	public class MiningSkillScroll : Item
	{
		private int m_SkillBonus;
		private MiningSkillScrollFlags m_Flags;
		private DateTime m_ExpireDate;

		public MiningSkillScrollFlags Flags{ get{ return m_Flags; } set{ m_Flags = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NewbieBall{ get{ return GetFlag( MiningSkillScrollFlags.Newbie ); } set{ SetFlag( MiningSkillScrollFlags.Newbie, value ); UpdateHue(); } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool Expires{ get{ return GetFlag( MiningSkillScrollFlags.Expires ); } set{ SetFlag( MiningSkillScrollFlags.Expires, value ); InvalidateProperties(); } }

		public bool GetFlag( MiningSkillScrollFlags flag )
		{
			return ( m_Flags & flag ) != 0;
		}

		public void SetFlag( MiningSkillScrollFlags flag, bool value )
		{
			if ( value )
				m_Flags |= flag;
			else
		...
		[Constructable]
		public MiningSkillScroll( int bonus, bool newbied, int days ) : base( 8800 )
		{
			m_SkillBonus = bonus;
			NewbieBall = newbied;
			if ( days > -1 )
		...
		[Constructable]
		public MiningSkillScroll( int bonus, bool newbied ) : this( bonus, false, -1 )
		{
		}

		[Constructable]
		public MiningSkillScroll( int bonus ) : this( bonus, false )
		{
		}

		[Constructable]
		public MiningSkillScroll() : this( 1 )
		{
            Weight = 1;
		}

		public MiningSkillScroll( Serial serial ) : base( serial )
		{
		}

		public override bool Nontransferable{ get { return false; } }

		public override string DefaultName{ get{ return "a Mining Skill Scroll"; } }
		...
		public virtual string GetLabelName()
		{
			...
			return "a +{0} Mining Skill Scroll";
		}
		...
		{
			from.SendGump( new MiningSkillScrollGump( from, from, this ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !from.HasGump( typeof(MiningSkillScroll) ) )
				SendGump( from );
			else
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 1:
				{
					m_Flags = (MiningSkillScrollFlags)reader.ReadEncodedInt();
...
{
	public class MiningSkillScrollGump : Gump
	{
		private const int FieldsPerPage = 1;

		private Skill m_Skill;
		private MiningSkillScroll m_MiningSkillScroll;
		private Mobile m_Target;

		public MiningSkillScrollGump ( Mobile from, Mobile target, MiningSkillScroll ball ) : base ( 20, 20 )
		{
			m_MiningSkillScroll = ball;
			m_Target = target;
			...
			SkillName[] allowedskills = m_MiningSkillScroll.GetAllowedSkills();
			...
			for ( int i = 0; i < allowedskills.Length; ++i )
			{
				...
				if ( (!m_MiningSkillScroll.NewbieBall || skill.Base == 0) && (skill.Base + m_MiningSkillScroll.SkillBonus) <= skill.Cap && skill.Lock == SkillLock.Up )
				{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( from == null || m_MiningSkillScroll.Deleted )
				return;

			if ( m_MiningSkillScroll.Expires && DateTime.UtcNow >= m_MiningSkillScroll.ExpireDate )
				m_MiningSkillScroll.SendLocalizedMessageTo( from, 1042544 ); // This item is out of charges.
			else if ( !m_MiningSkillScroll.IsChildOf( from.Backpack ) )
				from.SendLocalizedMessage( 1042001 ); // That must be in your pack for you to use it.
			else if (info.ButtonID == 1)
			{
				SkillName skillname = (m_MiningSkillScroll.GetAllowedSkills())[info.ButtonID-1];
				...
				int bonus = m_MiningSkillScroll.SkillBonus;

				List<Skill> decreased = m_MiningSkillScroll.GetDecreasableSkills( from, m_Target, count, cap, out decreaseamount );
				...
				else if ( m_MiningSkillScroll.NewbieBall && m_Skill.Base > 0 )
				...
				else
				{
					...
					{
						m_MiningSkillScroll.DecreaseSkills( from, m_Target, decreased, count, cap, decreaseamount );
						m_MiningSkillScroll.IncreaseSkill( from, m_Target, m_Skill );

						if ( !m_MiningSkillScroll.Rechargable )
							m_MiningSkillScroll.Delete();
					}
					else
