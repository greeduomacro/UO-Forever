...
namespace Server.Regions
{
    ...
    {               
        public RegionControl m_Controller;
        ...
        public override bool IsDisabled()
        {
            if (!m_Controller.IsGuarded != Disabled)
                Disabled = !m_Controller.IsGuarded;
        ...
        {
            return m_Controller.AllowCutCorpse;
        }
        ...
        { 
            ...
            if (m_Controller.IsSafeZone)
            {
                if (from.Region == this && target.Region != this)
                {
                    from.SendMessage("You cannot perform benificial acts on your target.");
                    return false;
                }
        ...
        public override bool YoungProtected { get { return m_Controller.YoungProtected; } }
        ...
        public bool NoRegs()
        {
            return m_Controller.CastWithoutReagents;
        }

        public bool IsT2A()
        {
            return m_Controller.IsT2AZone;
        }

        public bool CraftRegion()
        {
            return m_Controller.IsCraftRegion;
        }

        public bool PlayingGame(Mobile From)
        {
            return m_Controller.IsGameRegion;
        }

        public bool MoveThrough()
        {
            return m_Controller.CanMoveThrough;
        }

        public bool AlwaysGrey()
        {
            return m_Controller.AlwaysGrey;
        }

        public bool AllowRecallIn()
        {
            return m_Controller.AllowRecallIn;
        }

        public bool AllowGateIn()
        {
            return m_Controller.AllowGateIn;
        }
        ...
        public override bool OnMoveInto(Mobile m, Direction d, Point3D newLocation, Point3D oldLocation)
        {
            ...
            if (m_Controller.NoMounts || m_Controller.NoPets)
            {
                if (m.Mounted)
                {
                    m.SendMessage("Your mount refuses to go into this area!");
                    return false;
                }
            }

            if (m_Controller.NoPets)
            {
            ...
            if (m.AccessLevel == AccessLevel.Player && m_Controller.IsSafeZone)
            {
                if (m.InCombat(TimeSpan.FromSeconds(60.0)))
                {
                    m.SendMessage("You cannot enter a safe zone after recently having been in combat.");
                    return false;
                }
            }
