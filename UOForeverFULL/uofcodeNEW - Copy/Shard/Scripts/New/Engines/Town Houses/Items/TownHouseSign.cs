...
namespace Knives.TownHouses
{
    ...
    public class TownHouseSign : Item
    {
        ...
        private List<Rectangle2D> c_Blocks;
        ...
        public List<Rectangle2D> Blocks { get { return c_Blocks; } set { c_Blocks = value; } }
        ...
        protected DateTime DemolishTime { get { return c_DemolishTime; } }
        ...
        public void ShowSignPreview()
        {
            ...
            var sign = new Item(0xBD2) {Name = "Sign Preview", Movable = false, Location = SignLoc, Map = Map};
        ...
        public void ShowBanPreview()
        {
            ...
            var ban = new Item(0x17EE) {Name = "Ban Loc Preview", Movable = false, Location = BanLoc, Map = Map};
        ...
        private void HideOtherSigns()
        {
            foreach (
                Item item in
                    c_House.Sign.GetItemsInRange(0).OfType<Item>().Where(item => !(item is HouseSign))
                        .Where(item => item != null && (item.ItemID == 0xB95
                                                        || item.ItemID == 0xB96
                                                        || item.ItemID == 0xC43
                                                        || item.ItemID == 0xC44
        ...
        protected virtual void ConvertItems(bool keep)
        {
                    ...
                    c_Blocks.SelectMany(
                        rect =>
                            Map.GetItemsInBounds(rect)
                                .Where(item =>
                                        c_House.Region.Contains(item.Location) && item.RootParent == null &&
                                        !items.Contains(item))))
            {
                items.Add(item);
            }
            ...
            foreach (
                Item item in
                    items.Where(
                        item =>
                            !(item is HouseSign) && !(item is BaseMulti) && !(item is BaseAddon) &&
                            !(item is AddonComponent) && item != c_House.Hanger && item.Visible && !item.IsLockedDown &&
                            !item.IsSecure && !item.Movable && !c_PreviewItems.Contains(item)))
            {
                if (item is BaseDoor)
                {
        ...
        public virtual void ValidateOwnership()
        {
            ...
            if (!CanBuyHouse(c_House.Owner) && DemolishTimer == null)
            {
                BeginDemolishTimer();
            }
            else
        ...
        public void CheckRentTimer()
        {
            ...
            c_House.Owner.SendMessage("This rent cycle ends in {0} days, {1}:{2}:{3}.", (c_RentTime - DateTime.Now).Days,
                (c_RentTime - DateTime.Now).Hours, (c_RentTime - DateTime.Now).Minutes,
