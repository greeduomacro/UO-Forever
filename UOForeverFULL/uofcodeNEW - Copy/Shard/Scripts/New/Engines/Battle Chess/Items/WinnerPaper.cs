...
namespace Arya.Chess
{
	...
	public class WinnerPaper : Item
	{
		...
		private TimeSpan m_LoserTime;
		private int m_WinnerScore;
		private int m_LoserScore;

		public override string DefaultName{ get{ return String.Format( "{0} won a fair chess game vs {1}", m_Winner, m_Loser ); } }

		public WinnerPaper( Mobile winner, Mobile loser, TimeSpan gameTime, TimeSpan winnerTime, TimeSpan loserTime, int winnerScore, int loserScore) : base( 5357 )
		{
			Hue = 1159;
			Weight = 0.2;
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060661, "{0}'s Game Time\t{1} Hours, {2} Minutes, {3} Seconds", m_Loser, m_LoserTime.Hours, m_LoserTime.Minutes, m_LoserTime.Seconds );
			list.Add( 1060662, "{0}'s Score\t{1}", m_Winner, m_WinnerScore );
			list.Add( 1060663, "{0}'s Score\t{1}", m_Loser, m_LoserScore );
		}
