...
{
	public class HolidayWreathAddon : BaseHolidayGift, IAddon
	{
		[Constructable]
		public HolidayWreathAddon() : this( Utility.RandomDyedHue() )
		{
		}

		[Constructable]
		public HolidayWreathAddon( int hue )
		{
            ItemID = 0x232D;
        ...
        [Constructable]
        public HolidayWreathAddon(int hue, int year)
        {
            ItemID = 0x232D;
		...
		public HolidayWreathAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new HolidayWreathDeed( this.HolidayYear ); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( HolidayWreathAddonGump ) );
					from.SendGump( new HolidayWreathAddonGump( from, this ) );
				}
				else
				{
		...
		private class HolidayWreathAddonGump : Gump
		{
			private Mobile m_From;
			private HolidayWreathAddon m_Addon;

			public HolidayWreathAddonGump( Mobile from, HolidayWreathAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class HolidayWreathDeed : BaseHolidayGift
	{
		//public override int LabelNumber{ get{ return 1070882; } } // HolidayWreath Deed

		[Constructable]
		public HolidayWreathDeed( int year)
		{
            Name = "Holiday Wreath Deed";
		...
		public HolidayWreathDeed( Serial serial ) : base( serial )
		{
		}
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
                if (map.CanFit(loc, 16))
                {
                    ...
                    if (northWall && westWall)
                        from.SendGump(new HolidayWreathDeedGump(from, loc, this));
                    else
                        PlaceAddon(from, loc, northWall, westWall);
                }
                else
                    from.SendMessage("The wreath must be placed next to a wall.");
			}
			else
			{
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( northWall )
				...
				from.SendLocalizedMessage( 1070883 ); // The HolidayWreath must be placed next to a wall.

			if ( itemID > 0 )
			{
				Item addon = new HolidayWreathAddon( this.Hue, this.HolidayYear );
		...
		private class HolidayWreathDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private HolidayWreathDeed m_Deed;

			public HolidayWreathDeedGump( Mobile from, Point3D loc, HolidayWreathDeed    deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
