...
namespace Server.Mobiles
{
	public class BaseHire : BaseCreature
	{
		...
		private static Dictionary<Mobile, BaseHire> m_HireTable = new Dictionary<Mobile, BaseHire>();
		public static Dictionary<Mobile, BaseHire> HireTable{ get{ return m_HireTable; } }
		...
		public virtual bool AddHire( Mobile m )
		{
			...
			else if ( CanHire( m ) && SetControlMaster( m ) )
			{
				m_IsHired = true;
				return true;
			}
		...
		public static bool CanHire( Mobile m )
		{
			if ( m is PlayerMobile )
				foreach ( Mobile f in ((PlayerMobile)m).AllFollowers )
					if ( f is BaseHire )
						return false;
