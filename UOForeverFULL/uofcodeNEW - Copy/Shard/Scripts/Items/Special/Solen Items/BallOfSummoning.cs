...
namespace Server.Items
{
	public class BallOfSummoning : Item, ITranslocationItem
	{
		...
		public string TranslocationItemName { get { return "Pet summoning"; } }
			...
			: base(0xE2E)
		{
			...
			Name = "Pet Summoning Ball";

			m_Charges = Utility.RandomMinMax(9, 10);
		...
		{
			list.Add(1054131, m_Charges.ToString() + (m_PetName.Length == 0 ? "\t " : "\t" + m_PetName));
		...
		public override void OnSingleClick(Mobile from)
		{
			...
			LabelTo(from, 1054131, m_Charges.ToString() + (m_PetName.Length == 0 ? "\t " : "\t" + m_PetName));
				// a crystal ball of pet summoning: [charges: ~1_charges~] : [linked pet: ~2_petName~]
		}
		...
		private class PetLinkTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				else if (targeted is BaseCreature)
				{
					...
					{
						MessageHelper.SendLocalizedMessageTo(m_Ball, from, 1054117, 0x59);
							// You may only link your own pets to a Crystal Ball of Pet Summoning.
					}
					else if (!creature.IsBonded)
					{
						MessageHelper.SendLocalizedMessageTo(m_Ball, from, 1054118, 0x59);
							// You must bond with your pet before it can be linked to a Crystal Ball of Pet Summoning.
					}
					else
					{
						MessageHelper.SendLocalizedMessageTo(m_Ball, from, 1054119, 0x59);
				...
				{
					MessageHelper.SendLocalizedMessageTo(m_Ball, from, 1054115, 0x59);
						// The Crystal Ball of Pet Summoning cannot summon itself.
				}
				else
				{
					MessageHelper.SendLocalizedMessageTo(m_Ball, from, 1054116, 0x59);
		...
		public void CastSummonPet(Mobile from)
		{
			...
			{
				MessageHelper.SendLocalizedMessageTo(this, from, 1054124, 0x36);
			...
			{
				MessageHelper.SendLocalizedMessageTo(this, from, 1054125, 0x5);
			...
			{
				MessageHelper.SendLocalizedMessageTo(this, from, 1054126, 0x8FD);
			...
			{
				MessageHelper.SendLocalizedMessageTo(this, from, 1054127, 0x22);
					// The Crystal Ball fills with a red mist. You appear to have let your bond to your pet deteriorate.
					 ...
					 from.Region.IsPartOf(typeof(SafeZone)))
			{
				from.Send(
					new AsciiMessage(Serial, ItemID, MessageType.Regular, 0x22, 3, "", "You cannot summon your pet to this location."));
			}
			else if (EraML && from is PlayerMobile && DateTime.UtcNow < ((PlayerMobile)from).LastPetBallTime.AddSeconds(15.0))
			{
				MessageHelper.SendLocalizedMessageTo(this, from, 1080072, 0x22);
		...
		public void SummonPet(Mobile from)
		{
			...
			MessageHelper.SendLocalizedMessageTo(this, from, 1054128, 0x43);
		...
		private class PetSummoningSpell : Spell
		{
			...
			{
				var customRegion = m_Caster.Region as CustomRegion;
				if (customRegion != null && customRegion.Controller != null && customRegion.Controller.NoPets)
				{
					m_Caster.SendMessage("You cannot summon your pet here as no pets are allowed in this area!");
				}
