...
namespace Server.Items
{
	public class FlameSpurtTrap : BaseTrap
	{
		...
		{
			if ( m == null || m.IsDeadBondedPet || !m.Alive || m.AccessLevel > AccessLevel.Player )
				return true;

			if ( ( m.Player || ( m is BaseCreature && ((BaseCreature)m).ControlMaster != null && ((BaseCreature)m).ControlMaster.Player ) ) )
			{
				CheckTimer();
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			...
			if ( m == null || m.Location == oldLocation || m.IsDeadBondedPet || !m.Alive || m.AccessLevel > AccessLevel.Player )
				return;
