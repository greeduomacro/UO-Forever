...
namespace Server.Items
{
	public class FireworksWand : MagicWand
	{
		...
		public void BeginLaunch( Mobile from, bool useCharges )
		{
			...
			IEntity ent = new Entity( Serial.Zero, endLoc, map );

			Effects.SendMovingEffect( new Entity( Serial.Zero, startLoc, map ), ent,
				0x36E4, 5, 0, false, false );

			Timer.DelayCall<IEntity>( TimeSpan.FromSeconds( 1.0 ), new TimerStateCallback<IEntity>( FinishLaunch ), ent );
		}

		private void FinishLaunch( IEntity ent )
		{
			...
			Effects.PlaySound( ent.Location, ent.Map, Utility.Random( 0x11B, 4 ) );
			Effects.SendLocationEffect( ent.Location, ent.Map, 0x373A + (0x10 * Utility.Random( 4 )), 16, 10, hue, renderMode );
		}
