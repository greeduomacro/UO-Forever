...
namespace Server.Items
{
	...
	public abstract class BaseAxe : BaseMeleeWeapon
	{
		...
		public override void OnHit( Mobile attacker, Mobile defender, double damageBonus )
		{
			...
			if ((attacker.Player || attacker.Body.IsHuman) && !attacker.IsT2A 
                && Layer == Layer.TwoHanded 
                && (attacker.Skills[SkillName.Anatomy].Value / 100.0 * SpecialMovesController._ConcussionAxeMaxChance) >= Utility.RandomDouble()
                && Engines.ConPVP.DuelContext.AllowSpecialAbility( attacker, "Concussion Blow", false ) )
			{
				...
				if ( mod == null )
				{
                    defender.SendMessage("You have received a concussion!");
