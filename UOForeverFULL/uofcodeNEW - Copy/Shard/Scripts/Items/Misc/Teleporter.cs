...
namespace Server.Items
{
	...
	{
		private bool m_Active, m_Creatures, m_CombatCheck, m_CriminalCheck, m_AllowPets;
		...
		{
			if ( !m_Creatures && !m.Player && m.NetState == null)
			{}
		...
		{
			if ( Delay != TimeSpan.Zero && ( m.Location != Location || m.Map != Map ) )
				return;
	...
	public class SkillTeleporter : Teleporter
	{
		...
		private void EndMessageLock( Mobile m )
		{
			m.EndAction( this );
		}
			...
			if ( base.CanTeleport( m ) )
			{
				...
				if ( sk == null || sk.Base < m_Required )
				{
					if ( m.BeginAction( this ) )
					{
						...
						Timer.DelayCall<Mobile>( TimeSpan.FromSeconds( 5.0 ), new TimerStateCallback<Mobile>( EndMessageLock ), m );
					}
	...
	public class KeywordTeleporter : Teleporter
	{
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			if ( !e.Handled && Active )
			{
				...
				if ( m.InRange( GetWorldLocation(), m_Range ) && CanTeleport( m ) )
				{
					bool isMatch = false;
    ...
    public class WaitDelayTeleporter : Teleporter
    {
        private TimeSpan m_TimeoutDelay;
        public static Dictionary<Mobile, DateTime> m_CantTeleport;
        ...
        [Constructable]
        public WaitDelayTeleporter()
            : this(new Point3D(0, 0, 0), null, false)
        {
        }

        [Constructable]
        public WaitDelayTeleporter(Point3D pointDest, Map mapDest)
            : this(pointDest, mapDest, false)
        {
        }

        [Constructable]
        public WaitDelayTeleporter(Point3D pointDest, Map mapDest, bool creatures)
            : base(pointDest, mapDest, creatures)
        {
        ...
        public override bool CanTeleport(Mobile m)
        {
            if (base.CanTeleport(m))
            {
                if (m_CantTeleport.ContainsKey(m) && DateTime.UtcNow >=  (m_CantTeleport[m] + TimeoutDelay))
                    m_CantTeleport.Remove(m);
                if (m_CantTeleport.ContainsKey(m))
                {
                    m.SendMessage(44, "You cannot use a teleporter again so soon.");
                    return false;
                }
            }
            if (!m_CantTeleport.ContainsKey(m))
                m_CantTeleport.Add(m, DateTime.UtcNow);
            return true;
        }

        public WaitDelayTeleporter(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(m_TimeoutDelay);
            writer.Write(m_CantTeleport.Count);

            foreach (KeyValuePair<Mobile, DateTime> kvp in m_CantTeleport)
            {
                writer.Write(kvp.Key);
	...
	public class ConditionTeleporter : Teleporter
	{
		...
		public override bool CanTeleport( Mobile m )
		{
			if ( base.CanTeleport( m ) )
			{
				...
				if ( GetFlag( ConditionFlag.StaffOnly ) && m.AccessLevel < AccessLevel.Counselor )
					m.SendMessage( "You may not proceed." );
				else if ( GetFlag( ConditionFlag.DenyMounted ) && m.Mounted )
					m.SendLocalizedMessage( 1077252 ); // You must dismount before proceeding.
				else if ( GetFlag( ConditionFlag.DenyFollowers ) && ( m.Followers != 0 || ( m is PlayerMobile && ( (PlayerMobile) m ).AutoStabled.Count != 0 ) ) )
					m.SendLocalizedMessage( 1077250 ); // No pets permitted beyond this point.
				else if ( GetFlag( ConditionFlag.DenyPackContents ) && ( pack = m.Backpack ) != null && pack.TotalItems != 0 )
					m.SendMessage( "You must empty your backpack before proceeding." );
				else if ( GetFlag( ConditionFlag.DenyHolding ) && m.Holding != null )
					m.SendMessage( "You must let go of what you are holding before proceeding." );
				else if ( GetFlag( ConditionFlag.DenyEquipment ) && HasItems( m ) )
				{
				}
				else if ( GetFlag( ConditionFlag.DenyTransformed ) && m.IsBodyMod )
		...
		private static bool HasItems( Mobile m )
		{
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			m_Flags = (ConditionFlag) reader.ReadEncodedInt();
		}
