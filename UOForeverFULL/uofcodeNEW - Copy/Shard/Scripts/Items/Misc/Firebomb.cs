...
namespace Server.Items
{
	public class Firebomb : Item
	{
		...
		/*public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( Core.AOS && (from.Paralyzed || from.Frozen || (from.Spell != null && from.Spell.IsCasting)) )
			...
			switch ( m_Ticks )
			{
				...
				default:
				{
					...
					if ( RootParent is Mobile )
					{
						...
						parent.Damage( Utility.Random( 3 ));
					}
					else if ( RootParent == null )
					{
		...
		private void OnFirebombTarget( Mobile from, object obj )
		{
			...
			Timer.DelayCall<IEntity>( TimeSpan.FromSeconds( 1.0 ), new TimerStateCallback<IEntity>( FirebombReposition_OnTick ), to );
			Internalize();
		}
