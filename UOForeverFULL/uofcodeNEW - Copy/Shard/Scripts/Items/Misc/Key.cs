...
namespace Server.Items
{
	...
	public class Key : Item, ISecurable
	{
		...
		{
			if ( !this.IsChildOf( from.Backpack ) && this.Movable) // if not movable, checkAccess will already check security level
			{
		...
		{
			if ( o.KeyValue != 0 && o.KeyValue == this.KeyValue )
			{
		...
		private class UnlockTarget : Target
		{
			...
			{
                // if it's NOT movable, allow use (access check for friends, etc. has already been performed by this point)
                if (m_Key.Movable) 
                {
                    if (m_Key.Deleted || !m_Key.IsChildOf(from.Backpack))
                    {
