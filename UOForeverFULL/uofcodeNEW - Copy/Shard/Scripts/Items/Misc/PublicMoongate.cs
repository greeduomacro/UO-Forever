...
namespace Server.Items
{
	public class PublicMoongate : Item
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				if (!((BaseCreature)from).Pseu_CanUseGates)
				{
					return;
				}
			}
		...
		public override bool OnMoveOver(Mobile m)
		{
			...
			if (m.NetState != null)
			{
				...
				{
					if (!((BaseCreature)m).Pseu_CanUseGates)
					{
						return true;
					}
				}
		...
		public bool UseGate(Mobile m)
		{
			...
			else if (!AllowCombat && (SpellHelper.CheckCombat(m) || CheckCombat(m)))
			{
		...
		{
			CommandSystem.Register("MoonGen", AccessLevel.Owner, MoonGen_OnCommand);
		}
	...
	public class MoongateGump : Gump
	{
		...
		private readonly PublicMoongate m_Moongate;
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			else if (Sigil.ExistsOn(m_Mobile) && !Faction.IsFactionFacet(list.Map))
			{
