	...
	public class BaseShield : BaseArmor
	{
		...
		public override int OnHit( BaseWeapon weapon, Mobile attacker, int damage )
		{
			...
			{ */
				...
				double chance = owner.Skills[SkillName.Parry].Value / 200.0;
				...
				*/
				...
				if ( owner.CheckSkill( SkillName.Parry, chance ) )
				{
                    ...
                    if (0.50 > Utility.RandomDouble() && LootType != LootType.Blessed || weapon is BaseBashing) // 50% chance to lower durability
					{
							int wear = Utility.Random( (origdamage-damage) / 5 );
