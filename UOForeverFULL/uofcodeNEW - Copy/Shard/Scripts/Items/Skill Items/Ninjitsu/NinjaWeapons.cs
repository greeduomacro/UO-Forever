	...
	public class NinjaWeapon
	{
		...
		private static void OnHit(object[] states)
		{
			...
			if (from.CanBeHarmful(target))
			{
                ...
                target.Damage(weapon.WeaponDamage, from);
		...
		public class LoadEntry : ContextMenuEntry
		{
			...
			public LoadEntry( INinjaWeapon wep, int entry ) : base( entry, 0 )
			{
				weapon = wep;
			}
		...
		public class UnloadEntry : ContextMenuEntry
		{
			...
			public UnloadEntry( INinjaWeapon wep, int entry ) : base( entry, 0 )
			{
				weapon = wep;
