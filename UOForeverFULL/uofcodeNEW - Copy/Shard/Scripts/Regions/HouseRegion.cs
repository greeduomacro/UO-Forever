...
namespace Server.Regions
{
	public class HouseRegion : BaseRegion
	{
		...
		public override void OnLocationChanged(Mobile m, Point3D oldLocation)
		{
            ...
            if (m_House.IsCombatRestricted(m) && (m_House.IsFriend(m) || m_House.IsCoOwner(m) || m_House.IsOwner(m)))
            {
                m.Location = m_House.BanLocation;
                m.SendLocalizedMessage(501284); // You may not enter.		        
            }
			...
			if (m_House.InternalizedVendors.Count > 0 && m_House.IsInside(m) && !m_House.IsInside(oldLocation, 16) &&
				m_House.IsOwner(m) && m.Alive && !m.HasGump(typeof(NoticeGump)))
			{
		...
		public override bool OnMoveInto(Mobile from, Direction d, Point3D newLocation, Point3D oldLocation)
		{
			...
			if (m_House.InternalizedVendors.Count > 0 && m_House.IsInside(from) && !m_House.IsInside(oldLocation, 16) &&
				m_House.IsOwner(from) && from.Alive && !from.HasGump(typeof(NoticeGump)))
			{
				/* This house has been customized recently, and vendors that work out of this
				 * house have been temporarily relocated.  You must now put your vendors back to work.
				 * To do this, walk to a location inside the house where you wish to station
            ...
            from.Send(StopMusic.Instance);
            from.Send(new PlayMusic(Music));
			return true;
		}
		...
		public override TimeSpan GetLogoutDelay(Mobile m)
		{
			if (m_House.IsFriend(m) && m_House.IsInside(m))
			{
				...
				if (m.Aggressed.Any(info => info.Defender.Player && now - info.LastCombatTime < CombatHeatDelay))
				{
