...
namespace VitaNex.SuperGumps
{
	public abstract partial class SuperGump
	{
			...
			where TGump : SuperGump
		{
			...
			foreach (TGump gump in Instances[user].AsParallel().Where(g => g != null && g.TypeEquals<TGump>(inherited)).Cast<TGump>())
			{
				yield return gump;
			}
		}
		...
		public static IEnumerable<SuperGump> EnumerateInstances(PlayerMobile user, Type type, bool inherited = false)
		{
			...
			foreach (SuperGump gump in Instances[user].AsParallel().Where(g => g != null && g.TypeEquals(type, inherited)))
			{
				yield return gump;
			}
		}
