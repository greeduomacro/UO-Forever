...
namespace Server.Misc
{
	public static partial class AntiAdverts
	{
		...
		private static void OnSpeech(SpeechEventArgs e)
		{
			...
			Reports.Add(
				new AntiAdvertsReport(
					...
					GetDetectedString(e.Mobile, e.Speech, jailed, banned),
					e.Speech,
					jailed,
					banned));
		}

		private static string GetDetectedString(Mobile m, string speech, bool jailed, bool banned)
		{
		...
		private static void ToConsole(Mobile m, string speech, bool jailed, bool banned)
		{
			...
			{
				Console.WriteLine("[Warning: Advertising Detected]: " + GetDetectedString(m, speech, jailed, banned));
			}
		}
		...
		private static void SendPage(Mobile m, string speech, bool jailed, bool banned)
		{
			...
			if (entry == null || !entry.Message.StartsWith("[Warning: Advertising Detected]"))
			{
				PageQueue.Enqueue(
					new PageEntry(
						m, "[Warning: Advertising Detected]: " + GetDetectedString(m, speech, jailed, banned), PageType.Account));
			}
		}
