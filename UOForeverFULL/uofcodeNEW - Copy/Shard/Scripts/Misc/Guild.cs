...
namespace Server.Guilds
{
	...
	public class AllianceInfo
	{
        ...
        {
            CommandSystem.Register("AllianceDisband", AccessLevel.GameMaster, new CommandEventHandler(AllianceDisband_Command));
        }

        public static void AllianceDisband_Command(CommandEventArgs e)
        {
            e.Mobile.Target = new AllianceDisbandTarget();
        }

        private class AllianceDisbandTarget : Target
        {
            public AllianceDisbandTarget()
            ...
            protected override void OnTarget(Mobile gm, object target)
            {
                ...
                if (pm == null)
                {
                    gm.SendMessage("That is not a player.");
                    return;
                }

                if (pm.Guild != null && pm.Guild is Guild)
                {
                    Guild guild = pm.Guild as Guild;
                    if (guild.Alliance != null)
                    {
                        guild.Alliance.Disband();
                        gm.SendMessage("Their alliance (or pending alliance) has been disbanded.");
                    }
                    try
                    {
                        guild.ClearAllianceLeaderRaw();
                        gm.SendMessage("Any bugged out Alliance Leader situations have been fixed for their guild.");
                    }
                    catch (Exception except) { gm.SendMessage("Error: " + except.Message); }
                }
            }
        }
	...
	public class Guild : BaseGuild
	{
		public static void Configure()
		{
			...
			CommandSystem.Register( "GuildProps", AccessLevel.GameMaster, new CommandEventHandler( GuildProps_OnCommand ) );
		...
		private static void GuildProps_OnCommand( CommandEventArgs e )
		{
			...
			else
			{
                ...
                {
                    from.SendMessage("That guild was not found!");
                }
			}
        ...
        private DateTime m_virtuecheck;
        ...
        public void ClearAllianceLeaderRaw() // some stupid bug where it didn't clear, so a command allows this
        {
            m_AllianceLeader = null;
        }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public DateTime virtuecheck
        {
            get
            {
                return m_virtuecheck;
            }
            set
            {
                m_virtuecheck = value;
