...
namespace Server.Misc
{
	public class RegenRates
	{
		...
		public static void Configure()
		{
			...
			Mobile.HitsRegenRateHandler = Mobile_CustomHitsRegenRate;
		...
		private static TimeSpan Mobile_CustomHitsRegenRate(Mobile from)
		{
		...
		private static TimeSpan Mobile_ManaRegenRate(Mobile from)
		{
			...
			if (medPoints <= 0)
			{
				rate = Mobile.DefaultManaRate.TotalSeconds;
			}
			else if (medPoints <= 100)
			{
				rate = Mobile.DefaultManaRate.TotalSeconds - (242 * medPoints / 2400) + (19 * medPoints * medPoints / 48000);
			}
			else if (medPoints < 120)
			{
				rate = Mobile.DefaultManaRate.TotalSeconds / 7.0;
			}
			else
			{
				rate = Mobile.DefaultManaRate.TotalSeconds / 10.0;
			}
