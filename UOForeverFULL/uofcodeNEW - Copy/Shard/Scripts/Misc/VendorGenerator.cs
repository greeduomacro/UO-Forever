...
namespace Server
{
	public class VendorGenerator
	{
		...
		private static bool GetFloorZ( Map map, int x, int y, out int z )
		{
			...
			if ( IsFloor( lt.ID, Region.Find( new Point3D( x, y, lt.Z ), map ) ) && map.CanFit( x, y, lt.Z, 16, false, false ) )
			{
				z = lt.Z;
				return true;
			}
		...
		private static bool IsFloor( Map map, int x, int y, bool canFit )
		{
			...
			if ( IsFloor( lt.ID, Region.Find( new Point3D( x, y, lt.Z ), map ) ) && (canFit||CanFit( map, x, y, lt.Z )) )
				return true;
		...
		{
			return IsFloor( itemID, null );
		}

		private static bool IsFloor( int itemID, Region region )
		{
			itemID &= TileData.MaxLandValue;

			if ( region != null && region.IsPartOf( "Nujel'm" ) )
				return ( itemID >= 0x406 && itemID <= 0x415 ) || ( itemID >= 0x4B3 && itemID <= 0x4E1 );
		...
		private static void Process( Map map, Rectangle2D[] regions )
		{
			...
			Console.WriteLine( "Section 1 Completed: {0} Shops", m_ShopList.Count );
			...
			for ( int i = 0; i < m_ShopList.Count; ++i )
			{
				...
				Console.WriteLine( "Section 2A Completed" );
				...
				for ( int j = 0; j < names.Count; ++j )
				{
					...
					Console.WriteLine( "Section 2B Completed" );
					...
					new Spawner( 1, TimeSpan.FromMinutes( 1 ), TimeSpan.FromMinutes( 1 ), 0, 4, names[j] ).MoveToWorld( new Point3D( cp.X, cp.Y, z ), map );
				}
				Console.WriteLine( "Section 2C Completed" );
			}
		...
		private static void ProcessDisplayCase( Map map, StaticTile[] tiles, int x, int y )
		{
			...
			if ( flags != ShopFlags.None )
			{
				...
				ShopInfo si;
				m_ShopTable.TryGetValue( p, out si );

				if ( si == null )
				{
					List<Point2D> floor = new List<Point2D>();
					//Console.WriteLine( "1-PDC->{0}-{1}", x, y );
					RecurseFindFloor( map, x, y, floor, 1 );
					//Console.WriteLine( "1-PDC-C" );
		...
		private static void RecurseFindFloor( Map map, int x, int y, List<Point2D> floor, int count )
		{
			//Console.WriteLine( "1-RFF-{2}->{0}-{1} {3}", x, y, count, floor.Count );
			Point2D p = new Point2D( x, y );

			if ( floor.Contains( p ) )
			{
				//Console.WriteLine( "1-RFF-C-{0}", count );
				return;
			}
			...
			for ( int xo = -1; xo <= 1; ++xo )
			{
				for ( int yo = -1; yo <= 1; ++yo )
					if ( (xo != 0 || yo != 0) && IsFloor( map, x + xo, y + yo, false ) )
						RecurseFindFloor( map, x + xo, y + yo, floor, count+1 );
			}
