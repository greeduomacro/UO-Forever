...
namespace Server
{
	...
	{
		public const string AccountTagName = "hwinfoInstance";
...
/*			info.m_Unknown =*/ pvSrc.ReadStringSafe( 64 );
            ...
            if (acct != null)
            {
                ...
                if (!HasHWInfoInstance(acct, info.m_InstanceID))
                {
                    string currentTag = acct.GetTag(HardwareInfo.AccountTagName);
                    if (currentTag == null)
                    {
                        acct.SetTag(HardwareInfo.AccountTagName, info.m_InstanceID + "");
                    }
                    else
                    {
                        acct.SetTag(HardwareInfo.AccountTagName, currentTag + "|" + info.m_InstanceID);
                    }
                }
            }
		}
