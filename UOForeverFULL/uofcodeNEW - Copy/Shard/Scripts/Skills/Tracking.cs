...
namespace Server.SkillHandlers
{
	...
	public class TrackWhatGump : Gump
	{
		...
		{
			m_From = from;
			m_Success = from.CheckSkill(SkillName.Tracking, 0.0, SpecialMovesController._TrackingSuccessSkillTest);
	...
	public class TrackWhoGump : Gump
	{
		...
		private static readonly TrackTypeDelegate[] m_Delegates = new[]
		{IsAnimal, IsMonster, IsHumanNPC, new TrackTypeDelegate(IsPlayer)};
		...
		private static bool CheckDifficulty(Mobile from, Mobile m)
		{
			...
			if (from.EraML && m.Race == Race.Elf)
			{
				tracking /= 2;
				//The 'Guide' says that it requires twice as Much tracking SKILL to track an elf.  Not the total difficulty to track.
			}

			int hiding = m.Skills[SkillName.Hiding].Fixed / 2;
			int stealth = m.Skills[SkillName.Stealth].Fixed / 2;
			int divisor = 500 + hiding + stealth;

			double chance = (tracking + detectHidden + 50.0) / divisor;
