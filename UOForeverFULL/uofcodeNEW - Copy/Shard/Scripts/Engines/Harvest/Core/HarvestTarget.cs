	...
	public class HarvestTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object targeted )
		{
            ...
            if (customRegion != null && customRegion.Controller != null)
            {
                ...
                if (m_System is Mining) skill = SkillName.Mining;
                else if (m_System is Fishing) skill = SkillName.Fishing;
                else if (m_System is Lumberjacking) skill = SkillName.Lumberjacking;
            ...
            if (m_System is Lumberjacking && targeted is IChopable)
            {
                if (targeted is Item)
                {
                    if (((Item)targeted).Breakable) ((IChopable)targeted).OnChop(from);
                    else from.SendMessage("That is unbreakable!");
                }
                else
                {
            ...
            { 
                if (((Item)targeted).Breakable) DestroyFurniture(from, (Item)targeted);
                else from.SendMessage("That is unbreakable!");
            }
