...
namespace Server.Poker
{
	public class HandRanker
	{
		...
		private static void AddHighestCards(
			int numberOfCardsToAdd, IEnumerable<Card> sortedSourceCards, List<Card> excludeCards, ref List<Card> targetCards)
		{
			int cardsAdded = 0;

			foreach (Card card in sortedSourceCards.Where(card => !excludeCards.Contains(card)))
			{
				++cardsAdded;
		...
		public static bool HasFullHouse(List<Card> sortedCards, out List<Card> fullHouseCards)
		{
			...
			fullHouseCards = threeCards.ToList();
		...
		public static bool HasStraight(List<Card> sortedCards, out List<Card> straightCards)
		{
			...
			if (sequenceCardCount == 4 && straightCards[straightCards.Count - 1].Rank == Rank.Two &&
