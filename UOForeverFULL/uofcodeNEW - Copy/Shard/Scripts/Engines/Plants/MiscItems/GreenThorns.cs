	...
	public class GreenThorns : Item
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				{
					from.LocalOverheadMessage( MessageType.Regular, 0x2B2, true, "No solen lairs exist on this facet.  Try again in Felucca." );
					return;
				}
		...
		protected override TimeSpan Play( int step )
		{
			switch ( step )
			{
				...
				case 3:
				{
					...
					dummy.PublicOverheadMessage( MessageType.Regular, 0x3B2, 1114432 ); // * The ground erupts with chaotic growth! *
	...
	public class FurrowsGreenThornsEffect : GreenThornsEffect
	{
		...
		protected override TimeSpan Play( int step )
		{
			switch ( step )
			{
				...
				{
					EffectItem dummy = EffectItem.Create( Location, Map, TimeSpan.FromSeconds( 20.0 ) );
					dummy.PublicOverheadMessage( MessageType.Regular, 0x3B2, 1114428 ); // * A magical bunny leaps out of its hole, disturbed by the thorn's effect! *
	...
	public class SwampGreenThornsEffect : GreenThornsEffect
	{
		...
		protected override TimeSpan Play( int step )
		{
			switch ( step )
			{
				...
				{
					EffectItem dummy = EffectItem.Create( Location, Map, TimeSpan.FromSeconds( 20.0 ) );
					dummy.PublicOverheadMessage( MessageType.Regular, 0x3B2, 1114429 ); // * Strange green tendrils rise from the ground, whipping wildly! *
	...
	public class SnowGreenThornsEffect : GreenThornsEffect
	{
		...
		protected override TimeSpan Play( int step )
		{
			switch ( step )
			{
				...
				{
					EffectItem dummy = EffectItem.Create( Location, Map, TimeSpan.FromSeconds( 20.0 ) );
					dummy.PublicOverheadMessage( MessageType.Regular, 0x3B2, 1114430 ); // * Slithering ice serpents rise to the surface to investigate the disturbance! *
	...
	/*public class SandGreenThornsEffect : GreenThornsEffect
	{
		...
		protected override TimeSpan Play( int step )
		{
			switch ( step )
			{
				...
				{
					EffectItem dummy = EffectItem.Create( Location, Map, TimeSpan.FromSeconds( 20.0 ) );
					dummy.PublicOverheadMessage( MessageType.Regular, 0x3B2, 1114431 ); // * The sand collapses, revealing a dark hole. *
