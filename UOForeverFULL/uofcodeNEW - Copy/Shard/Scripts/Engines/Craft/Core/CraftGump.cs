...
namespace Server.Engines.Craft
{
	public class CraftGump : Gump
	{
		...
		public void CreateResList(bool opt, Mobile from)
		{
			...
			for (int i = 0; i < res.Count; ++i)
			{
				...
				if (index == 0)
				{
					...
					AddHtmlLocalized(
						255, 263, 200, 18, (context == null || !context.DoNotColor) ? 1061591 : 1061590, LabelColor, false, false);
				}
				...
				else
				{
					AddLabel(
						...
						60 + (index * 20),
						LabelHue,
						String.Format("{0} ({1})", subResource.NameString, resourceCount.ToString(CultureInfo.InvariantCulture)));
				}
			}
		}

		private CraftItem[] _ItemCache;
		...
		public void CreateMakeLastList()
		{
			...
			_ItemCache = context.Items.Where(ci => ci != null && m_From.Expansion >= ci.RequiredExpansion).ToArray();
			...
			if (_ItemCache.Length > 0)
			{
				for (int i = 0; i < _ItemCache.Length; ++i)
				{
					...
					CraftItem craftItem = _ItemCache[i];
		...
		public void CreateItemList(int selectedGroup)
		{
			...
			_ItemCache = craftGroup.CraftItems.Where(ci => ci != null && m_From.Expansion >= ci.RequiredExpansion).ToArray();
			...
			for (int i = 0; i < _ItemCache.Length; ++i)
			{
				...
				if (craftItem.Recipe != null && m_From is PlayerMobile && !((PlayerMobile)m_From).HasRecipe(craftItem.Recipe))
				{
					continue;
				}
				...
				case 1: // Create item
					{
						...
						*/
						if (_ItemCache.InBounds(index))
						{
							CraftItem(_ItemCache[index]);
						}
				...
				case 2: // Item details
					{
						...
						*/
						if (_ItemCache.InBounds(index))
						{
							m_From.SendGump(new CraftGumpItem(m_From, system, _ItemCache[index], m_Tool));
						}
