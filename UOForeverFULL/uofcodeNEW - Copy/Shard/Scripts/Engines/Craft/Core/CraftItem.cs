...
namespace Server.Engines.Craft
{
	...
	{
		int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, IBaseTool tool, CraftItem craftItem, int resHue );
	}
	...
	public class CraftItem
	{
		...
		{
			if ( DefTailoring.IsColorable( m_Type ) )
			{
		...
		public bool Find<TItem>(Mobile from, int[] itemIDs, out TItem item) where TItem : Item
		{
			item = null;
			...
			item =
				from.GetEntitiesInRange<TItem>(map, 2)
					.FirstOrDefault(i => i.Z + 16 > from.Z && from.Z + 16 > i.Z && Find(i.ItemID, itemIDs));
			...
			for (int x = -2; x <= 2; ++x)
			{
				for (int y = -2; y <= 2; ++y)
				{
					...
					if (tiles.Any(t => t.Z + 16 > from.Z && from.Z + 16 > t.Z && Find(t.ID, itemIDs)))
					{
						return true;
					}
				}
		...
		public bool ConsumeRes( Mobile from, Type typeRes, CraftSystem craftSystem, ref int resHue, ref int maxAmount, ConsumeType consumeType, ref object message, bool isFailure )
		{
			...
			if (m_NeedHeat && !Find(from, m_HeatSources, out source))
			{
				message = 1044487; // You must be near a fire source to cook.
				return false;
			}

			if (m_NeedOven && !Find(from, m_Ovens, out source))
			{
			...
			if (m_NeedMill)
			{
				if (ItemType.IsEqual<SackBarley>())
				{
					BaseMillAddon<BarleySheaf, SackBarley> mill;

					if (!Find(from, m_Mills, out mill))
					{
						message = "You must be near a barley mill to do that.";
						return false;
					}
				}
				else if (ItemType.IsEqual<SackSugar>())
				{
					BaseMillAddon<SugarcaneSheaf, SackSugar> mill;

					if (!Find(from, m_Mills, out mill))
					{
						message = "You must be near a sugar mill to do that.";
						return false;
					}
				}
				else if (ItemType.IsEqual<SackFlour>())
				{
					BaseMillAddon<WheatSheaf, SackFlour> mill;
				...
				else if (!Find(from, m_Mills, out source))
				{
					message = "You must be near a mill to do that.";
					return false;
				}
			}
			...
			if ( consumeType == ConsumeType.All )
			{
             ...
             CustomRegion region1 = from.Region as CustomRegion;
                ...
                if (region1 != null && region1.CraftRegion())
                    for (int i = 0; i < amounts.Length; ++i)
                    {
                        amounts[i] = (int)Math.Round(amounts[i] - (amounts[i] * .2)); //consume 20% less resources
                    }
		...
		public double GetSuccessChance( Mobile from, Type typeRes, CraftSystem craftSystem, bool gainSkills, ref bool allRequiredSkills )
		{
			...
			for ( int i = 0; i < m_arCraftSkill.Count; i++)
			{
                ...
                Skill tskill = from.Skills[craftSkill.SkillToMake];
		...
		public void Craft( Mobile from, CraftSystem craftSystem, Type typeRes, IBaseTool tool )
		{
			if ( from.BeginAction( typeof( CraftSystem ) ) )
			{
				if (RequiredExpansion == Expansion.None || (from.NetState != null && from.NetState.SupportsExpansion(RequiredExpansion) && from.Expansion >= RequiredExpansion))
				{
		...
		public void CompleteCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, IBaseTool tool, CustomCraft customCraft )
		{
			int badCraft = craftSystem.CanCraft( from, tool, m_Type );
			...
			if ( CheckSkills( from, typeRes, craftSystem, ref ignored, ref allRequiredSkills ) )
			{
				...
				if ( item != null )
				{
                    ...
                    if (XmlScript.HasTrigger(from, TriggerName.onCraft))
                        UberScriptTriggers.Trigger(from, from, TriggerName.onCraft, item, null, null, 0, null, craftSystem.MainSkill, from.Skills[craftSystem.MainSkill].Base);
					...
					Conquests.Conquests.CheckProgress<CraftingConquest>(
						from as PlayerMobile, item, quality, slayer, resource, craftSystem);
					//from.PlaySound( 0x57 );
				}
						...
						if ( faction != null )
						{
							...
							if ( town != null && town.Owner == faction )
							{
								...
								if ( pack != null )
								{
									...
									{
										GuardedRegion region = from.Region as GuardedRegion;
										queryFactionImbue = ( region != null);
									}
								}
							}
						}
			...
			else
			{
                ...
                if (XmlScript.HasTrigger(from, TriggerName.onCraft))
                    UberScriptTriggers.Trigger(from, from, TriggerName.onCraft, null, null, null, 0, null, craftSystem.MainSkill, from.Skills[craftSystem.MainSkill].Base);
		...
		private class InternalTimer : Timer
		{
			...
			private IBaseTool m_Tool;

			public InternalTimer( Mobile from, CraftSystem craftSystem, CraftItem craftItem, Type typeRes, IBaseTool tool, int iCountMax ) : base( TimeSpan.Zero, TimeSpan.FromSeconds( craftSystem.Delay ), iCountMax )
			{
				m_From = from;
