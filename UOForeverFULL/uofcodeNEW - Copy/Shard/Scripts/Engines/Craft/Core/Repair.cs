...
namespace Server.Engines.Craft
{
    public class Repair
    {
        ...
        private class InternalTarget : Target
        {
            ...
            private readonly IBaseTool m_Tool;
            ...
            private bool IsSpecialClothing(BaseClothing clothing)
            {
                ...
                {
                    return (clothing is BearMask) || (clothing is DeerMask) || (clothing is TheMostKnowledgePerson) ||
                           (clothing is TheRobeOfBritanniaAri) || (clothing is EmbroideredOakLeafCloak);
                }
            ...
            private bool IsSpecialWeapon(BaseWeapon weapon)
            {
                ...
                {
                    return (weapon is Cleaver) || (weapon is Hatchet) || (weapon is Pickaxe) || (weapon is ButcherKnife) ||
                ...
                else if (m_CraftSystem is DefBlacksmithy)
                {
                    return (weapon is Pitchfork)
                        ...
                        // TODO: Make these items craftable
                           || (weapon is RadiantScimitar) || (weapon is WarCleaver) || (weapon is ElvenSpellblade) ||
                           (weapon is AssassinSpike) || (weapon is Leafblade) || (weapon is RuneBlade) ||
                           (weapon is ElvenMachete) ||
                           (weapon is OrnateAxe) || (weapon is DiamondMace);
                ...
                {
                    return (weapon is ElvenCompositeLongbow) || (weapon is MagicalShortbow);
                }
            ...
            private bool IsSpecialArmor(BaseArmor armor)
            {
                ...
                if (m_CraftSystem is DefTailoring)
                {
                    return (armor is LeafTonlet) || (armor is LeafArms) || (armor is LeafChest) || (armor is LeafGloves) ||
                           (armor is LeafGorget) || (armor is LeafLegs) || (armor is HideChest) || (armor is HideGloves) ||
                           (armor is HideGorget) || (armor is HidePants) || (armor is HidePauldrons) ||
                           (armor is RangerArms) || (armor is RangerChest) || (armor is RangerGloves) ||
                           (armor is RangerGorget) || (armor is RangerLegs);
                }
                else if (m_CraftSystem is DefCarpentry)
                {
                    return (armor is WingedHelm) || (armor is RavenHelm) || (armor is VultureHelm) ||
                           (armor is WoodlandArms) ||
                           (armor is WoodlandChest) || (armor is WoodlandGloves) || (armor is WoodlandGorget) ||
                           (armor is WoodlandLegs);
                }
                else if (m_CraftSystem is DefBlacksmithy)
                {
                    return (armor is Circlet) || (armor is RoyalCirclet) || (armor is GemmedCirclet) ||
                           (armor is PhoenixArms) || (armor is PhoenixChest) || (armor is PhoenixGloves) ||
                           (armor is PhoenixGorget) || (armor is PhoenixHelm) || (armor is PhoenixLegs);
                }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                {
                    number = 1044282;
                    // You must be near a forge and and anvil to repair items. * Yes, there are two and's *
                }
                else if (targeted is BaseWeapon && ((BaseWeapon) targeted).EthicsItemState != null ||
                         targeted is BaseArmor && ((BaseArmor) targeted).EthicsItemState != null
                         || targeted is BaseClothing && ((BaseClothing) targeted).EthicsItemState != null)
                {
                    from.SendMessage(54, "Ethic items cannot be repaired!");
                }
                else if (m_CraftSystem is DefTinkering && targeted is Golem)
                {
                    ...
                    else
                    {
                        ...
                        {
                            number = 1044153;
                            // You don't have the required skills to attempt this item.	//TODO: How does OSI handle this with deeds with golems?
                        }
                ...
                else if (targeted is BaseWeapon)
                {
                    ...
                    else if (!weapon.IsChildOf(from.Backpack) && (!weapon.EraML || weapon.Parent != from))
                    {
                ...
                else if (targeted is BaseArmor)
                {
                    ...
                    else if (!armor.IsChildOf(from.Backpack) && (!armor.EraML || armor.Parent != from))
                    {
                        number = 1044275; // The item must be in your backpack to repair it.
                    }
                ...
                else if (targeted is BaseClothing)
                {
                    ...
                    if (m_CraftSystem.CraftItems.SearchForSubclass(clothing.GetType()) == null &&
                        ...
                        !((targeted is TribalMask) || (targeted is HornedTribalMask) ||
                          targeted.GetType().IsSubclassOf(typeof(HornedTribalMask)) ||
                          targeted.GetType().IsSubclassOf(typeof(TribalMask))))
                    {
                        number = (usingDeed) ? 1061136 : 1044277;
                        // That item cannot be repaired. // You cannot repair that item with this type of repair contract.
                    }
                    else if (!clothing.IsChildOf(from.Backpack) && (!clothing.EraML || clothing.Parent != from))
                    {
                        number = 1044275; // The item must be in your backpack to repair it.
                    }
