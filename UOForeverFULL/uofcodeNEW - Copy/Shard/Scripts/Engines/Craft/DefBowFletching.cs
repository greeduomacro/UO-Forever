...
namespace Server.Engines.Craft
{
	public class DefBowFletching : CraftSystem
	{
		...
		public static CraftSystem CraftSystem { get { return m_CraftSystem ?? (m_CraftSystem = new DefBowFletching()); } }
		...
		public override void InitCraftList()
		{
			...
			#region AOS
			index = AddCraft(typeof(CompositeBow), 1044566, 1029922, 70.0, 110.0, typeof(Log), 1044041, 7, 1044351);
			SetNeededExpansion(index, Expansion.AOS);
			index = AddCraft(typeof(RepeatingCrossbow), 1044566, 1029923, 90.0, 130.0, typeof(Log), 1044041, 10, 1044351);
			SetNeededExpansion(index, Expansion.AOS);
			#endregion
            ...
            // Add every material you want the player to be able to choose from, fletching cannot go over 100 so must keep the materials no more than 100
            // This will override the overridable material	
            AddSubRes(typeof(Log), 1072643, 00.0, 1044041, 1072652);
            AddSubRes(typeof(OakLog), 1072644, 100.0, 1044041, 1072652);
            AddSubRes(typeof(AshLog), 1072645, 100.0, 1044041, 1072652);
            AddSubRes(typeof(YewLog), 1072646, 100.0, 1044041, 1072652);
