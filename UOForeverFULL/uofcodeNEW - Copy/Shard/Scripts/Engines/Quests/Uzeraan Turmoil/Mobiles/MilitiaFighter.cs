...
namespace Server.Engines.Quests.Haven
{
	public class MilitiaFighter : BaseCreature
	{
		...
		public MilitiaFighter() : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			...
			AddItem( Immovable(new ThighBoots( 0x1BB )) );
			AddItem( Immovable(new LeatherChest()) );
			AddItem( Immovable(new LeatherArms()) );
			AddItem( Immovable(new LeatherLegs()) );
			AddItem( Immovable(new LeatherCap()) );
			AddItem( Immovable(new LeatherGloves()) );
			AddItem( Immovable(new LeatherGorget()) );
