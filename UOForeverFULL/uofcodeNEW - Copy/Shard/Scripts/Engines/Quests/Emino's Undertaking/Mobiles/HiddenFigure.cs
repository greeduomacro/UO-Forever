...
namespace Server.Engines.Quests.Ninja
{
	public class HiddenFigure : BaseQuester
	{
		...
		public override void InitOutfit()
		{
			...
			AddItem( Immovable(new TattsukeHakama( GetRandomHue() )) );
			AddItem( Immovable(new Kasa()) );
			AddItem( Immovable(new HakamaShita( GetRandomHue() )) );

			if ( Utility.RandomBool() )
				AddItem( Immovable(new Shoes( GetShoeHue() )) );
			else
				AddItem( Immovable(new Sandals( GetShoeHue() )) );
		}
