...
namespace Server.Engines.Doom
{
    public class LeverPuzzleController : Item
    {
        ...
        {
            CommandSystem.Register("GenLeverPuzzle", AccessLevel.Administrator,
                GenLampPuzzle_OnCommand);
        }
        ...
        public static void GenLampPuzzle_OnCommand(CommandEventArgs e)
        {
            if (Map.Malas.GetItemsInRange(lp_Center, 0).OfType<LeverPuzzleController>().Any())
            {
                e.Mobile.SendMessage(
                    "Lamp room puzzle already exists: please delete the existing controller first ...");
                return;
            }
            e.Mobile.SendMessage("Generating Lamp Room puzzle...");
            new LeverPuzzleController().MoveToWorld(lp_Center, Map.Malas);

            e.Mobile.SendMessage(!installed
        ...
        {
            var statue = (LeverPuzzleStatue) m_Statues[index];
        ...
        {
            var lever = (LeverPuzzleLever) m_Levers[index];
        ...
        public virtual void LeverPulled(UInt16 code)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    PuzzleStatus(Statue_Msg[Correct], (Correct > 0) ? Correct.ToString(CultureInfo.InvariantCulture) : null);
            ...
            protected override void OnTick()
            {
                ...
                else
                {
                    ...
                    else if (Count == 3)
                    {
                        ...
                        for (int i = 0; i < j; i++)
                        {
                            IEntity m_IEntity = new Entity(Serial.Zero, RandomPointIn(m_Player.Location, 10),
                                m_Player.Map);

                            var mobiles = m_IEntity.Map.GetMobilesInRange(m_IEntity.Location, 2).Cast<Mobile>().ToList();
                            foreach (Mobile t in mobiles)
                            {
                                if (IsValidDamagable(t) && t != m_Player)
                                {
                                    PlayEffect(m_Player, t, Rock(), 8, true);
                                    DoDamage(t, 25, 30, false);
