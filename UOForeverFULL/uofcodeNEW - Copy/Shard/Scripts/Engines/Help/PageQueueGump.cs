        ...
        public PageQueueGump() : base(30, 30)
        {
            ...
            if (m_List.Length > 0)
            {
                ...
                for (int i = 0; i < m_List.Length; ++i)
                {
                    ...
                    string html = String.Format("[{0}] {1} <basefont color=#{2:X6}>[<u>{3}</u>]</basefont>", typeString,
                        e.Message, e.Handler == null ? 0xFF0000 : 0xFF, e.Handler == null ? "Unhandled" : "Handling");
    ...
    public class PredefinedResponse
    {
        ...
        public static void Save()
        {
            ...
            try
            {
                ...
                {
                    foreach (object t in m_List)
                    {
                        PredefinedResponse resp = (PredefinedResponse) t;
    ...
    public class PageEntryGump : Gump
    {
        ...
        public PageEntryGump(Mobile m, PageEntry entry) : base(30, 30)
        {
            try
            {
                ...
                AddLabelCropped(128, 18, 264, 20, 2100, entry.Sent.ToString(CultureInfo.InvariantCulture));

                AddLabel(18, 38, 2100, "Sender:");
                AddLabelCropped(128, 38, 264, 20, 2100,
                    String.Format("{0} {1} [{2}]", entry.Sender.RawName, entry.Sender.Location, entry.Sender.Map));
                ...
                {
                    AddLabelCropped(128, 58, 264, 20, m_AccessLevelHues[(int) entry.Handler.AccessLevel],
                        entry.Handler.Name);
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // go to sender
                {
                    ...
                    else
                    {
                        ...
                        m.SendMessage("You have been teleported to that player whom paged.");
                ...
                case 2: // go to handler
                {
                    ...
                    if (h != null)
                    {
                        ...
                        else
                        {
                            ...
                            m.SendMessage("You have been teleported to the staff handling the page.");
                            Resend(state);
                        }
                    }
                    else
