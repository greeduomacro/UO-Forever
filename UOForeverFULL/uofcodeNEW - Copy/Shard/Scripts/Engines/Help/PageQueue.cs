...
namespace Server.Engines.Help
{
    ...
    public class PageEntry
    {
        ...
        private class InternalTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                {
                    m_Entry.Sender.SendLocalizedMessage(1008077, true, (index + 1).ToString(CultureInfo.InvariantCulture));
    ...
    public class PageQueue
    {
        ...
        private static void SendEmail(PageEntry entry)
        {
            ...
            MailMessage mail = new MailMessage("noreply@uoforever.com", Email.SpeechLogPageAddresses)
            {
                Subject = "Forever Speech Log Page Forwarding"
            };

            using (StringWriter writer = new StringWriter())
            {
                writer.WriteLine("Forever Speech Log Page - {0}", GetPageTypeName(entry.Type));
                writer.WriteLine();
                ...
                writer.WriteLine("Sent on: {0}/{1:00}/{2:00} {3}:{4:00}:{5:00}", time.Year, time.Month, time.Day,
                ...
                foreach (SpeechLogEntry logEntry in entry.SpeechLog)
                {
                    ...
                    writer.WriteLine("{0}:{1:00}:{2:00} - {3} ({4}): '{5}'", created.Hour, created.Minute,
                        created.Second, fromName, fromAccount, speech);
                }
