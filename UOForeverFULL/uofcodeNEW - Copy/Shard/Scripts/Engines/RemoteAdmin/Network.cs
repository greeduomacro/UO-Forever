...
namespace Server.RemoteAdmin
{
	public class AdminNetwork
	{
		...
		public static void OnReceive( NetState state, PacketReader pvSrc )
		{
			...
			{
				string statStr = String.Format( ", Name={0}, Age={1}, Clients={2}, Items={3}, Chars={4}, Mem={5}K, Ver={6}", Server.Misc.ServerList.ServerName, (int)(DateTime.UtcNow - Server.Items.Clock.ServerStart).TotalHours, NetState.Instances.Count + 100, World.Items.Count, World.Mobiles.Count, (int)(System.GC.GetTotalMemory( false ) / 1024), ProtocolVersion );
		...
		{
			Timer.DelayCall<NetState>( TimeSpan.FromSeconds( 15.0 ), new TimerStateCallback<NetState>( Disconnect ), state );
		}
