...
namespace Server.Engines.ConPVP
{
	...
	public class Ladder
	{
		...
		private List<LadderEntry> m_Entries;

		public List<LadderEntry> Entries{ get { return m_Entries; } }

		private int Swap( int idx, int newIdx )
		{
			LadderEntry hold = m_Entries[idx];

			m_Entries[idx] = m_Entries[newIdx];
			m_Entries[newIdx] = hold;

			m_Entries[idx].Index = idx;
			m_Entries[newIdx].Index = newIdx;
		...
		public Ladder()
		{
			...
			m_Entries = new List<LadderEntry>();
		}
		...
		public Ladder( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Entries = new List<LadderEntry>( count );
	...
	public class LadderEntry : IComparable
	{
		...
		[CommandProperty( AccessLevel.EventMaster, AccessLevel.Administrator )]
