...
namespace Server.Engines.ConPVP
{
	...
	{
		private static readonly List<RulesetLayout> _Rulesets = new List<RulesetLayout>
		{
			new RulesetLayout(
				"Spells",
				new[]
				{
					...
					new[]
					{
						"Chain Lightning", "Energy Field", "Flame Strike", //
		...
		private static readonly List<RulesetLayout> _RulesetAOS = new List<RulesetLayout>
		{
			new RulesetLayout(
				"Chivalry",
				new[]
				{
					"Cleanse by Fire", "Close Wounds", "Consecrate Weapon", //
					"Dispel Evil", "Divine Fury", "Enemy of One", //
					"Holy Light", "Noble Sacrifice", "Remove Curse", //
					"Sacred Journey"
				}),
			new RulesetLayout(
				"Necromancy",
				new[]
				{
					"Animate Dead", "Blood Oath", "Corpse Skin", //
					"Curse Weapon", "Evil Omen", "Horrific Beast", //
					"Lich Form", "Mind Rot", "Pain Spike", //
					"Poison Strike", "Strangle", "Summon Familiar", //
					"Vampiric Embrace", "Vengeful Spirit", "Wither", //
					"Wraith Form"
				})
		};

		private static readonly List<RulesetLayout> _RulesetSE = new List<RulesetLayout>
		{
			new RulesetLayout(
				"Bushido",
				new[]
				{
					"Confidence", "Counter Attack", "Evasion", //
					"Honorable Execution", "Lightning Strike", "Momentum Strike"
				}),
			...
			new RulesetLayout(
				...
				new[]
				{
					"Animal Form", "Backstab", "Death Strike", //
					"Focus Attack", "Ki Attack", "Mirror Image", //
					"Shadow Jump", "Suprise Attack"
				})
		};

		private static readonly List<RulesetLayout> _RulesetML = new List<RulesetLayout>
		{
			new RulesetLayout(
				"Spellweaving",
				new[]
				{
					"Arcane Circle", "Arcane Empowerment", "Attune Weapon", //
					"Dryad Allure", "Essence of Wind", "Ethereal Voyage", //
					"Gift of Life", "Gift of Renewal", "Immolating Weapon", //
					"Nature's Fury", "Reaper Form", "Summon Fey", //
					"Summon Fiend", "Thunderstorm", "Wildfire", //
					"Word of Death"
				})
		};

		private static RulesetLayout GetRuleset(Expansion e)
		{
			List<RulesetLayout> entries = new List<RulesetLayout>(_Rulesets);

			if (e >= Expansion.AOS)
			{
				entries.AddRange(_RulesetAOS);

				if (e >= Expansion.SE)
				{
					entries.AddRange(_RulesetSE);

					if (e >= Expansion.ML)
					{
						entries.AddRange(_RulesetML);
					}
				}
			}
				...
				if (e >= Expansion.SE)
				{
					entries.Add(
						new RulesetLayout(
							...
							new[]
							{
								"Stun", "Disarm", "Armor Ignore", "Bleed Attack", //
								"Concussion Blow", "Crushing Blow", "Disarm", //
								"Dismount", "Double Strike", "Infectious Strike", //
								"Mortal Strike", "Moving Shot", "Paralyzing Blow", //
								"Shadow Strike", "Whirlwind Attack", "Riding Swipe", //
				...
				else
				{
					entries.Add(
						new RulesetLayout(
							...
							new[]
							{
								"Stun", "Disarm", "Armor Ignore", //
								"Bleed Attack", "Concussion Blow", "Crushing Blow", //
								"Disarm", "Dismount", "Double Strike", //
								"Infectious Strike", "Mortal Strike", "Moving Shot", //
								"Paralyzing Blow", "Shadow Strike", "Whirlwind Attack"
							}));
				}
			}
			else
			{
				entries.Add(
					new RulesetLayout(
						...
						new[]
						{
							"Stun", "Disarm", "Concussion Blow", //
							"Crushing Blow", "Paralyzing Blow"
						}));
			}
			...
			entries.Add(
				new RulesetLayout(
					...
					new[]
					{
						"Anatomy", "Detect Hidden", "Evaluating Intelligence", //
						"Hiding", "Poisoning", "Snooping", //
						"Stealing", "Spirit Speak", "Stealth"
					}));
			...
			if (e >= Expansion.AOS)
			{
				...
				entries.Add(new RulesetLayout("Armor", new[] {"Magical", "Shields"}));
			}
			else
			{
				entries.Add(
					new RulesetLayout(
						...
						{
							"Magical", "Melee", "Ranged", //
							"Poisoned", "Wrestling", "Runics"
						}));

				entries.Add(new RulesetLayout("Armor", new[] {"Magical", "Shields", "Colored"}));
			}
				...
				entries.Add(
					new RulesetLayout(
						...
						new[]
						{
							...
							new[]
							{
								"Agility", "Cure", "Explosion", //
								"Heal", "Nightsight", "Poison", //
								"Refresh", "Strength"
							})
						},
						new[]
						{
							"Bandages", "Trapped Containers", "Bolas", //
							"Orange Petals", "Shurikens", "Fukiya Darts", //
							"Fire Horns"
						}));
			}
			else
			...
			return new RulesetLayout("Rules", entries.ToArray());
		}
		...
		public RulesetLayout FindByTitle(string title)
		{
			...
			return m_Children.Select(t => t.FindByTitle(title)).FirstOrDefault(layout => layout != null);
		}
		...
		public string FindByIndex(int index)
		{
			...
			return m_Children.Select(t => t.FindByIndex(index)).FirstOrDefault(opt => opt != null);
		}
		...
		public RulesetLayout FindByOption(string title, string option, ref int index)
		{
			...
			foreach (RulesetLayout l in m_Children)
			{
				RulesetLayout layout = l.FindByOption(title, option, ref index);
		...
		private int RecurseComputeOffsets(ref int offset)
		{
			...
			foreach (RulesetLayout l in m_Children)
			{
				m_TotalLength += l.RecurseComputeOffsets(ref offset);
			}
		...
		public RulesetLayout(string title, string description, RulesetLayout[] children, string[] options)
		{
			...
			foreach (RulesetLayout l in children)
			{
				l.m_Parent = this;
			}
		}
