...
namespace Server.Engines.ConPVP
{
	...
	{
        private Type[] m_AlwaysDisallowedSpells = new Type[] { typeof(RecallSpell), typeof(TeleportSpell), typeof(MarkSpell), typeof(GateTravelSpell) };
		...
		public List<Participant> Participants{ get{ return m_Participants; } }
		...
		public bool CheckItemEquip( Mobile from, Item item )
		{
			...
			else if ( item is BaseArmor )
			{
				...
				if (!from.EraAOS && armor.Resource != armor.DefaultResource && !m_Ruleset.GetOption("Armor", "Colored"))
					return false;
			...
			else if ( item is BaseWeapon )
			{
				...
				if (!from.EraAOS && weapon.Resource != CraftResource.Iron && weapon.Resource != CraftResource.None && !m_Ruleset.GetOption("Weapons", "Runics"))
					return false;
		...
		public bool AllowItemUse( Mobile from, Item item )
		{
            ...
            else if (item is BallOfSummoning)
            {
                from.SendMessage(38, "You cannot summon pets while in a duel!");
                return false;
            }
		...
		public bool CheckFull()
		{
			...
			{
				Participant p = m_Participants[i];
        ...
        {
            return m_Participants.All(p => !p.Players.Select(player => player.Mobile as PlayerMobile).Any(pm => pm != null && pm.PokerGame != null));
        }
		...
		public void Finish( Participant winner )
		{
			...
			for ( int i = 0; i < m_Participants.Count; ++i )
			{
				Participant loser = m_Participants[i];
			...
			if ( IsOneVsOne )
			{
				DuelPlayer dp1 = m_Participants[0].Players[0];
				DuelPlayer dp2 = m_Participants[1].Players[0];
		...
		public void QueryRematch()
		{
			...
			for ( int i = 0; i < m_Participants.Count; ++i )
			{
				Participant oldPart = m_Participants[i];
		...
		public Participant CheckCompletion()
		{
			...
			if ( hasWinner )
				return winner == null ? m_Participants[0] : winner;
		...
		public void InvokeAutoTie()
		{
			...
			List<TournyParticipant> remaining = new List<TournyParticipant>();
		...
		public bool IsOneVsOne
		{
			get
			{
				...
				if ( (m_Participants[0]).Players.Length != 1 )
					return false;

				if ( (m_Participants[1]).Players.Length != 1 )
					return false;
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register( "vli", AccessLevel.EventMaster, new CommandEventHandler( vli_oc ) );
		}
			...
			for ( int i = 0; i < m_Participants.Count; ++i )
			{
				...
				for ( int j = 0; j < p.Players.Length; ++j )
				{
					...
					if ( count > 0 )
					{
					    ...
					    if (m_Ruleset.Era == Era.T2A)
					        mob.IsT2A = true;

                        if (m_Ruleset.Era == Era.Arcade)
                            mob.IsArcade = true;
					}
					else
					{
		...
		private class ReturnEntry
		{
			...
			public ReturnEntry( GenericReader reader )
			{
