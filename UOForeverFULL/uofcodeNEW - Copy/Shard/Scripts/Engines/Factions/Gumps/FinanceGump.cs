...
namespace Server.Factions
{
    public class FinanceGump : FactionGump
    {
        ...
        {
            CommandSystem.Register("TownFinance", AccessLevel.GameMaster, new CommandEventHandler(TownFinance_Command));
        }
        ...
        {
            if (m_From.AccessLevel < AccessLevel.GameMaster && (!m_Town.IsFinance(m_From) || m_Town.Owner != m_Faction))
            {
            ...
            switch (type)
            {
                case 0: // general
                {
                    switch (index)
                    {
                        case 0: // set price
                        {
                            ...
                            {
                                if (m_Town.Definition != null && m_Town.Definition.TownName != null &&
                                    m_Town.Definition.TownName.String.ToLower() == "britain")
                                {
                                    m_From.SendMessage("Sorry, Britain is untaxable.");
                                    m_Town.Tax = 0;
                                }
                                else
                                {
                ...
                case 1: // make vendor
                {
                    ...
                    if (index >= 0 && index < vendorLists.Count)
                    {
                        ...
                        var itemlist = m_From.GetItemsInRange(m_From.Map, 10);
                        ...
                        else if (!itemlist.Exists(x=> x is VendorObelisk))
                        {
                            m_From.SendMessage(61, "Faction vendors can only be placed near a vendor obelisk.");
                        }
