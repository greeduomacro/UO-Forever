...
namespace Server.Factions
{
	public class Keywords
	{
		...
		private static void EventSink_Speech( SpeechEventArgs e )
		{
            ...
            if (e.Mobile is BaseCreature) return;

			if ( e.Speech.ToLower().IndexOf( "i wish to renew my spirit" ) > -1 )
			{
			...
			for ( int i = 0; i < keywords.Length; ++i )
			{
				switch ( keywords[i] )
				{
					...
					case 0x00EC: // *showscore*
					{
						...
						if ( pl != null )
							Timer.DelayCall<PlayerState>( TimeSpan.Zero, new TimerStateCallback<PlayerState>( ShowScore_Sandbox ), pl );
