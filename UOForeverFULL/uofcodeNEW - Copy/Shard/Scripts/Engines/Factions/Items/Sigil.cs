...
namespace Server.Factions
{
	public class Sigil : BaseSystemController
	{
		...
		public static TimeSpan CorruptionGrace { get { return PvPController._SigilCorruptionGrace; } }     

		// Sigil must be held at a stronghold for this amount of time in order to become corrupted
        public static TimeSpan CorruptionPeriod { get { return PvPController._SigilCorruptionPeriod; } }     

		// After a sigil has been corrupted it must be returned to the town within this period of time
		public static TimeSpan ReturnPeriod { get { return PvPController._SigilReturnPeriod; } }  

		// Once it's been returned the corrupting faction owns the town for this period of time
        public static TimeSpan PurificationPeriod { get { return PvPController._SigilPurificationPeriod; } } 
		...
		public void Update()
		{
			...
			if ( m_Town != null && !m_Town.State.Active )
				Visible = false;
			else
				Visible = true;
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if ( IsCorrupting )
			{
				double hours = ( DateTime.UtcNow - m_CorruptionStart ).TotalHours; //Should be less than or equal to CorruptPeriod.
