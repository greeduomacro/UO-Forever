...
namespace Server.Factions
{
	...
	{
		public static readonly int InactiveHue = 1156;
		...
		public bool Active
		{
			get{ return m_Town != null ? m_Town.State.Active : true; }
			set
			{
				if ( m_Town != null )
				{
					if ( value )
					{
						...
						UpdateSigil();
						if ( m_Sigil != null )
							m_Sigil.Visible = true;

						m_Town.State.Active = true;
					}
					else
					{
						Hue = InactiveHue;
						UpdateSigil();
						if ( m_Sigil != null )
							m_Sigil.Visible = false;

						m_Town.State.Active = false;
					}
				}
			}
		}
