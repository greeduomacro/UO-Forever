	...
	public class Spawner : Item, ISpawner
	{
        ...
        public List<string> SpawnNames
        {
            get {
                ...
                foreach (SpawnerEntry entry in m_Entries)
                {
                    output.Add(entry.SpawnedName);
                }
                return output;
            }
        }
...
//		[Constructable]
        ...
        public Spawner(int amount, TimeSpan minDelay, TimeSpan maxDelay, int team, int homeRange, List<string> spawnedNames)
            : base(0x1f13)
        {
            InitSpawn(amount, minDelay, maxDelay, team, homeRange);
            foreach (string spawnedName in spawnedNames)
            {
                AddEntry(spawnedName, 100, amount, false);
            }
        }
