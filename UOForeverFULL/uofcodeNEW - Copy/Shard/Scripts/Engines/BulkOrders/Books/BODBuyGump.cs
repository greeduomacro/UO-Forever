...
namespace Server.Engines.BulkOrders
{
	public class BODBuyGump : Gump
	{
				...
				if (m_Book.Entries.Contains(m_Object) && pv != null)
				{
					...
					else
					{
						...
						else
						{
							...
							{
								Type cType = pv.TypeOfCurrency;

								if (pack.ConsumeTotal(cType, price, true) || Banker.Withdraw(m_From, cType, price))
								{
									m_Book.Entries.Remove(m_Object);
									m_Book.InvalidateProperties();
									pv.HoldCurrency += price;
			...
			: base(100, 200)
		{
			...
			AddLabel(125, 95, 0, price.ToString("#,0"));
