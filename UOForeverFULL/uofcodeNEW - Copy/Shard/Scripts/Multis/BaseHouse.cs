...
namespace Server.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        public static int MaxFriends { get { return 150; } }
        public static int MaxBans { get { return 150; } }
        ...
        public DateTime NextDecayStage { get { return m_NextDecayStage; } set { m_NextDecayStage = value; } }
        ...
        public virtual DecayType DecayType
        {
            get
            {
                ...
                if (m_Owner == null)
                {
                    return EraAOS ? DecayType.Condemned : DecayType.ManualRefresh;
                }
...
*/
        ...
        public static bool CanOwnHouse(Mobile mob)
        {
            ...
            var account = mob.Account as Account;
            if (account == null)
            {
                return true; // must be a serious problem going on
            }
            int skillSum = 0;
            TimeSpan totalGameTime = TimeSpan.Zero;
            foreach (Mobile accountMob in account.Mobiles)
            {
                if (accountMob == null)
                {
                    continue;
                }
                skillSum += accountMob.SkillsTotal;
                if (accountMob is PlayerMobile)
                {
                    var pm = accountMob as PlayerMobile;
                    totalGameTime += pm.GameTime;
                }
                else
                {
                    return true; // must be something really weird going on
                }
            }
            // divide skillsum by 10 since 7000 would be 700.0 (interesting way they did it)
            skillSum /= 10;
            if (skillSum > HouseSystemController._OwnHouseMinSkillsOnAccount &&
                totalGameTime > HouseSystemController._OwnHouseMinGameTime)
            {
                return true;
            }
            return false;
        ...
        public TimeSpan TimeToDecay
        {
            get
            {
                TimeSpan timeAfterRefresh = DecayPeriod - (DateTime.UtcNow - m_LastRefreshed);
                return timeAfterRefresh;
            }
        }
            ...
            get
            {
                ...
                else if (DynamicDecay.Enabled)
                {
                    ...
                    if (stage == DecayLevel.Ageless ||
                        (DynamicDecay.Decays(stage) && m_NextDecayStage <= DateTime.UtcNow))
                    {
        ...
        public DecayLevel GetOldDecayLevel()
        {
            ...
            {
                //return ( HasRentedVendors || VendorInventories.Count > 0 ) ? DecayLevel.DemolitionPending : DecayLevel.Collapsed; // Kill all vendors
                return DecayLevel.Collapsed;
            }
            else if (percent >= 986) // 95.0% - 98.5%
            {
        ...
        {
            var list = new List<Mobile>(PlayerVendors);
        ...
        {
            get { return TimeSpan.FromHours(1.25 + (Utility.RandomDouble() * 1.75)); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual double BonusStorageScalar { get { return (EraML ? 1.2 : 1.0); } }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual double StorageMultiplier { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual double VendorMultiplier { get; set; }
        ...
        public virtual bool IsAosRules { get { return EraAOS; } }
        ...
        public virtual int GetAosMaxSecures()
        {
            ...
            {
                return (int) Math.Ceiling(hpe.Storage * StorageMultiplier);
            }
            return (hpe.Storage);
        }
        ...
        public virtual int GetAosCurSecures(
            out int fromSecures, out int fromVendors, out int fromLockdowns, out int fromMovingCrate)
        {
            fromSecures = 0;
            fromVendors = 0;
            fromLockdowns = 0;
        ...
        public virtual int GetNewVendorSystemMaxVendors()
        {
            ...
            if (VendorMultiplier > 1.0)
            {
                return (int)((Math.Ceiling(hpe.Vendors * VendorMultiplier)));
            }
            return (hpe.Vendors);
        }
        ...
        public void RestoreRelocatedEntities()
        {
            foreach (RelocatedEntity relocEntity in RelocatedEntities)
            {
                ...
                {
                    Console.WriteLine(
                        "WARNING: non-item or mobile entity found in RelocateEntities!  Should not be possible!");
                }
            }
        ...
        public static bool CheckHold(
            Mobile m, Container cont, Item item, bool message, bool checkItems, int plusItems, int plusWeight)
        {
        ...
        public virtual bool IsInside(Point3D p, int height)
        {
            ...
            if (this is Castle)
            {
                if ((p.X >= X + 2 && p.Y >= Y - 10 && p.X <= X + 7 && p.Y <= Y - 6) ||
                    (p.X >= X + 8 && p.Y >= Y - 7 && p.X <= X + 8 && p.Y <= Y - 6) ||
                    (p.X >= X + 9 && p.Y >= Y - 7 && p.X <= X + 10 && p.Y <= Y + 7) ||
                    (p.X >= X + 8 && p.Y >= Y + 6 && p.X <= X + 8 && p.Y <= Y + 7) ||
                    (p.X >= X + 3 && p.Y >= Y + 6 && p.X <= X + 7 && p.Y <= Y + 10) ||
                    (p.X >= X - 1 && p.Y >= Y + 7 && p.X <= X + 2 && p.Y <= Y + 9) ||
                    (p.X >= X - 7 && p.Y >= Y + 6 && p.X <= X - 2 && p.Y <= Y + 10) ||
                    (p.X >= X - 8 && p.Y >= Y + 6 && p.X <= X - 8 && p.Y <= Y + 7) ||
                    (p.X >= X - 10 && p.Y >= Y - 7 && p.X <= X - 9 && p.Y <= Y + 7) ||
                    (p.X >= X - 8 && p.Y >= Y - 7 && p.X <= X - 8 && p.Y <= Y - 6))
                {
                    if (p.Z >= Z - 4 && p.Z <= Z + 16)
                    {
        ...
        public static ArrayList AllHouses
        {
            get
            {
                var output = new ArrayList(3200); // like 2600 houses on UO Forever 9/14/2013
                output.AddRange(m_AllHouses);
                return output;
            }
        }
        ...
        private class TransferItem : Item
        {
            ...
            public override string DefaultName { get { return "a house transfer contract"; } }
            ...
            public override void OnSingleClick(Mobile from)
            {
                ...
                bool valid = m_House != null &&
                             Sextant.Format(m_House.Location, m_House.Map, ref xLong, ref yLat, ref xMins, ref yMins,
            ...
            public override bool AllowSecureTrade(Mobile from, Mobile to, Mobile newOwner, bool accepted)
            {
                ...
                {
                    from.SendLocalizedMessage(501388);
                        // You cannot transfer ownership to another house owner or co-owner!
                    return false;
                }
        ...
        public void BeginConfirmTransfer(Mobile from, Mobile to)
        {
            ...
            if (!CanOwnHouse(to))
            {
                from.SendMessage(
                    38,
                    "They do not meet the requirements to own a house. They must have over " +
                    HouseSystemController._OwnHouseMinSkillsOnAccount +
                    " skill points total across all characters on their account and also have accrued " +
                    UberScriptFunctions.Methods.TIMESPANSTRING(null, HouseSystemController._OwnHouseMinGameTime) +
                    " of game time on their account.");
                return;
            }
        ...
        public void AddSecure(Mobile m, Item item)
        {
            ...
            {
                if (item is BaseHouseDoor)
                {
                    m.SendGump(new SetSecureLevelGump(m_Owner, item as ISecurable, this));
                }
                else if (LockDown(m, item))
                {
                    if (item is ISecurable)
                    {
                        m.SendGump(new SetSecureLevelGump(m_Owner, item as ISecurable, this));
        ...
        public virtual bool IsCombatRestricted(Mobile m)
        {
            ...
            for (int i = 0; i < m.Aggressed.Count; ++i)
            {
				...
				{
					return true; //change to false-- Adam
				}*/
                Faction attackerFaction = Faction.Find(m);
                Faction defenderFaction = Faction.Find(info.Defender);

                if (info.Defender.Player && info.Defender.Alive &&
                    (DateTime.UtcNow - info.LastCombatTime) < HouseRegion.CombatHeatDelay &&
                    attackerFaction != null && defenderFaction != null && attackerFaction != defenderFaction)
                {
                    return true;
                }
            }
        ...
        public void Kick(Mobile from, Mobile targ)
        {
            if (IsFriend(from))
            {
                ...
                else if (IsFriend(targ) /*&& EraUOTD && !EraML*/)
                {
                    from.SendLocalizedMessage(501349); // You cannot eject a friend of the house!
                }
        ...
        {
            if (IsFriend(from) && m_Bans != null && m_Bans.Contains(targ))
            {
                m_Bans.Remove(targ);
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(VendorMultiplier);
            writer.Write(StorageMultiplier);
        ...
        public int MaxSecures {
            get
            {
                ...
                {
                    return (int)Math.Ceiling(m_MaxSecures * StorageMultiplier);
                }
        ...
        public int MaxLockDowns {
            get
            {
                ...
                {
                    return (int)Math.Ceiling(m_MaxLockDowns * StorageMultiplier);
                }
        ...
        public override void OnAfterDelete()
        {
            ...
            var inventories = new List<VendorInventory>(VendorInventories);
        ...
        {
            return m != null &&
                   (m == m_Owner || m.AccessLevel >= AccessLevel.GameMaster || AccountHandler.CheckAccount(m, m_Owner));
        }

        public bool IsCoOwner(Mobile m)
        {
            return m != null && m_CoOwners != null && (IsOwner(m) || m_CoOwners.Contains(m));
        }

        public bool IsGuildMember(Mobile m)
        {
            return m != null && Owner != null && Owner.Guild != null && m.Guild == Owner.Guild;
        }
        ...
        {
            return m != null && m_Friends != null && (IsCoOwner(m) || m_Friends.Contains(m));
        }
        ...
        public bool HasAccess(Mobile m)
        {
            ...
            if (m is BaseCreature)
            {
                ...
                if (m != null &&
                    (m.AccessLevel > AccessLevel.Player || IsFriend(m) || (m_Access != null && m_Access.Contains(m))))
                {
                    return true;
                }
            }
        ...
        {
            return check != null && m_LockDowns != null &&
                   (m_LockDowns.Contains(check) || VendorRentalContracts.Contains(check));
        }

        public new bool IsSecure(Item item)
        {
            if (item == null || m_Secures == null)
            {
                return false;
            }
    ...
    public class AddonTarget : Target
    {
        ...
        public AddonTarget(bool release, BaseHouse house)
                ...
                else
                {
                    if (targeted is VendorRentalContract ||
                        (targeted is Container &&
                         ((Container) targeted).FindItemByType(typeof(VendorRentalContract)) != null))
                    {
    ...
    public class SetSecureLevelEntry : ContextMenuEntry
    {
        ...
        public static ISecurable GetSecurable(Mobile from, Item item)
        {
            ...
            if (item is ISecurable)
            {
                bool isOwned = (item is BaseDoor && house.Doors.Contains(item)) ||
                               (house is HouseFoundation && ((HouseFoundation) house).IsFixture(item)) ||
                               house.IsLockedDown(item);
