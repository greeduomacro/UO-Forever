...
namespace Server.Multis
{
    ...
    public class BoatDamageEntry
    {
        public int Damage { get; set; }
        public Mobile Mob { get; set; }
        public DateTime Expiration { get; set; }

        public BoatDamageEntry(int damage, Mobile mob, DateTime expiration)
        {
            Damage = damage;
            Mob = mob;
    ...
    {
        // Alan Mod
        public const int DamagedHits = 60;
        public const int CriticalHits = 30;

        private bool m_Sunk = false;
        [CommandProperty(AccessLevel.GameMaster)]
        public bool Sunk { get { return m_Sunk; } set { m_Sunk = value; if (m_Sunk) SunkenList = GetMovingEntities(); } }

        
        private List<BoatDamageEntry> m_DamageEntries = new List<BoatDamageEntry>();
        public List<BoatDamageEntry> DamageEntries { get { return m_DamageEntries; } set { m_DamageEntries = value; } }

        private List<IEntity> SunkenList = null;

        public void AddDamageEntry(Mobile mob, int damage, DateTime expiration)
        {
            BoatDamageEntry existingEntry = null;
            foreach (BoatDamageEntry entry in m_DamageEntries)
            {
                if (entry.Mob == mob) // update that entry
                {
                    entry.Damage += damage;
            ...
            {
                m_DamageEntries.Remove(existingEntry);
                m_DamageEntries.Add(existingEntry); // put it at the end
            }
            else
            {
                m_DamageEntries.Add(new BoatDamageEntry(damage, mob, expiration));
            }
        }

        private class BoatDamageDecayTimer : Timer
        {
            public static void Initialize()
            {
                new BoatDamageDecayTimer().Start();
            }

            public BoatDamageDecayTimer()
            ...
            protected override void OnTick()
            {
                ...
                foreach (BaseBoat instance in m_Instances)
                {
                    while (instance.m_DamageEntries.Count > 0 && instance.m_DamageEntries[0].Expiration < now)
                    {
                        instance.m_DamageEntries.RemoveAt(0);
                    }
                }
            }
        }
        ...
        private static Rectangle2D[] m_BritWrap = new Rectangle2D[] { new Rectangle2D(16, 16, 5120 - 32, 4096 - 32), new Rectangle2D(5136, 2320, 992, 1760), new Rectangle2D(5164, 763, 300, 300) };
        ...
        private List<BoatComponent> m_BoatComponents = new List<BoatComponent>();
        public List<BoatComponent> BoatComponents { get { return m_BoatComponents; } set { m_BoatComponents = value; } }
        ...
        public int Hits { get { return m_Hits; } 
            set
            {
                ...
                foreach (BoatComponent component in BoatComponents)
                {
                    component.SetFacing(Facing);
                    if (m_Hits < CriticalHits)
                    {
                        if (component is BoatRope)
                        {
                            ((BoatRope)component).Open();
                        }
                    }
                }
                if (m_Hits < CriticalHits)
                {
                    if (m_PPlank.Visible) { m_PPlank.Open(); }
                    if (m_SPlank.Visible) { m_SPlank.Open(); }
                }
            } 
        }

        private bool m_RequiresWater = true;
        [CommandProperty(AccessLevel.GameMaster)]
        public bool RequiresWater { get { return m_RequiresWater; } set { m_RequiresWater = value; } }
        ...
        public void UpdateComponents()
        {
            ...
            foreach (BoatComponent component in BoatComponents)
            {
                Point3D componentLoc = GetRotatedLocation(component.Offset.X, component.Offset.Y);
                componentLoc.Z += component.Offset.Z;
                component.MoveToWorld(componentLoc, Map);
                component.SetFacing(m_Facing);
            }
        }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // version 8
            writer.Write((bool)m_Sunk);

            // version 7
            writer.Write((int)m_DamageEntries.Count);
            foreach (BoatDamageEntry entry in m_DamageEntries)
            {
                writer.Write((Mobile)entry.Mob);
                writer.Write((int)entry.Damage);
                writer.Write((TimeSpan)(entry.Expiration - DateTime.UtcNow));
            }

            // version 6
            writer.Write((int)BoatComponents.Count);
            foreach (BoatComponent component in BoatComponents)
            {
            ...
            // version 4
            writer.Write((bool)m_RequiresWater);
        ...
        public override void OnAfterDelete()
        {
            ...
            BoatComponents = new List<BoatComponent>();
        ...
        public override void OnLocationChange(Point3D old)
        {
            ...
            foreach (BoatComponent component in BoatComponents)
            {
                component.Location = new Point3D(X + (component.X - old.X), Y + (component.Y - old.Y), Z + (component.Z - old.Z));
            }
            // change the z level of everything on board if the z level changed
            if (old.Z != Location.Z)
            {
                List<IEntity> onBoardStuff = GetMovingEntities();
                int zDiff = Location.Z - old.Z;
                if (Sunk)
                {
                    foreach (IEntity entity in onBoardStuff)
                    {
                        if (entity is BoatComponent)
                        {
                            if (((BoatComponent)entity).Boat == this)
                            {
                                ((BoatComponent)entity).Z += zDiff;
                            }
                        }
                        // if it's still onBoard and in the sunkenlist, it should sink as well (this catches stuff like stacked items that
                        // would have been missed by the on-deck check below
                        else if (SunkenList != null && SunkenList.Contains(entity))
                        {
                            if (entity is Item) { ((Item)entity).Z += zDiff; }
                            else if (entity is Mobile) { ((Mobile)entity).Z += zDiff; }
                            continue;
                        }
                        else
                        {
                            // only shift those things that are right on the deck (prevent boat from being pulled down on top of it)
                            if (entity.Z == old.Z + this.MarkOffset.Z) // mark offset is the Z offset to the deck
                            {
                                if (entity is Item) { ((Item)entity).Z += zDiff; }
                                else if (entity is Mobile) { ((Mobile)entity).Z += zDiff; }
                            }
        ...
        {
            if (Key.ContainsKey(m.Backpack, m_SPlank.KeyValue) && m.Alive)
                return true;
            return false;
        }
        ...
        public enum DryDockResult { Valid, Dead, NoKey, NotAnchored, Mobiles, Items, Hold, Decaying, Damaged }
        ...
        public DryDockResult CheckDryDock(Mobile from)
        {
            ...
            List<Mobile> GhostsToMove = new List<Mobile>();
            foreach (IEntity entity in ents)
            {
                if (entity is Mobile)
                {
                    if (!((Mobile)entity).Alive)
                    {
                        GhostsToMove.Add((Mobile)entity);
                        continue;
                    }
            ...
            foreach (Mobile ghost in GhostsToMove)
            {
                Strandedness.MoveStrandedMobile(ghost);
            }
        ...
        public void BeginDryDock(Mobile from)
        {
            ...
            if (result == DryDockResult.Dead)
                ...
                from.SendMessage("A boat must be fully repaired before it can be dry-docked! You can get repair tools from a shipwright!");
        ...
        public void EndDryDock(Mobile from)
        {
            ...
            List<SmallShipCannon> cannons = new List<SmallShipCannon>();
            foreach (BoatComponent component in BoatComponents)
            {
                if (component is SmallShipCannon)
                {
                    cannons.Add((SmallShipCannon)component);
                }
            }
            foreach (SmallShipCannon cannon in cannons)
            {
                from.Backpack.DropItem(cannon.Deed);
            }

            RemoveKeys(from);

            from.Backpack.DropItem(boat); // I changed it to DropItem b/c with AddToBackpack, it could drop on the ground - Alan
            Delete();
        }

        public void ForceDryDock(Mobile from)
        {
            if (Deleted || CheckDecay())
                return;
                ...
                for (int i = 0; i < e.Keywords.Length; ++i)
                {
                    ...
                    if (keyword >= 0x42 && keyword <= 0x6B)
                    {
                        switch (keyword)
                        {
                            ...
                            case 0x46: StartMove(Backward, false); break;
                            case 0x47: StartMove(Left, false); break;
                            case 0x48: StartMove(Right, false); break;
                            case 0x4B: StartMove(ForwardLeft, true); break;
                            case 0x4C: StartMove(ForwardRight, true); break;
                            case 0x4D: StartMove(BackwardLeft, false); break;
                            case 0x4E: StartMove(BackwardRight, false); break;
        ...
        public bool Move(Direction dir, int speed, int clientSpeed, bool message)
        {
            ...
            bool glitchedDirection = (xOffset == 1 && yOffset == -1) || (xOffset == -1 && yOffset == 1);
            if (glitchedDirection || !NewBoatMovement || Math.Abs(xOffset) > 1 || Math.Abs(yOffset) > 1)
            {
            ...
            else
            {
                ...
                foreach (Item boatComponent in m_BoatComponents)
                {
                    SafeAdd(boatComponent, toMove);    
                }
                ...
                foreach (IEntity e in toMove)
                {
                    if (e is Item)
                    {
                        ...
                        if (!(item is TillerMan || item is Hold || item is Plank || item is BoatComponent)) // || m_BoatComponents.Contains(item)))
        ...
        public List<IEntity> GetMovingEntities()
        {
            ...
            {
                if (o == this || o is TillerMan || o is Hold || o is Plank || o is BoatComponent || o is BaseBoat || o is Static)
                    continue;
                ...
                if (o is Item)
                {
                    ...
                    if (item.ItemID >= 6038 && item.ItemID <= 6066) { continue; } // static water tiles
        ...
        public bool SetFacing(Direction facing)
        {
            ...
            foreach (BoatComponent component in m_BoatComponents)
            {
                component.SetFacing(facing);
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register("BoatLocationLog", AccessLevel.GameMaster, new CommandEventHandler(BoatLocationLog_Command));

            /* // Alan Mod to print out boat tiles
            for (int i = 48; i < 60; i++)
            {
                MultiData.Load(i); // 
            }
            PrintOutComponentConstructors();
             */
        }

        public static void BoatLocationLog_Command(CommandEventArgs e)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("================ " + DateTime.Now + " ================");
            foreach (BaseBoat boat in m_Instances)
            {
                if (boat == null || boat.Deleted) continue;
                sb.AppendLine(boat + "\t" + boat.Owner + "\t" + boat.Map + "\t" + boat.Location);
            }
            LoggingCustom.Log("LOG_BoatLocations.txt", sb.ToString());
            e.Mobile.SendMessage("Boat locations have been logged.");
        }

        public static void PrintOutComponentConstructors() // Alan Mod to print out code to make boat tiles
        {
            foreach (KeyValuePair<int, MultiComponentList.MultiEntry> pair in MultiComponentList.MultiEntries)
            {
                try
                {
                    using (StreamWriter writer = new StreamWriter("boatConstructors.txt", true))
                    {
                        writer.WriteLine("=========== Multi " + pair.Value.BaseMultiID + " ================");
                    }
                }
                catch (Exception e)
                {
                    Console.WriteLine("boat constructors Logging error: " + e.Message);
                }
                foreach (MultiComponentList.MultiEntryComponent comp in pair.Value.Components)
                {
                    try
                    {
                        using (StreamWriter writer = new StreamWriter("boatConstructors.txt", true))
                        {
                            if (BoatComponent.GetBoatComponentType(comp.DirectionalItemIDs[0,0]) == BoatComponentType.Rope)
                            {
                                writer.WriteLine("BoatComponents.Add(new BoatRope(this, new Point3D(" + comp.Offset.X + ", " + comp.Offset.Y + ", " + comp.Offset.Z + "), 0));");
                            }
                            //else if (BoatComponent.GetBoatComponentType(comp.DirectionalItemIDs[0,0]) == BoatComponentType.Hold)
                            else
                            {
                                writer.WriteLine("BoatComponents.Add(new BoatComponent(this, new Point3D(" + comp.Offset.X + ", " + comp.Offset.Y + ", " + comp.Offset.Z + "), new int[,] { {"
                                    + comp.DirectionalItemIDs[0, 0] + ", " + comp.DirectionalItemIDs[0, 1] + ", " + comp.DirectionalItemIDs[0,2] + ", " + comp.DirectionalItemIDs[0, 3] + ", " + "}, {"
                                    + comp.DirectionalItemIDs[1, 0] + ", " + comp.DirectionalItemIDs[1, 1] + ", " + comp.DirectionalItemIDs[1, 2] + ", " + comp.DirectionalItemIDs[1, 3] + ", " + "}, {"
                                    + comp.DirectionalItemIDs[2, 0] + ", " + comp.DirectionalItemIDs[2, 1] + ", " + comp.DirectionalItemIDs[2, 2] + ", " + comp.DirectionalItemIDs[2, 3] + "} } ));");
                            }
                        }
                    }
                    catch (Exception e)
