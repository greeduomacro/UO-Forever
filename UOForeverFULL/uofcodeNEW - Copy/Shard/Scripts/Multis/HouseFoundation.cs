...
namespace Server.Multis
{
	...
	public class HouseFoundation : BaseHouse
	{
		...
		public virtual int CustomizationCost { get { return Owner == null || Owner.EraAOS ? 0 : 10000; } }
			...
			for (int i = 0; i < m_Fixtures.Count; ++i)
			{
				...
				else if (fixture is BaseHouseDoor)
				{
					...
					for (int j = i + 1; j < m_Fixtures.Count; ++j)
					{
						...
						if (check != null && check.Link == null && check.Facing == linkFacing && (check.X - door.X) == xOffset &&
							(check.Y - door.Y) == yOffset && (check.Z == door.Z))
						{
							check.Link = door;
							door.Link = check;
							break;
		...
		public static void GetFoundationGraphics(
			FoundationType type, out int east, out int south, out int post, out int corner)
		{
			switch (type)
			{
				default:
		...
		{
			return (item == m_Signpost || item == m_SignHanger || item == Sign ||
		...
		private static readonly int[] m_BlockIDs = new[]
		{0x3EE, 0x709, 0x71E, 0x721, 0x738, 0x750, 0x76C, 0x788, 0x7A3, 0x7BA, 0x35D2, 0x3609};

		private static readonly int[] m_StairSeqs = new[] {0x3EF, 0x70A, 0x722, 0x739, 0x751, 0x76D, 0x789, 0x7A4};

		private static readonly int[] m_StairIDs = new[]
		{0x71F, 0x736, 0x737, 0x749, 0x35D4, 0x35D3, 0x35D6, 0x35D5, 0x360B, 0x360A, 0x360D, 0x360C, 0x7BB, 0x7BC};
		...
		public static void Designer_Roof(NetState state, IEntity e, EncodedReader pvSrc)
		{
			...
			if (context != null)
			{
				...
				for (int i = 0; i < list.Length; i++)
				{
					...
					if (mte.m_OffsetX == x && mte.m_OffsetY == y && GetZLevel(mte.m_OffsetZ, context.Foundation) == context.Level &&
	...
	public sealed class DesignStateDetailed : Packet
	{
			...
			: base(0xD8)
		{
			...
			if (m_PlaneBuffers == null)
			{
				...
				{
					m_PlaneBuffers[i] = new byte[2048];
				}
...
#if Framework_4_0
			Task compresstask = Task.Factory.StartNew(() => CompressionThread());
#else
					...
					try
					{
						...
						if (p == null)
						{
							p = new DesignStateDetailed(
								sqe.m_Serial, sqe.m_Revision, sqe.m_xMin, sqe.m_yMin, sqe.m_xMax, sqe.m_yMax, sqe.m_Tiles);
							p.SetStatic();
						...
						Timer.DelayCall(TimeSpan.Zero, SendPacket_Sandbox, new PacketInfo(sqe.m_NetState, p));
					}
					catch (Exception e)
					{
						Console.WriteLine(e);
		...
		public static void SendPacket_Sandbox(PacketInfo info)
		{
			info.State.Send(info.Packet);
		}
