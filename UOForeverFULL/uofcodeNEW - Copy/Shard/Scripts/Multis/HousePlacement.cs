...
namespace Server.Multis
{
	...
	public class HousePlacement
	{
		...
		public static HousePlacementResult Check( Mobile from, int multiID, Point3D center, out ArrayList toMove, bool mobsAndItemsBlock = true)
        {
             ...
             * MOD: 6) Can't place on impassable items or mobiles
             */

            Point3D testPoint = new Point3D(start.X, start.Y, center.Z);

            if (mobsAndItemsBlock)
            {
                Item blockingItem = null;
                Mobile blockingMobile = null;
                
                // exclude the porch in the test
                Rectangle2D[] testRectangles = null;
                if (multiID == 122 || multiID == 123) // large tower, don't include wings or porch
                {
                    testRectangles = new Rectangle2D[1] 
                { 
                    new Rectangle2D(new Point2D(testPoint.X + 4, testPoint.Y), new Point2D(testPoint.X + mcl.Width - 5, testPoint.Y + mcl.Height - 2))
                };
                }
                else if (multiID == 124 || multiID == 125) // stone keep, don't include outer tile (a bit more lax)
                {
                    testRectangles = new Rectangle2D[1]
                    { 
                        new Rectangle2D(new Point2D(testPoint.X + 1, testPoint.Y + 1), new Point2D(testPoint.X + mcl.Width - 2, testPoint.Y + mcl.Height - 2))
                    };
                }
                else if (multiID >= 118 && multiID <= 121) // 2 story house
                {
                    testRectangles = new Rectangle2D[2]
                    { 
                        new Rectangle2D(new Point2D(testPoint.X, testPoint.Y), new Point2D(testPoint.X + 9, testPoint.Y + 7)),
                        new Rectangle2D(new Point2D(testPoint.X, testPoint.Y + 7), new Point2D(testPoint.X + mcl.Width - 1, testPoint.Y + 14))
                    };
                }
                else
                {
                    testRectangles = new Rectangle2D[1] 
                    { 
                        new Rectangle2D(testPoint, new Point2D(testPoint.X + mcl.Width - 1, testPoint.Y + mcl.Height - 1))
                    };
                }

                foreach (Rectangle2D testRectangle in testRectangles)
                {
                    if (HouseSystemController._ItemsBlockPlacement != HousePlacementItemBlocking.None)
                    {
                        IPooledEnumerable testItems = map.GetItemsInBounds(testRectangle);
                        foreach (Item testItem in testItems)
                        {
                            if (testItem.Movable)
                            {
                                if (HouseSystemController._ItemsBlockPlacement == HousePlacementItemBlocking.Impassable)
                                {
                                    if (testItem.ItemData.Impassable)
                                    {
                                        blockingItem = testItem;
                                        break;
                                    }
                                }
                                else //  HouseSystemController._ItemsBlockPlacement == HousePlacementItemBlocking.All
                                {
                                    blockingItem = testItem;
                                    break;
                                }
                        ...
                        {
                            return HousePlacementResult.ItemBlocking;
                        }
                    }

                    if (HouseSystemController._MobsBlockPlacement)
                    {
                        IPooledEnumerable testMobs = map.GetMobilesInBounds(testRectangle);
                        foreach (Mobile testMob in testMobs)
                        {
                            if (testMob.Alive)
                            {
                                if (HouseSystemController._PetsBlockPlacement)
                                {
                                    if (testMob is BaseCreature)
                                    {
                                        BaseCreature bc = (BaseCreature)testMob;
                                        if (!bc.Controlled)
                                        {
                                            blockingMobile = testMob;
                                            break;
                                        }
                                    }
                                    else
                                ...
                                else  // pets don't block placement
                                {
                                    if (testMob is BaseCreature)
                                    {
                                        BaseCreature bc = (BaseCreature)testMob;
                        ...
                        {
                            return HousePlacementResult.MobileBlocking;
                        }
                    }
                }
            }
            ...
            for (int x = 0; x < mcl.Width; ++x)
            {
                for (int y = 0; y < mcl.Height; ++y)
                {
                    ...
                    testPoint = new Point3D(tileX, tileY, center.Z);
