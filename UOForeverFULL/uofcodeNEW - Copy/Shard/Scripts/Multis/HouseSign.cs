	...
	public class HouseSign : Item
	{
		...
		public TimeSpan TimeToDecay
		{
			get
			{
				...
				TimeSpan timeAfterRefresh = m_Owner.DecayPeriod - (DateTime.UtcNow - m_Owner.LastRefreshed);
		...
		public bool ForceRefresh
		{
			...
			set
			{
				if (value == false || m_Owner == null)
				{
					return;
				}
				m_Owner.LastRefreshed = DateTime.UtcNow;
			}
		}
		...
		public bool GettingProperties { get { return m_GettingProperties; } }
		...
		public void ShowSign(Mobile m)
		{
			if (m_Owner != null)
			{
				if (m_Owner.IsFriend(m) && m.AccessLevel < AccessLevel.GameMaster)
				{
					...
					if ((m.EraML && m_Owner.IsOwner(m)) || !m.EraML)
					{
		...
		public override void OnDoubleClick(Mobile m)
		{
			...
			if (m.AccessLevel < AccessLevel.GameMaster && m_Owner.Owner == null &&
