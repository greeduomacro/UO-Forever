		...
		public class FireFieldItem : Item
		{
			...
			public override bool OnMoveOver(Mobile m)
			{
					...
					m_Caster.CanBeHarmful(m, false))
				{
					...
					if (!EraAOS && m.CheckSkill(SkillName.MagicResist, 0.0, 30.0))
					{
						damage /= 2;
					...
					if (!m_Item.Visible)
					{
						...
						if (!m_Item.Deleted)
						{
							...
							Effects.SendLocationParticles(
								EffectItem.Create(m_Item.Location, m_Item.Map, EffectItem.DefaultDuration), 0x376A, 9, 10, 5029);
						}
					}
					...
					else
					{
						...
						if (map != null && caster != null)
						{
							...
							while (m_Queue.Count > 0)
							{
								...
								if (!m_Item.EraAOS && m.CheckSkill(SkillName.MagicResist, 0.0, 30.0))
								{
									damage = 1;
