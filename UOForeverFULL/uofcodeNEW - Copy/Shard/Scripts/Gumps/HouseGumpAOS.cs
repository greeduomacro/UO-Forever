...
namespace Server.Gumps
{
	...
	public class HouseGumpAOS : Gump
	{
		...
		private static readonly int[] m_HangerNumbers = new[] {2968, 2970, 2972, 2974, 2976, 2978};

		private static readonly int[] m_FoundationNumbers = new[] {20, 189, 765, 65, 101, 11767, 11771, 13938, 13942};

		private static readonly int[] m_PostNumbers = new[]
		{
			9, 29, 54, 90, 147, 169, 177, 204, 251, 257, 263, 298, 347, 424, 441, 466, 514, 600, 601, 602, 603, 660, 666, 672,
			898, 970, 974, 982
		};
			...
			: base(50, 40)
		{
			...
			switch (page)
			{
				case HouseGumpPageAOS.Information:
					{
						...
						AddHtmlLocalized(20, 210, 380, 20, (house is HouseFoundation) ? 1060681 : 1060680, SelectedColor, false, false);
							// This is a (pre | custom)-built house.
						AddHtmlLocalized(20, 230, 380, 20, house.Public ? 1060678 : 1060679, SelectedColor, false, false);
						...
						switch (house.DecayType)
						{
							...
							case DecayType.AutoRefresh:
								{
									AddHtmlLocalized(20, 250, 380, 20, 1062209, SelectedColor, false, false);
										// This house is <a href = "?ForceTopic97">Automatically</a> refreshed.
									break;
								}
							case DecayType.ManualRefresh:
								{
									AddHtmlLocalized(20, 250, 380, 20, 1062208, SelectedColor, false, false);
										// This house is <a href = "?ForceTopic97">Grandfathered</a>.
									break;
								}
							case DecayType.Condemned:
								{
									AddHtmlLocalized(20, 250, 380, 20, 1062207, WarningColor, false, false);
						...
						AddHtmlLocalized(20, 360, 300, 20, 1011241, SelectedColor, false, false);
				...
				case HouseGumpPageAOS.Customize:
					{
						...
						AddButtonLabeled(10, 120, GetButtonID(5, 0), 1060759, isOwner && !isCustomizable && (house.ConvertEntry != null));
						...
						AddButtonLabeled(10, 180, GetButtonID(5, 2), 1060760, isOwner && house.MovingCrate != null);
						...
						AddButtonLabeled(
							10, 250, GetButtonID(5, 5), 1060763, isOwner && isCustomizable && (((HouseFoundation)house).Signpost != null));
							// Change Signpost
				...
				case HouseGumpPageAOS.Ownership:
					{
						AddButtonLabeled(
							10, 130, GetButtonID(6, 0), 1061794, isOwner && house.MovingCrate == null && house.InternalizedVendors.Count == 0);
							// Demolish House
				...
				case HouseGumpPageAOS.ChangeSign:
					{
						...
						int signsPerPage = house.EraML ? 24 : 18;
						int totalSigns = house.EraML ? 56 : 54;
		...
		public static void ConvertHouse_Callback(Mobile from, bool okay, object state)
		{
			...
			if (okay && house.IsOwner(from) && !house.HasRentedVendors)
			{
				...
				if (e != null)
				{
					Type cType = house.Expansion == Expansion.T2A ? typeof(Silver) : typeof(Gold);
					...
					if (cost > 0)
					{
						...
						else
						{
							...
							from.SendLocalizedMessage(1061624);
							return;
						}
					}
					else if (cost < 0)
					...
					if (newHouse != null)
					{
						...
						newHouse.MoveToWorld(
							new Point3D(house.X + house.ConvertOffsetX, house.Y + house.ConvertOffsetY, house.Z + house.ConvertOffsetZ),
							house.Map);
						house.Delete();
							...
							case 12: // Make Private
								{
									if (isOwner)
									{
										...
										else if (m_House.VendorRentalContracts.Count > 0)
										{
											...
											from.SendGump(
												new NoticeGump(1060637, 30720, 1062351, 32512, 320, 180, PublicPrivateNotice_Callback, m_House));
										}
										else if (th != null && th.ForSaleSign.ForcePublic && th.Public)
										{
											from.SendGump(
												new NoticeGump(
													1060637, 30720, "This house cannot be private.", 32512, 320, 180, PublicPrivateNotice_Callback, m_House));
										}
										else
										{
							...
							case 13: // Make Public
								{
									if (isOwner)
									{
										if (th != null && th.ForSaleSign.ForcePrivate && !th.Public)
										{
											from.SendGump(
												new NoticeGump(
													1060637, 30720, "This house cannot be public.", 32512, 320, 180, PublicPrivateNotice_Callback, m_House));
										}
										else
										{
											m_House.Public = true;
											...
											if (BaseHouse.NewVendorSystem)
											{
												...
												from.SendGump(
													new NoticeGump(1060637, 30720, 501886, 32512, 320, 180, PublicPrivateNotice_Callback, m_House));
											}
											else
											{
												from.SendGump(
									...
									if (isOwner && isCustomizable)
									{
										try
										{
											foreach (AggressorInfo aggressor in from.Aggressors)
											{
												...
												{
													from.SendMessage("You cannot enter decoration mode while in combat with another player!");
													return;
												}
											}
							...
							case 0: // Demolish
								{
									if (isOwner && m_House.MovingCrate == null && m_House.InternalizedVendors.Count == 0)
									{
										...
										else if (m_House.EraML && from.AccessLevel < AccessLevel.GameMaster &&
