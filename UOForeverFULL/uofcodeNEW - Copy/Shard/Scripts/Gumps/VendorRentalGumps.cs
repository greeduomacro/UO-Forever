...
namespace Server.Gumps
{
	public abstract class BaseVendorRentalGump : Gump
	{
			...
			: base(100, 100)
		{
			...
			AddLabel(230, 115, 0x64, price > 0 ? price.ToString("#,0") : "FREE");
			...
			else if (type == GumpType.VendorLandlord || type == GumpType.VendorRenter)
			{
				...
				AddLabel(230, 250, 0x64, renewalPrice.ToString("#,0"));
	...
	public class VendorRentalContractGump : BaseVendorRentalGump
	{
		...
		{
			from.SendMessage(
				"Please enter the amount of {0} that should be charged for this contract (ESC to cancel):",
				m_Contract.Expansion == Expansion.T2A ? "Silver" : "Gold");
	...
	public class VendorRentalOfferGump : BaseVendorRentalGump
	{
		...
		protected override bool IsValidResponse(Mobile from)
		{
			return m_Contract.IsUsableBy(m_Landlord, true, false, false, false) && from.CheckAlive() &&
	...
	public class VendorRentalRefundGump : Gump
	{
			...
			: base(50, 50)
		{
			...
			AddLabel(160, 220, 0x480, refundAmount.ToString("#,0"));
