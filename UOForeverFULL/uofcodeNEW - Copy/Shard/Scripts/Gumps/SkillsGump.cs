...
{
	public class EditSkillGump : PropertiesGump
	{
	...
	public class EditSkillGump : Gump
	{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			if (info.ButtonID == 1)
			{
				try
				{
					...
					{
						TextRelay text0 = info.GetTextEntry(0);
						...
						if (text0 != null)
						{
							CommandLogging.LogChangeProperty(
								m_From,
								m_Target,
								String.Format("{0}.Base", m_Skill.Name),
								m_Skill.Base.ToString(CultureInfo.InvariantCulture),
								Convert.ToDouble(text0.Text).ToString(CultureInfo.InvariantCulture));
							m_Skill.Base = Convert.ToDouble(text0.Text);
						}
						...
						if (text1 != null)
						{
							CommandLogging.LogChangeProperty(
								m_From,
								m_Target,
								String.Format("{0}.Cap", m_Skill.Name),
								m_Skill.Cap.ToString(CultureInfo.InvariantCulture),
								Convert.ToDouble(text1.Text).ToString(CultureInfo.InvariantCulture));
							m_Skill.Cap = Convert.ToDouble(text1.Text);
						}
					}
					else
					{
			...
			: base(GumpOffsetX, GumpOffsetY)
		{
			...
			string initialText0 = m_Skill.Base.ToString("F1");
			string initialText1 = m_Skill.Cap.ToString("F1");
			...
			AddImageTiled(x, y, EntryWidth, EntryHeight, EntryGumpID);
			AddTextEntry(x + TextOffsetX, y, EntryWidth - TextOffsetX, EntryHeight, TextHue, 0, initialText0);
			x += EntryWidth + OffsetSize;
			//<-Base
			...
			AddImageTiled(x, y, EntryWidth, EntryHeight, EntryGumpID);
			AddTextEntry(x + TextOffsetX, y, EntryWidth - TextOffsetX, EntryHeight, TextHue, 1, initialText1);
			x += EntryWidth + OffsetSize;
			//<-Cap
	...
	*/
	...
	public class SkillsGump : Gump
	{
						...
						if (index >= 0 && index < m_Groups.Length)
						{
							...
							m_From.SendGump(
								m_Selected != newSelection
									? new SkillsGump(m_From, m_Target, newSelection)
									: new SkillsGump(m_From, m_Target, null));
						}
			...
			: base(GumpOffsetX, GumpOffsetY)
		{
			...
			for (int i = 0; i < m_Groups.Length; ++i)
			{
				...
				AddLabel(x + TextOffsetX, y, TextHue, group != null ? group.Name : "(null)");
				...
				for (int j = 0; j < group.Skills.Length; ++j)
				{
					...
					AddImageTiled(
						x, y, emptyWidth + (OldStyle ? OffsetSize * 2 : 0) - OffsetSize - IndentWidth, EntryHeight, EntryGumpID);
	...
	public class SkillsGumpGroup
	{
		...
		private static readonly SkillsGumpGroup[] m_Groups = new[]
		{
			new SkillsGumpGroup(
				"Crafting",
				new[]
				{
					SkillName.Alchemy, SkillName.Blacksmith, SkillName.Cartography, SkillName.Carpentry, SkillName.Cooking,
					SkillName.Fletching, SkillName.Inscribe, SkillName.Tailoring, SkillName.Tinkering, SkillName.Imbuing
				}),
			new SkillsGumpGroup(
				"Bardic", new[] {SkillName.Discordance, SkillName.Musicianship, SkillName.Peacemaking, SkillName.Provocation}),
			new SkillsGumpGroup(
				"Magical",
				new[]
				{
					SkillName.Chivalry, SkillName.EvalInt, SkillName.Magery, SkillName.MagicResist, SkillName.Meditation,
					SkillName.Necromancy, SkillName.SpiritSpeak, SkillName.Ninjitsu, SkillName.Bushido, SkillName.Spellweaving,
					SkillName.Mysticism
				}),
			new SkillsGumpGroup(
				"Miscellaneous",
				new[]
				{
					SkillName.Camping, SkillName.Fishing, SkillName.Focus, SkillName.Healing, SkillName.Herding, SkillName.Lockpicking,
					SkillName.Lumberjacking, SkillName.Mining, SkillName.Snooping, SkillName.Veterinary
				}),
			new SkillsGumpGroup(
				"Combat Ratings",
				new[]
				{
					SkillName.Archery, SkillName.Fencing, SkillName.Macing, SkillName.Parry, SkillName.Swords, SkillName.Tactics,
			...
			new SkillsGumpGroup(
				...
				new[]
				{
					SkillName.AnimalTaming, SkillName.Begging, SkillName.DetectHidden, SkillName.Hiding, SkillName.RemoveTrap,
					SkillName.Poisoning, SkillName.Stealing, SkillName.Stealth, SkillName.Tracking
				}),
			new SkillsGumpGroup(
				"Lore & Knowledge",
				new[]
				{
					SkillName.Anatomy, SkillName.AnimalLore, SkillName.ArmsLore, SkillName.Forensics, SkillName.ItemID,
					SkillName.TasteID
				})
		};
