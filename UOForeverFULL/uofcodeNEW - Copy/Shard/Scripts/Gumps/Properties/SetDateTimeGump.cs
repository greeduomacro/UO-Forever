...
namespace Server.Gumps
{
	public class SetDateTimeGump : Gump
	{
		...
		public SetDateTimeGump( PropertyInfo prop, Mobile mobile, object o, Stack stack, int page, ArrayList list ) : base( GumpOffsetX, GumpOffsetY )
		{
			m_Property = prop;
			m_Mobile = mobile;
			m_Object = o;
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 2: // From YYYY MM DD H:M
				{
					...
					string toapply = String.Format("{0}/{1}/{2} {3}:{4}:00", String.IsNullOrEmpty( Y ) ? String.Format("{0:yyyy}", m_OldDT) : Y,
					                             String.IsNullOrEmpty( M ) ? String.Format("{0:MM}", m_OldDT) : M, String.IsNullOrEmpty( D ) ? String.Format("{0:dd}", m_OldDT) : D,
					                             String.IsNullOrEmpty( h ) ? String.Format("{0:HH}", m_OldDT) : h, String.IsNullOrEmpty( m ) ? String.Format("{0:mm}", m_OldDT) : m );
