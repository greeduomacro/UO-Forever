...
namespace Server.Gumps
{
	public class RunebookGump : Gump
	{
		...
		public Runebook Book { get; private set; }
		...
		private void AddIndex()
		{
			...
			for ( int i = 0; i < 16; ++i )
			{
				...
				if ( i < entries.Count )
				{
				    ...
				    hue = entries[i].Hue == 0 ? 81 : entries[i].Hue;
				}
				else
				{
					desc = "Empty";
		...
		private class InternalPrompt : Prompt
		{
			...
			{
				if ( m_Book.Deleted || !from.InRange( m_Book.GetWorldLocation(), (m_Book.EraML ? 3 : 1) ) )
					return;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if (Book.Deleted || !from.InRange(Book.GetWorldLocation(), (Book.EraML ? 3 : 1)) || !Multis.DesignContext.Check(from))
			{
