...
namespace Server.Gumps
{
	...
	public class AdminGump : Gump
	{
		...
		public void AddPageButton(
			int x, int y, int buttonID, string text, AdminGumpPage page, params AdminGumpPage[] subPages)
		{
		...
		private static readonly string[] m_AccessLevelStrings = new[]
		{"Player", "Counselor", "Seer", "Event Master", "Game Master", "Lead", "Administrator", "Developer", "Owner"};
			...
			: base(50, 40)
		{
			...
			AddPageButton(
				10, 10, GetButtonID(0, 0), "INFORMATION", AdminGumpPage.Information_General, AdminGumpPage.Information_Perf);
			AddPageButton(
				10,
				30,
				GetButtonID(0, 1),
				...
				case AdminGumpPage.Administer_Access_Lockdown:
					{
						...
						AddSelectedButton(
							20, 260, GetButtonID(3, 504), "Administrators", (isLockedDown && level <= AccessLevel.Administrator));
						AddSelectedButton(20, 280, GetButtonID(3, 502), "Game Masters", (isLockedDown && level <= AccessLevel.GameMaster));
						AddSelectedButton(20, 300, GetButtonID(3, 503), "Seers", (isLockedDown && level <= AccessLevel.Seer));
				...
				case AdminGumpPage.Administer_Access:
					{
						...
						AddButtonLabeled(20, 310, GetButtonID(3, 312), "Make Seer");
						AddButtonLabeled(20, 330, GetButtonID(3, 313), "Make Event Master");
						AddButtonLabeled(20, 350, GetButtonID(3, 314), "Make Game Master");

						AddButtonLabeled(220, 270, GetButtonID(3, 315), "Make Lead");

						if (from.AccessLevel > AccessLevel.Administrator)
						{
							AddButtonLabeled(220, 290, GetButtonID(3, 316), "Make Administrator");

							if (from.AccessLevel > AccessLevel.Developer)
							{
								AddButtonLabeled(220, 310, GetButtonID(3, 317), "Make Developer");

								if (from.AccessLevel >= AccessLevel.Owner)
								{
									AddButtonLabeled(220, 330, GetButtonID(3, 318), "Make Owner");
								}
							}
						}
				...
				case AdminGumpPage.Administer:
					{
						...
						AddPageButton(
							200, 60, GetButtonID(3, 2), "Access", AdminGumpPage.Administer_Access, AdminGumpPage.Administer_Access_Lockdown);
				...
				case AdminGumpPage.Clients:
					{
						...
						for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
						{
							...
							AddLabelCropped(
								...
								20,
								LabelHue,
								m == null ? (a != null ? FormatAccessLevel(a.AccessLevel) : "") : FormatAccessLevel(m.AccessLevel));
				...
				case AdminGumpPage.AccountDetails_ChangeAccess:
					{
						...
						AddButtonLabeled(20, 240, GetButtonID(5, 22), "Seer");
						AddButtonLabeled(20, 260, GetButtonID(5, 23), "Game Master");
						AddButtonLabeled(20, 280, GetButtonID(5, 24), "Event Master");
						AddButtonLabeled(20, 300, GetButtonID(5, 25), "Lead");

						if (from.AccessLevel > AccessLevel.Administrator)
						{
							AddButtonLabeled(20, 320, GetButtonID(5, 35), "Administrator");

							if (from.AccessLevel > AccessLevel.Developer)
							{
								AddButtonLabeled(20, 340, GetButtonID(5, 36), "Developer");

								if (from.AccessLevel >= AccessLevel.Owner)
								{
									AddButtonLabeled(20, 360, GetButtonID(5, 37), "Owner");
								}
							}
						}
				...
				case AdminGumpPage.AccountDetails_Access:
					{
						...
						AddPageButton(
							20, 170, GetButtonID(5, 15), "Manage restrictions", AdminGumpPage.AccountDetails_Access_Restrictions);
				...
				case AdminGumpPage.AccountDetails_Access_ClientIPs:
					{
						...
						AddHtml(
							225, 315, 180, 80, Color("List of IP addresses which have accessed this account.", LabelColor32), false, false);
		...
		public static void AccountDelete_Callback(Mobile from, bool okay, object state)
		{
			...
			if (okay)
			{
				...
				CommandLogging.WriteLine(
					from, "{0} {1} deleting account {2}", from.AccessLevel, CommandLogging.Format(from), a.Username);
				a.Delete();

				from.SendGump(
					new AdminGump(
						from, AdminGumpPage.Accounts, 0, null, String.Format("{0} : The account has been deleted.", a.Username), null));
			}
			else
			{
				from.SendGump(
					new AdminGump(
						from, AdminGumpPage.AccountDetails_Information, 0, null, "You have chosen not to delete the account.", state));
			}
		}
		...
		public static void Marked_Callback(Mobile from, bool okay, object state)
		{
			...
			if (okay)
			{
				...
				for (int i = 0; i < rads.Count; ++i)
				{
					...
					{
						CommandLogging.WriteLine(
							from, "{0} {1} banning account {2}", from.AccessLevel, CommandLogging.Format(from), acct.Username);
					...
					{
						CommandLogging.WriteLine(
							from, "{0} {1} deleting account {2}", from.AccessLevel, CommandLogging.Format(from), acct.Username);
						acct.Delete();
						rads.RemoveAt(i--);
						list.Remove(acct);
					}
				...
				from.SendGump(
					new NoticeGump(
						1060637,
						30720,
						String.Format("You have {0} the account{1}.", ban ? "banned" : "deleted", rads.Count == 1 ? "" : "s"),
						0xFFC000,
						420,
						280,
						ResendGump_Callback,
			...
			else
			{
				from.SendGump(
					new NoticeGump(
						1060637,
						30720,
						String.Format("You have chosen not to {0} the account{1}.", ban ? "ban" : "delete", rads.Count == 1 ? "" : "s"),
						0xFFC000,
						420,
						280,
						ResendGump_Callback,
		...
		public static void RemoveLoginIP_Callback(Mobile from, bool okay, object state)
		{
			...
			if (okay)
			{
				...
				if (a.LoginIPs.Count != 0 && ip == a.LoginIPs[0] && AccountHandler.IPTable.ContainsKey(a.LoginIPs[0]))
				{
		...
		public static void RemoveLoginIPs_Callback(Mobile from, bool okay, object state)
		{
			...
			if (okay)
			{
				...
				if (ips.Count != 0 && AccountHandler.IPTable.ContainsKey(ips[0]))
				{
				...
				case 3:
					{
						...
						switch (index)
						{
							...
							case 312:
								...
								break;
							case 313:
								InvokeCommand("Set AccessLevel EventMaster");
								notice = "Target the player to change their access level. (Seer)";
								break;
							case 314:
								...
								break;
							case 315:
								InvokeCommand("Set AccessLevel Lead");
								notice = "Target the player to change their access level. (Lead)";
								break;
							case 316:
								{
									if (from.AccessLevel > AccessLevel.Administrator)
							...
							case 1:
								{
									...
									if (results.Count == 1)
									{
										...
										{
											from.SendGump(
												new AdminGump(from, AdminGumpPage.AccountDetails_Information, 0, null, "One match found.", state));
										}
										else
										{
									...
									else
									{
										from.SendGump(
											new AdminGump(
												...
												0,
												results,
												notice == null ? (results.Count == 0 ? "Nothing matched your search terms." : null) : notice,
												null));
									}
							...
							case 6:
								{
									...
									else
									{
										...
										else
										{
											...
											CommandLogging.WriteLine(
												from, "{0} {1} adding new account: {2}", from.AccessLevel, CommandLogging.Format(from), un);
										}
									}
							...
							case 7:
								{
									...
									{
										from.SendGump(
											new AdminGump(from, AdminGumpPage.AccountDetails_Information, 0, null, "One match found.", results[0]));
									}
									else
									{
										from.SendGump(
							...
							case 11:
								{
									...
									from.SendGump(
										new AdminGump(
											...
											m_ListPage,
											m_List,
											String.Format("The account has been {0}.", a.Banned ? "banned" : "unbanned"),
											m_State));
							...
							case 12:
								{
									...
									else
									{
										...
										CommandLogging.WriteLine(
											from, "{0} {1} changing password of account {2}", from.AccessLevel, CommandLogging.Format(from), a.Username);
									}
							...
							case 17: // ban shared
								{
									...
									if (list.Count > 0)
									{
										...
										sb.AppendFormat(
											"You are about to ban {0} account{1}. Do you wish to continue?", list.Count, list.Count != 1 ? "s" : "");
										...
										from.SendGump(
											new WarningGump(
												...
												String.Format(
													...
													a.LoginIPs.Count,
													a.LoginIPs.Count != 1 ? "s" : ""),
												0xFFC000,
												420,
												400,
												FirewallShared_Callback,
												a));
									}
									else
									{
							...
							case 37:
								{
									...
									switch (index)
									{
										...
										case 22:
											...
											break;
										case 23:
											newLevel = AccessLevel.EventMaster;
											break;
										case 24:
											newLevel = AccessLevel.GameMaster;
											break;
										case 25:
											newLevel = AccessLevel.Lead;
											break;
										case 35:
									...
									if (newLevel < from.AccessLevel || from.AccessLevel == AccessLevel.Owner)
									{
										...
										from.SendGump(
											new AdminGump(
												from, AdminGumpPage.AccountDetails_Information, 0, null, "The access level has been changed.", m_State));
									}
							...
							case 25:
								{
									...
									from.SendGump(
										new WarningGump(
											...
											30720,
											String.Format(
												"<center>Account of {0}</center><br>You are about to <em><basefont color=red>permanently delete</basefont></em> the account. Likewise, all characters on the account will be deleted, including equipped, inventory, and banked items. Any houses tied to the account will be demolished.<br><br>Do you wish to continue?",
												a.Username),
											0xFFC000,
											420,
											280,
											AccountDelete_Callback,
											m_State));
									break;
								}
									...
									else
									{
										from.SendGump(
											new WarningGump(
												...
												30720,
												String.Format(
													"You are about to clear the address list for account {0} containing {1} {2}. Do you wish to continue?",
													a,
													ips.Count,
													(ips.Count == 1) ? "entry" : "entries"),
												0xFFC000,
												420,
												280,
												RemoveLoginIPs_Callback,
												a));
									}

									break;
								}
								//35-37 used in changing access levels
							default:
								{
									index -= 50;
							...
							case 0:
								{
									...
									else if (results.Count > 1)
									{
										from.SendGump(
											new AdminGump(
												from, AdminGumpPage.Firewall, 0, results, String.Format("Search results for : {0}", match), m_State));
									}
									else
									{
										from.SendGump(
											new AdminGump(
												...
												m_ListPage,
												m_List,
												notice == null ? "Nothing matched your search terms." : notice,
												m_State));
									}
							...
							case 1:
								{
									...
									{
										from.SendGump(
											new AdminGump(from, m_PageType, m_ListPage, m_List, "You must enter an address or pattern to add.", m_State));
									}
									else if (!Utility.IsValidIP(text))
									{
										from.SendGump(
											new AdminGump(from, m_PageType, m_ListPage, m_List, "That is not a valid address or pattern.", m_State));
									}
									else
									{
										...
										CommandLogging.WriteLine(
											from, "{0} {1} firewalling {2}", from.AccessLevel, CommandLogging.Format(from), toAdd);

										Firewall.Add(toAdd);
										from.SendGump(
											new AdminGump(
												from, AdminGumpPage.FirewallInfo, 0, null, String.Format("{0} : Added to firewall.", toAdd), toAdd));
									}
							...
							case 3:
								{
									...
									{
										CommandLogging.WriteLine(
											from, "{0} {1} removing {2} from firewall list", from.AccessLevel, CommandLogging.Format(from), m_State);

										Firewall.Remove(m_State);
										from.SendGump(
											new AdminGump(
												from, AdminGumpPage.Firewall, 0, null, String.Format("{0} : Removed from firewall.", m_State), null));
									}
				...
				case 7:
					{
						...
						switch (index)
						{
							...
							case 2:
								{
									...
									{
										CommandLogging.WriteLine(
											from, "{0} {1} {2} {3}", from.AccessLevel, CommandLogging.Format(from), "kicking", CommandLogging.Format(m));
										ns.Dispose();
							...
							case 3:
								{
									...
									{
										CommandLogging.WriteLine(
											from, "{0} {1} {2} {3}", from.AccessLevel, CommandLogging.Format(from), "banning", CommandLogging.Format(m));
										a.Banned = true;
							...
							case 12:
								{
									CommandLogging.WriteLine(
										from, "{0} {1} killing {2}", from.AccessLevel, CommandLogging.Format(from), CommandLogging.Format(m));
									m.Kill();
							...
							case 13:
								{
									CommandLogging.WriteLine(
										from, "{0} {1} resurrecting {2}", from.AccessLevel, CommandLogging.Format(from), CommandLogging.Format(m));
									m.Resurrect();
							...
							if (m_PageType == AdminGumpPage.AccountDetails_Access_ClientIPs)
							{
								...
								from.SendGump(
									new WarningGump(
										1060635,
										30720,
										String.Format("You are about to remove address {0} from account {1}. Do you wish to continue?", ip, a),
										0xFFC000,
										420,
										280,
										RemoveLoginIP_Callback,
										new object[] {a, ip}));
							}
						}
		...
		private class AddCommentPrompt : Prompt
		{
			...
			public override void OnCancel(Mobile from)
			{
				from.SendGump(
					new AdminGump(
						from, AdminGumpPage.AccountDetails_Comments, 0, null, "Request to add comment was canceled.", m_Account));
			}
		...
		private class AddTagNamePrompt : Prompt
		{
			...
			{
				from.SendGump(
					new AdminGump(from, AdminGumpPage.AccountDetails_Tags, 0, null, "Request to add tag was canceled.", m_Account));
			}
