	...
	public class Ninja : BaseCreature
	{
		...
		public Ninja() : base( AIType.AI_Melee, FightMode.Aggressor, 10, 1, 0.2, 0.4 )
		{
			...
			if ( !Female )
				...
				AddItem( NotCorpseCont(( new LeatherNinjaHood()) ));

			AddItem( NotCorpseCont(( new LeatherNinjaPants())));
			AddItem( NotCorpseCont(( new LeatherNinjaBelt()))) ;
			AddItem( NotCorpseCont(( new LeatherNinjaJacket()))) ;
            AddItem(NotCorpseCont((new NinjaTabi())));
