		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( !e.Handled && InRange( e.Mobile, 3 ) )
			{
				if ( m_NewsTimer == null && e.HasKeyword( 0x30 ) ) // *news*
				{
					...
					{
						m_NewsTimer = Timer.DelayCall<NewsInfo>( TimeSpan.FromSeconds( 1.0 ), TimeSpan.FromSeconds( 3.0 ), new TimerStateCallback<NewsInfo>( ShoutNews_Callback ), new NewsInfo( tce ) );
