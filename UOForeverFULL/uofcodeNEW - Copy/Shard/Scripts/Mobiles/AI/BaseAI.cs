...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		public virtual string Name { get { return "AI"; } } //Unique Name
		public virtual int Index { get { return 0; } } //Unique ID
		...
		public BaseAI(BaseCreature m)
		{
			...
			else if (!m.PlayerRangeSensitive)
			{
				activate = true;
			}
			...
			public override void OnClick()
			{
				if (!m_Mobile.Deleted && m_Mobile.Controlled && m_From.CheckAlive())
				{
					...
					switch (m_Order)
					{
						...
						case OrderType.Unfriend:
							{
								...
								{
									if (m_Mobile.CheckControlChance(m_From, false))
									{
						...
						default:
							{
								if (m_Mobile.CheckControlChance(m_From, true))
								{
...
*/
				if ((SolenHelper.CheckRedFriendship(from) && (target is IRedSolen)) ||
					(SolenHelper.CheckBlackFriendship(from) && (target is IBlackSolen)))
				{
			...
			if (m_Mobile.CheckControlChance(from, true))
			{
		...
		public virtual void OnSpeech(SpeechEventArgs e)
		{
			...
			if (m_Mobile.Controlled && m_Mobile.Commandable)
			{
				...
				if (e.Mobile.Alive && (isOwner || isFriend))
				{
					...
					if (StringSearchFlags.Equals.Execute(speech, "all forget", true))
					{
						if (isOwner && m_Mobile.CheckControlChance(e.Mobile, true))
						{
							m_Mobile.Friends.Clear();
						}
					...
					for (int i = 0; i < keywords.Length; ++i)
					{
						...
						switch (keyword)
						{
							case 0x164: // all come
								{
									...
									if (m_Mobile.CheckControlChance(e.Mobile, true))
									{
							...
							case 0x169: // all attack
								{
									if (!isOwner || !m_Mobile.CheckControlChance(e.Mobile, false))
									{
										break;
									}
					...
					if (StringSearchFlags.EndsWith.Execute(speech, "forget", true))
					{
						if (isOwner && WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile, true))
						{
							m_Mobile.Friends.Clear();
						}
					...
					for (int i = 0; i < keywords.Length; ++i)
					{
						...
						switch (keyword)
						{
							case 0x155: // *come
								{
									...
									if (WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile, true))
									{
										m_Mobile.ControlTarget = null;
										m_Mobile.ControlOrder = OrderType.Come;
									}
							...
							case 0x156: // *drop
								{
									...
									if (!m_Mobile.IsDeadPet && !m_Mobile.Summoned && WasNamed(speech) &&
										m_Mobile.CheckControlChance(e.Mobile, true))
									{
										m_Mobile.ControlTarget = null;
							...
							case 0x15A: // *follow
								{
									if (WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile, false))
									{
										BeginPickTarget(e.Mobile, OrderType.Follow);
									}
								}
							...
							case 0x15B: // *friend
								{
									...
									if (WasNamed(speech))
									{
										...
										else if (m_Mobile.CheckControlChance(e.Mobile, false))
										{
							...
							case 0x15C: // *guard
								{
									...
									if (!m_Mobile.IsDeadPet && WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile, true))
									{
										m_Mobile.ControlTarget = null;
										m_Mobile.ControlOrder = OrderType.Guard;
									}
							...
							case 0x15E: // *attack
								{
									...
									if (!m_Mobile.IsDeadPet && WasNamed(speech) && m_Mobile.CheckControlChance(e.Mobile, false))
									{
										BeginPickTarget(e.Mobile, OrderType.Attack);
									}
								}
		...
		public virtual void OnActionChanged()
		{
			...
			//Try to maintain a fast pace when following.
			if (m_Mobile.ControlMaster is PlayerMobile &&
				(m_Mobile.ControlOrder == OrderType.Come || m_Mobile.ControlOrder == OrderType.Follow) &&
		...
		public virtual bool DoOrderCome()
		{
			if (m_Mobile.ControlMaster != null && !m_Mobile.ControlMaster.Deleted)
			{
				...
				else
				{
					...
					if (WalkMobileRange(m_Mobile.ControlMaster, 1, bRun, 1, 1))
					{
						if (m_Mobile.Combatant != null && !m_Mobile.Combatant.Deleted && m_Mobile.Combatant.Alive &&
							!m_Mobile.Combatant.IsDeadBondedPet)
						{
		...
		public virtual bool CheckHerding()
		{
			...
			if (distance < 2 || distance > 30)
			{
				...
				// reset speed to passive speed if in wander mode (having a TargetLocation changes their speed to ActiveSpeed)
				if (m_Mobile.AIObject != null && m_Mobile.AIObject.Action == ActionType.Wander)
				{
					m_Mobile.CurrentSpeed = m_Mobile.PassiveSpeed;
				}
				if (!m_Mobile.Controlled && m_Mobile.FightMode != FightMode.Evil)
				{
		...
		public virtual bool DoOrderFollow()
		{
			if (CheckHerding())
			{
				m_Mobile.DebugSay("Praise the shepherd!");
					 ...
					 (m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget == m_Mobile.ControlMaster) &&
		...
		public virtual bool DoOrderGuard()
		{
			...
			if (mastercombatant != null && !mastercombatant.Deleted &&
				mastercombatant.GetDistanceToSqrt(m_Mobile) <= m_Mobile.RangePerception)
			{
				if (combatant == null ||
					mastercombatant.GetDistanceToSqrt(controlMaster) < combatant.GetDistanceToSqrt(controlMaster))
				{
				...
				m_Mobile.CanBeHarmful(combatant, false) && combatant.Map == m_Mobile.Map)
			{
			    ...
			    if (combatant != m_Mobile.Combatant && combatant is PlayerMobile && m_Mobile.CanBreath)
			    {
			        if (m_Mobile.NextBreathTime == DateTime.UtcNow)
			        {
			            m_Mobile.NextBreathTime += TimeSpan.FromSeconds(Utility.Random(1, 4));
			        }
			    }
			...
			else
			{
				...
				WalkMobileRange(controlMaster, 1, false, 1, 1);
			}
		...
		public virtual bool DoOrderAttack()
		{
				...
				!m_Mobile.InRange(m_Mobile.ControlTarget, m_Mobile.RangePerception * 2))
			{
				...
				if (m_Mobile.FightMode == FightMode.Closest || m_Mobile.FightMode == FightMode.Aggressor)
				{
					...
					{
                        if (newCombatant != m_Mobile.Combatant && newCombatant is PlayerMobile && m_Mobile.CanBreath)
                        {
                            if (m_Mobile.NextBreathTime == DateTime.UtcNow)
                            {
                                m_Mobile.NextBreathTime += TimeSpan.FromSeconds(Utility.Random(1, 4));
		...
		public virtual bool DoOrderRelease()
		{
		    ...
		    if (m_Mobile is MetaDragon)
		    {
                m_Mobile.Say("*Turns back into a dragon egg*");
		        var egg = new EvolutionEgg(m_Mobile.Hue);
		        if (m_Mobile.ControlMaster.Backpack != null)
		        {
		            m_Mobile.ControlMaster.Backpack.DropItem(egg);
		        }
		        else
		        {
			...
			public override void OnSecureTrade(Mobile from, Mobile to, Mobile newOwner, bool accepted)
			{
				...
				if (accepted)
				{
					if (m_Creature.SetControlMaster(to))
					{
                        ...
                        if (m_Creature is BaseMetaPet)
                        {
                            m_Creature.IsBonded = true;
                            var metapet = m_Creature as BaseMetaPet;
                            metapet.EvolutionPoints = 0;
                        }
		...
		public virtual bool DoOrderTransfer()
		{
			...
			if (from != to && from != null && !from.Deleted && to != null && !to.Deleted && to.Player)
			{
				...
				else if (m_Mobile is BaseHire && !BaseHire.CanHire(to))
				{
					from.SendMessage("You may not transfer a hireling to someone who already has a hireling.");
					to.SendMessage("The hireling cannot be transferred because you already have a hireling.");
				}
				else
				{
					NetState fromState = from.NetState, toState = to.NetState;
		...
		public virtual bool DoBardPacified()
		{
			if (DateTime.UtcNow > m_Mobile.BardEndTime || m_Mobile.BardMaster == null || m_Mobile.BardMaster.Deleted ||
				!m_Mobile.BardMaster.Alive ||
				(m_Mobile.BardMaster.Hidden && m_Mobile.BardMaster.Skills[SkillName.Hiding].Value < 80.0) ||
				m_Mobile.BardMaster.Map != m_Mobile.Map ||
				m_Mobile.GetDistanceToSqrt(m_Mobile.BardMaster) >
				BaseInstrument.GetBardRange(m_Mobile.BardMaster, SkillName.Peacemaking))
			{
				m_Mobile.DebugSay("I am no longer pacified");
		...
		{
			if (m_Mobile.BardMaster == null || m_Mobile.BardMaster.Deleted || !m_Mobile.BardMaster.Alive ||
				(m_Mobile.BardMaster.Hidden && m_Mobile.BardMaster.Skills[SkillName.Hiding].Value < 80.0) ||
				m_Mobile.BardMaster.Map != m_Mobile.Map ||
				m_Mobile.GetDistanceToSqrt(m_Mobile.BardMaster) >
				BaseInstrument.GetBardRange(m_Mobile.BardMaster, SkillName.Provocation))
			{
			...
			else if (m_Mobile.BardTarget == null || m_Mobile.BardTarget.Deleted || !m_Mobile.BardTarget.Alive ||
		...
		public virtual MoveResult DoMoveImpl(Direction d)
		{
			...
			// NOTE: I change this to true later because either 1 of 2 scenarios happens:
			// 1) the mob breaks them
			// 2) if the top of the item is below the mob or the bottom of the item is above the top of the mob
			//      (which could happen on a steep incline, for example)
			//    then the mob will walk through it without breaking it.  The issue here is that
			//    if it is above or below the mob then it doesn't check whether he breaks it...
			//    we COULD check the destination z twice to fix this, but normally it's not checked at the
			//    early stages (because it has to do quite a few calculations), and without the
			//    destination z information, we just do a simple z check for above or below the monster
			//    (obviously if the monster is in a building, we don't want him breaking things on a floor above or below)
			//    Again, this will only be an issue on steep inclines.
		            ...
		            if (canOpenDoors || canDestroyObstacles)
		            {
		                ...
		                if (map != null)
		                {
		                    ...
		                    foreach (Item item in eable)
		                    {
		                        ...
		                        {
		                            // don't use this because it will destroy more stuff than
		                            // just the square they walk towards
		                    ...
		                    {
		                        var item = (Item) m_Obstacles.Dequeue();
		                        ...
		                        {
		                            m_Mobile.DebugSay("I can't break this because I'm tamed!");
		                        }
		                        else if (m_Mobile.AI == AIType.AI_Animal)
		                        {
		                            m_Mobile.DebugSay("I can't break this because I'm an animal!");
		                        }
		                    ...
		                    {
		                        // just let the mob walk through it (as described above)
		                        MoveImpl.IgnoreMovableImpassables = true;
		                        if (m_Mobile.Move(d))
		                        {
		                            blocked = false;
		                        }
		...
		{
			if (m_Mobile.Deleted || m_Mobile.DisallowAllMoves || m_Mobile.CantWalk)
			{
				return;
			}
		...
		public virtual bool AcquireFocusMob(int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe)
		{
			...
			if (map != null)
			{
				...
				foreach (Mobile m in eable)
				{
					...
					bool customTeams = m_Mobile.CustomTeam && m.CustomTeam;

					// Does it have to be a player?
					if (bPlayerOnly && !m.Player && !customTeams)
					{
						continue;
					}
					...
					if (customTeams)
					{
						List<XmlTeam> fromTeams = XmlAttach.GetTeams(m_Mobile);
						List<XmlTeam> targetTeams = XmlAttach.GetTeams(m);
					...
					if (customTeams) // Alan mod
					{
						theirVal = m_Mobile.GetFightModeRanking(m, acqType, false);
					}
					else
					{
		...
		public virtual void Deactivate()
		{
			if (m_Mobile.PlayerRangeSensitive)
			{
				...
				if (XmlScript.HasTrigger(m_Mobile, TriggerName.onDeactivate))
				{
					UberScriptTriggers.Trigger(m_Mobile, m_Mobile, TriggerName.onDeactivate);
				}
		...
		private void ReturnToHome()
		{
			...
			var sp = m_Mobile.Spawner as Spawner;

			if (sp != null)
			{
				Point3D loc = sp.GetSpawnPosition(m_Mobile, sp.Map);
		...
		public virtual void Activate()
		{
			if (!m_Timer.Running)
			{
				...
				m_Timer.Start();
				if (XmlScript.HasTrigger(m_Mobile, TriggerName.onActivate))
				{
					UberScriptTriggers.Trigger(m_Mobile, m_Mobile, TriggerName.onActivate);
				}
			}
		}
		...
		private class AITimer : Timer
		{
			...
			public AITimer(BaseAI owner)
				: base(
					TimeSpan.FromSeconds(Utility.RandomDouble()), TimeSpan.FromSeconds(Math.Max(0.005, owner.m_Mobile.CurrentSpeed)))
			{
				m_Owner = owner;
			...
			{
				if (!m_Owner.m_Mobile.Deleted && m_Owner.m_Mobile.NetState != null)
				{
					return; // pseudoseer controlled, don't use AI
				}
