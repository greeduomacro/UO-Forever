...
namespace Server.Mobiles
{
	public class MageAI : BaseAI
	{
		...
		{
			get { return ( m_Mobile is BaseVendor || m_Mobile is BaseEscortable || m_Mobile is Changeling || m_Mobile is DaemonZombieEvent); }
		}

		public virtual double HealChance{ get{ return 0.005; } }
		public virtual double TeleportChance{ get{ return 0.0; } }
		public virtual double DispelChance{ get{ return 0.20; } }
		public virtual double RevealChance{ get{ return 0.005; } }
		public virtual double InvisChance{ get{ return 0.005; } }
			...
			if ( !SmartAI )
			{
				...
				switch (Utility.Random(16))
				{
					...
					case 5:	// Paralyze them
						{
							if (m_Mobile.Controlled || m_Mobile.Skills[ SkillName.Magery ].Value <= 50.0)
								goto default;
...
*/
		...
		public override bool DoActionCombat()
		{
			...
			if ( m_Mobile.Spell == null && DateTime.UtcNow > m_NextCastTime && m_Mobile.InRange( c, m_Mobile.EraML ? 10 : 12 ) )
			{
				...
				if ( SmartAI && toDispel != null )
				{
					if ( m_Mobile.InRange( toDispel, 10 ) )
						RunFrom( toDispel );
					else if ( !m_Mobile.InRange( toDispel, m_Mobile.EraML ? 10 : 12 ) )
						RunTo( toDispel );
				}
				else
				{
		...
		public override bool DoActionGuard()
		{
            if (m_Mobile.ForceWaypoint && (m_Mobile.CurrentWayPoint != null || m_Mobile.TargetLocation != null))
            {
                m_Mobile.DebugSay("I'm guarding but must go to my waypoint!");
                Action = ActionType.Wander;
                return true;
            }
            ...
            if( m_LastTarget != null && m_LastTarget.Hidden )
			{
				...
				if( map == null || !m_Mobile.InRange( m_LastTargetLoc, m_Mobile.EraML ? 10 : 12 ) )
				{
					m_LastTarget = null;
				}
				/*
		...
		public override bool DoActionFlee()
		{
            ...
            if (m_Mobile.ForceWaypoint && (m_Mobile.CurrentWayPoint != null || m_Mobile.TargetLocation != null))
            {
                m_Mobile.DebugSay("I'm fleeing but must go to my waypoint!");
                Action = ActionType.Wander;
                return true;
            }
		...
		public Mobile FindDispelTarget( bool activeOnly )
		{
			...
			if ( activeOnly )
			{
				...
				if ( comb != null && !comb.Deleted && comb.Alive && !comb.IsDeadBondedPet && m_Mobile.InRange( comb, m_Mobile.EraML ? 10 : 12 ) && CanDispel( comb ) )
				{
					active = comb;
				...
				for ( int i = 0; i < aggressed.Count; ++i )
				{
					...
					if ( m != comb && m.Combatant == m_Mobile && m_Mobile.InRange( m, m_Mobile.EraML ? 10 : 12 ) && CanDispel( m ) )
					{
			...
			else
			{
				...
				if ( map != null )
				{
					...
					foreach ( Mobile m in m_Mobile.GetMobilesInRange( m_Mobile.EraML ? 10 : 12 ) )
					{
