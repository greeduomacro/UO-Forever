...
namespace Server.Mobiles
{
    public class Revenant : BaseCreature
    {
        ...
        {
            if (m_Target == null || !m_Target.Alive || DateTime.UtcNow > m_ExpireTime)
            {
                Kill();
                return;
            }
            if (Map != m_Target.Map || !InRange(m_Target, 15))
            {
                ...
                Effects.SendLocationParticles(EffectItem.Create(@from, fromMap, EffectItem.DefaultDuration), 0x3728, 1,
                    13, 37, 7, 5023, 0);
            ...
            if (m_Target.Hidden && InRange(m_Target, 3) && DateTime.UtcNow >= NextSkillTime &&
        ...
        public override bool OnBeforeDeath()
        {
            ...
            Effects.SendLocationParticles(EffectItem.Create(Location, Map, TimeSpan.FromSeconds(10.0)), 0x37CC, 1, 50,
                2101, 7, 9909, 0);
