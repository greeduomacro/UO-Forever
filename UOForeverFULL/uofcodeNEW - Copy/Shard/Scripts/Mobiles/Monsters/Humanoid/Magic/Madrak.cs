...
namespace Server.Mobiles
{
    ...
    public class Madrak : BaseChampion
    {
        ...
        [Constructable]
        public Madrak() : base(AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "Madrak";
            SpecialTitle = "The Warlock King";
            TitleHue = 1174;
            Body = 830;
            BaseSoundID = 412;
        ...
        public override void GiveSpecialItems(List<Mobile> eligibleMobs, List<double> eligibleMobScores, double totalScores)
        {
            ...
            if (0.3 > Utility.RandomDouble())
            {
                for (int i = 0; i < eligibleMobScores.Count; i++)
                {
                    ...
                    CrackedCrystalBall crystalball = new CrackedCrystalBall();
                    if (eligibleMobs[i] is PlayerMobile && eligibleMobs[i].Backpack != null)
                    {
                        eligibleMobs[i].Backpack.DropItem(crystalball);
                        eligibleMobs[i].SendMessage(54, "You have received a cracked crystal ball!");
                        return;
                    }
                }
            }
