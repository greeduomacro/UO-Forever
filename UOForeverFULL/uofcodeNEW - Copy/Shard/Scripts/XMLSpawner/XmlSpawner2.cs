	...
	public class XmlSpawner : Item, ISpawner
	{
        ...
        */
		...
		public TimeSpan TOD
		{
			get
			{
				if (m_TODMode == TODModeType.Gametime)
				{
					...
					Clock.GetTime(Map, Location.X, Location.Y, out hours, out minutes);
					return
						(new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, DateTime.UtcNow.Day, hours, minutes, 0).TimeOfDay);
				}
				else
				{
						...
						if ((SpawnerPrefix == null) || (SpawnerPrefix.Length == 0) || SpawnName.StartsWith(SpawnerPrefix))
						{
							...
							else if (questionable_spawner)
							{
								...
								try
								{
									...
									{
										op.WriteLine(
											"# Questionable spawner : {0}: Format: X Y Z Map SpawnerName Fileposition Xmlfile", DateTime.UtcNow);
										op.WriteLine(
											"{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}",
											SpawnCentreX,
											SpawnCentreY,
						...
						if (keyvalueargs.Length > 0)
						{
							switch (keyvalueargs[0])
							{
								...
								case "#XY":
									...
									break;
								case "#RECT":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.Rectangle, m_mob_who_triggered, keyvalueargs));
									break;
								case "#RANDOMDUNGEON":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.RandomDungeon, m_mob_who_triggered, keyvalueargs));
									break;
								case "#CONDITION":
				...
				foreach (SpawnPositionInfo s in spawnpositioning)
				{
					...
					switch (s.positionType)
					{
						...
						case SpawnPositionType.Perimeter:
							...
							break;
						case SpawnPositionType.RandomDungeon:
							// INCLUSIVE x & y
							// syntax RANDOMDUNGEON
							positioning = s.positionType;
							...
							break;
						case SpawnPositionType.Rectangle:
							// INCLUSIVE x & y
							// syntax RECT,xstart,ystart,xend,yend
							positioning = s.positionType;
							if (positionargs != null && positionargs.Length > 4)
							{
								try
								{
									xinc = int.Parse(positionargs[1]);
									yinc = int.Parse(positionargs[2]);
									xend = int.Parse(positionargs[3]);
									yend = int.Parse(positionargs[4]);
									if (xend < xinc)
									{
										xend = xinc;
									}
			...
			for (int i = 0; i < 10; i++)
			{
				...
				else
				{
					switch (positioning)
					{
						...
						case SpawnPositionType.Rectangle:
						case SpawnPositionType.RandomDungeon:
							x = Utility.RandomMinMax(xinc, xend);
							y = Utility.RandomMinMax(yinc, yend);
							defaultZ = zinc;
							break;
						case SpawnPositionType.RelXY:
						...
						case SpawnPositionType.DeltaLocation:
							...
							if (!requiresurface)
							{
								return new Point3D(xinc, yinc, zinc); // i added this to fix that problem
							}
							break;
						case SpawnPositionType.Location:
							// Still checks whether it "Map.CanFit", which seems silly when location is specified using #DX,x,y[,z].
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			// Verision 31
			writer.Write((int)m_BasePriority);
