...
namespace Server.Mobiles
{
	public class TalkingBaseCreature : BaseCreature
	{
		...
		public int EItemID { get { return m_EItemID; } set { m_EItemID = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
		public Point3D EOffset { get { return m_Offset; } set { m_Offset = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
		public int EDuration { get { return m_Duration; } set { m_Duration = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
		public int EHue { get { return m_EHue; } set { m_EHue = value; } }
		...
		public static void SendOffsetTargetEffect(
			IEntity target, Point3D loc, int itemID, int speed, int duration, int hue, int renderMode)
		{
			if (target is Mobile)
			{
		...
		public static void Configure()
		{
			EventSink.ServerStarted += () =>
			{
				...
				foreach (TalkingBaseCreature m in World.Mobiles.Values.OfType<TalkingBaseCreature>())
				{
					m.DialogAttachment = XmlAttach.FindAttachment(m, typeof(XmlDialog)) as XmlDialog;
				}
			};
		}
