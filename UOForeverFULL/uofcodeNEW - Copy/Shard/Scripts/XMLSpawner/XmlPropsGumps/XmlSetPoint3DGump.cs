	...
	public class XmlSetPoint3DGump : Gump
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( p != null )
				{
					try
					{
                        CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, m_Property.GetValue(m_Object, null).ToString(), new Point3D(p).ToString());
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( shouldSet )
			{
				try
				{
                    CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, m_Property.GetValue(m_Object, null).ToString(), toSet.ToString());
