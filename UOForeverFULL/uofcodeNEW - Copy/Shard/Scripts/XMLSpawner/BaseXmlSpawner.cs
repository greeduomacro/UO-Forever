...
namespace Server.Mobiles
{
	...
	public class BaseXmlSpawner
	{
		...
		private static readonly Type[] m_ParseTypes = new[] {typeof(string)};
		...
		public class KeywordTag
		{
			...
			public KeywordTag(
				string typename, XmlSpawner spawner, TimeSpan delay, TimeSpan timeout, string condition, int gotogroup)
				: this(typename, spawner, 0, delay, timeout, condition, gotogroup)
			{ }

			public KeywordTag(
				string typename, XmlSpawner spawner, int type, TimeSpan delay, TimeSpan timeout, string condition, int gotogroup)
			{
				Type = type;
				m_Delay = delay;
				m_Timeout = timeout;
			...
			private class KeywordTimer : Timer
			{
				...
				public KeywordTimer(
					XmlSpawner spawner, KeywordTag tag, TimeSpan delay, TimeSpan repeatdelay, string condition, int gotogroup)
					: base(delay)
				{
		...
		public static string InternalSetValue(Mobile from, object o, PropertyInfo p, string value, bool shouldLog, int index)
		{
			...
			try
			{
				if (shouldLog)
				{
					CommandLogging.LogChangeProperty(from, o, p.Name, p.GetValue(o, null).ToString(), value);
				}
		...
		public static string SetPropertyValue(XmlSpawner spawner, object o, string name, string value)
		{
			...
			if (keywordargs[0] == "ATTACHMENT")
			{
				...
				ArrayList attachments = XmlAttach.FindAttachments(o as IEntity, attachtype, aname);
		...
		public static string ApplyToProperty(
			XmlSpawner spawner, object getobject, object setobject, string getpropertystring, string setpropertystring)
		{
			Type ptype;
			...
			XmlSpawner spawner, string str, object o, Mobile trigmob, object refobject, out string status_str)
		{
			...
			while (arglist.Length > 1)
			{
				...
				if (arglist.Length > 1)
				{
					...
					if (value_keywordargs[0].Length > 0 && !Char.IsUpper(value_keywordargs[0][0]) && arglist[0].Length > 0 &&
						...
						//else
						{
							...
							if (singlearglist[1] != null && singlearglist[1].Length > 0 && singlearglist[1][0] == '@')
							{
								...
								string result = SetPropertyValue(
									spawner, o, arglist[0], CompleteEscapedCharacters(lstr.Remove(0, 1), escapedCharacters));
						...
						if (IsValuemodKeyword(value_keywordargs[0]))
						{
								...
								if (value_keywordargs.Length > 2)
								{
									...
									try
									{
										randvalue = String.Format(
											"{0}", Utility.RandomMinMax(int.Parse(value_keywordargs[1]), int.Parse(value_keywordargs[2])));
									}
									catch
									{
							...
							else if (kw == valuemodKeyword.GETVAR)
							{
								...
								if (value_keywordargs.Length > 1)
								{
									...
									XmlLocalVariable var =
										(XmlLocalVariable)XmlAttach.FindAttachment(refobject as IEntity, typeof(XmlLocalVariable), varname);
								...
								if (value_keywordargs.Length > 3)
								{
									...
									if (no_error)
									{
										...
										ArrayList nearbylist = GetNearbyObjects(
											relativeto, targetname, targettype, typestr, range, searchcontainers, null);
							...
							else if (kw == valuemodKeyword.MOB)
							{
								...
								if (value_keywordargs.Length > 1)
								{
									...
									try
									{
										mob_id = FindMobileByName(spawner, value_keywordargs[1], typestr);
						...
						else if (IsTypemodKeyword(keywordargs[0]))
						{
							...
							else if (kw == typemodKeyword.MEFFECT)
							{
								...
								if (effect >= 0 && hasloc && emap != Map.Internal)
								{
									Effects.SendPacket(
										eloc1,
										emap,
										new HuedEffect(EffectType.Moving, -1, -1, effect, eloc1, eloc2, speed, duration, false, false, 0, 0));
								}
							...
							else if (kw == typemodKeyword.ADD)
							{
								no_error = AddItemToTarget(
									spawner, o, keywordargs, arglist, trigmob, refobject, false, out remainder, out status_str);
							}
							else if (kw == typemodKeyword.EQUIP)
							{
								no_error = AddItemToTarget(
									spawner, o, keywordargs, arglist, trigmob, refobject, true, out remainder, out status_str);
							}
							...
							else if (kw == typemodKeyword.ATTACH)
							{
								no_error = AddAttachmentToTarget(
									spawner, o, keywordargs, arglist, trigmob, refobject, out remainder, out status_str);
							}
							...
							else if (kw == typemodKeyword.SENDMSG)
							{
								...
								if (o is Mobile)
								{
									...
									{
										((Mobile)o).Send(
											new UnicodeMessage(Serial.MinusOne, -1, MessageType.Regular, hue, font, "ENU", "System", msgstr));
									}
								}
								if (arglist.Length < 3)
								{
		...
		public static bool TestMobProperty(
			XmlSpawner spawner, Mobile mobile, string testString, Mobile trigmob, out string status_str)
		{
			status_str = null;
		...
		public static bool TestItemProperty(
			XmlSpawner spawner, Item ObjectPropertyItem, string testString, Mobile trigmob, out string status_str)
		{
			status_str = null;
				...
				if (startc == '"' || startc == '(')
				{
					...
					else if (IsValueKeyword(pname))
					{
						...
						else if ((kw == valueKeyword.GETVAR) && arglist.Length > 1)
						{
							...
							// look for the xmllocalvariable attachment with the given name
							XmlLocalVariable var =
								(XmlLocalVariable)XmlAttach.FindAttachment(o as IEntity, typeof(XmlLocalVariable), varname);
		...
		public static bool CheckSubstitutedPropertyString(
			XmlSpawner spawner, object o, string testString, Mobile trigmob, out string status_str)
		{
		...
		public static void ExecuteActions(Mobile trigmob, object target, string actions)
		{
			...
			string escapedCharacters = "";
			actions = ParseEscapedCharacters(actions, out escapedCharacters, ";");
			...
			for (int j = 0; j < args.Length; j++)
			{
				...
				if (arg.Trim().StartsWith("#CONDITION"))
				{
					...
					arg = ParseEscapedCharacters(arg, out escapedCharacters, ";");
					var splitConditionArgs = arg.Split(';');
					var ckeyvalueargs = ParseCommaArgs(splitConditionArgs[0], 2);

					string status_str;
					if (splitConditionArgs.Length >= 2 || ckeyvalueargs.Length > 1)
					{
						// dont spawn if it fails the test
						if (CheckPropertyString(null, target, ckeyvalueargs[1], trigmob, out status_str))
						{
					...
					if (status_str != null)
					{
						trigmob.SendMessage(status_str);
					}
				}
				else
				{
		...
		public static bool CheckCondition(string condition, Mobile trigmob, object target)
		{
			...
			if (condition != null && condition.Length > 0)
			{
				...
				return CheckPropertyString(null, target, condition, trigmob, out status_str);
			}
			...
			XmlSpawner spawner, object o, string testString, Mobile trigmob, out string status_str)
		{
			...
			testString = ApplySubstitution(spawner, o, trigmob, testString);
		...
		private static void GetItemsIn(
			Item source, string targetname, Type targettype, string typestr, ref ArrayList nearbylist, string proptest)
		{
			string status_str;
			if (source != null && source.Items != null && nearbylist != null)
			{
				foreach (Item i in source.Items)
				{
					...
					if (!i.Deleted && CheckNameMatch(targetname, i.Name) &&
						(typestr == null ||
		...
		private static ArrayList GetNearbyObjects(
			object invoker, string targetname, Type targettype, string typestr, int range, bool searchcontainers, string proptest)
		{
			...
			if (targettype == null || targettype == typeof(Item) || targettype.IsSubclassOf(typeof(Item)))
			{
				...
				if (itemlist != null)
				{
					foreach (Item i in itemlist)
					{
						...
						else if (!i.Deleted && CheckNameMatch(targetname, i.Name) &&
								 (typestr == null ||
								  (itemtype != null && targettype != null && (itemtype.Equals(targettype) || itemtype.IsSubclassOf(targettype)))))
						{
							if (proptest == null || CheckPropertyString(null, i, proptest, null, out status_str))
			...
			if (targettype == null || targettype == typeof(Mobile) || targettype.IsSubclassOf(typeof(Mobile)))
			{
				...
				if (mobilelist != null)
				{
					foreach (Mobile m in mobilelist)
					{
						...
						if (!m.Deleted && CheckNameMatch(targetname, m.Name) &&
							(typestr == null ||
		...
		public static Item SearchMobileForItem(
			Mobile m, string targetName, string typeStr, bool searchbank, bool equippedonly)
		{
			if (m != null && !m.Deleted)
			{
		...
		public static Item FindInRecentItemSearchList(XmlSpawner spawner, string name, string typestr)
		{
			...
			foreach (Item item in spawner.RecentItemSearchList)
			{
				...
				{
					if (typestr == null ||
						(item.GetType() != null && targettype != null &&
							...
							case itemKeyword.JEWELRY:
								{
									...
									if (itemkeywordargs.Length == 3)
									{
										...
										Item item = MagicJewelry(trigmob.Expansion, min, max);
										if (item != null)
										{
											if (equip && m != null)
											{
							...
							case itemKeyword.SHIELD:
								{
									...
									if (itemkeywordargs.Length == 3)
									{
										...
										Item item = MagicShield(trigmob.Expansion, min, max);
										if (item != null)
										{
											if (equip && m != null)
											{
		...
		public static string[] ParseToMatchingParen(string str, char opendelim, char closedelim)
		{
			...
			for (int i = 0; i < remaining.Length; i++)
			{
				...
				if (remaining[i] == opendelim)
				{
					nopen++;
				}
				if (remaining[i] == closedelim)
				{
					nclose++;
				}
		...
		public static void PublicOverheadMobileMessage(
			Mobile mob, MessageType type, int hue, int font, string text, bool noLineOfSight)
		{
		...
		public static Item GetTaken(object o)
		{
			...
			XmlSaveItem si = (XmlSaveItem)XmlAttach.FindAttachment(o as IEntity, typeof(XmlSaveItem), "Taken");
		...
		{
			// find the XmlSaveItem attachment
			XmlSaveItem si = (XmlSaveItem)XmlAttach.FindAttachment(o as IEntity, typeof(XmlSaveItem), "Given");
		...
		public static Item MagicJewelry(Expansion e, int minLevel, int maxLevel)
		{
		...
		public static Item MagicShield(Expansion e, int minLevel, int maxLevel)
		{
			BaseCreature.Cap(ref minLevel, 0, 5);
			BaseCreature.Cap(ref maxLevel, 0, 5);
			if (e >= Expansion.AOS)
							...
							foreach (Mobile p in rangelist)
							{
								if (!petres && p is PlayerMobile && p.Body.IsGhost)
								{
									p.Resurrect();
										 ...
										 ((BaseCreature)p).Controlled && ((BaseCreature)p).IsDeadPet)
								{
		...
		public static void ApplyDamage(string arglist, Mobile triggermob, object refobject, out string status_str)
		{
			...
			if (str.Length > 4)
			{
				if (str[4].ToLower() == "playeronly")
				{
					playeronly = true;
				}
				else if (str[4].ToLower() == "mobsonly")
				{
					mobsonly = true;
				}
			}
			...
			try
			{
				if (range >= 0 || (triggermob != null && !triggermob.Deleted))
				{
						...
						if (rangelist != null)
						{
							foreach (Mobile p in rangelist)
							{
								if ((p is PlayerMobile && !mobsonly) || (p is BaseCreature && !playeronly))
								{
									...
									SpellHelper.Damage(TimeSpan.FromTicks(1), p, p, Utility.RandomMinMax(damagemin, damagemax));
								}
							}
							rangelist.Free();
						}
					...
					else
					{
						...
						{
							SpellHelper.Damage(TimeSpan.FromTicks(1), triggermob, triggermob, Utility.RandomMinMax(damagemin, damagemax));
						}
					}
				}
			}
		...
		public static void ApplyRewardToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
		{
			...
			if (str.Length > 6)
			{
				...
				if (str.Length > 7)
				{
					...
					{
						status_str = "bad probability arg in REWARD";
					}
				}
			}
			try
			...
			{
				status_str = "bad platinumAmount arg in REWARD";
			}
				...
				if (sortedValues.Count > 0 && sortedValues[0].Value != 0)
				{
					...
					if (rare)
					{
						...
						if (owner != null && !owner.Deleted)
						{
							...
							{
								owner.SendMessage(
									38, "Although you were #1 in the " + eventname + ", you didn't get a special reward this time. So sorry!");
							}
						}
					}
				}
		...
		private static void AwardPlatinum(bool putInBank, Mobile mob, int platinumAmount, string eventname)
		{
			if (platinumAmount == 0)
			{
				mob.SendMessage(
					38,
					"Sorry, but you did not acquire enough points to gain any platinum for the " +
			...
			mob.SendMessage(
				38,
				"You have been awarded " + platinumAmount + " platinum coin(s) for your participation in the " +
				(eventname != null ? eventname : "event") + "! " + (putInBank ? "It has been placed into your bank." : ""));
		}

		private static void LogReward(XmlValue value, int platinumAmount, string eventname)
		{
			if (value == null || value.Owner == null || !(value.Owner is Mobile))
			{
				return;
			}
			...
			out string status_str)
		{
					...
					case typeKeyword.SETVAR:
						{
							...
							if (arglist.Length > 1)
							{
								...
								XmlLocalVariable a =
									(XmlLocalVariable)XmlAttach.FindAttachment(invoker as IEntity, typeof(XmlLocalVariable), varname);

								if (a == null)
								{
									// doesnt already exist so add it
									XmlAttach.AttachTo(invoker as IEntity, new XmlLocalVariable(varname, varval));
								}
								else
								{
									a.Data = varval;
					...
					case typeKeyword.SETONNEARBY:
						{
							...
							ArrayList nearbylist = GetNearbyObjects(
								invoker, targetname, targettype, typestr, range, searchcontainers, proptest);
					...
					case typeKeyword.SETONPETS:
						{
							...
							ArrayList nearbylist = GetNearbyObjects(
								triggermob, targetname, targettype, typestr, range, searchcontainers, null);
					...
					case typeKeyword.SETONPARENT:
						{
							...
							if (invoker != null && (invoker is Item))
							{
								ApplyObjectStringProperties(
									spawner, substitutedtypeName, ((Item)invoker).Parent, triggermob, invoker, out status_str);
							}
							else if (invoker != null && (invoker is XmlAttachment))
							{
								ApplyObjectStringProperties(
									spawner, substitutedtypeName, ((XmlAttachment)invoker).Attached, triggermob, invoker, out status_str);
							}
					...
					case typeKeyword.GIVE:
						{
							...
							if (arglist.Length > 1)
							{
								...
								AddItemToTarget(
									spawner, triggermob, keywordargs, arglist, triggermob, invoker, false, out remainder, out status_str);
							}
							TheSpawn.SpawnedObjects.Add(new KeywordTag(substitutedtypeName, spawner));
					...
					case typeKeyword.TAKE:
						{
							...
							if (drop_probability == 1 || Utility.RandomDouble() < drop_probability)
							{
								...
								if (itemTarget != null)
								{
									...
									XmlSaveItem si = (XmlSaveItem)XmlAttach.FindAttachment(invoker as IEntity, typeof(XmlSaveItem), "Taken");

									if (si == null)
									{
										XmlAttach.AttachTo(invoker as IEntity, new XmlSaveItem("Taken", savedItem, triggermob));
									}
									else
									{
					...
					case typeKeyword.SENDMSG:
						{
							...
							if (triggermob != null && !triggermob.Deleted && (triggermob is PlayerMobile))
							{
								...
								triggermob.Send(
									new UnicodeMessage(Serial.MinusOne, -1, MessageType.Regular, hue, font, "ENU", "System", msgText));
							}
					...
					case typeKeyword.WAITUNTIL:
						{
							...
							TheSpawn.SpawnedObjects.Add(
								new KeywordTag(
									substitutedtypeName, spawner, TimeSpan.FromMinutes(delay), TimeSpan.FromMinutes(timeout), condition, gotogroup));
					...
					case typeKeyword.EFFECT:
						{
							...
							if (arglist.Length > 0)
							{
								var keywordargs = ParseString(arglist[0], 8, ",");
								int effect = -1;
								int duration = 1;
									...
									if (spelltargetparms != null && spelltargetparms.Length > 0)
									{
										...
										else if (spelltargetparms[0].ParameterType == typeof(IPoint3D))
										{
											...
											targetargs[0] = new Point3D(
												triggermob.Location.X + randx, triggermob.Location.Y + randy, triggermob.Location.Z);
										}
										else
										{
			...
			else if (IsSpecialItemKeyword(typeName))
			{
				...
				switch (kw)
				{
					...
					case itemKeyword.SHIELD:
						{
							...
							if (itemkeywordargs.Length == 3)
							{
								...
								Item item = MagicShield(triggermob.Expansion, min, max);
								if (item != null)
								{
									AddSpawnItem(
										spawner,
					...
					case itemKeyword.JEWELRY:
						{
							...
							if (itemkeywordargs.Length == 3)
							{
								...
								Item item = MagicJewelry(map.Expansion, min, max);
								if (item != null)
								{
									AddSpawnItem(
										spawner,
					...
					case itemKeyword.RARE:
						{
							// syntax is RARE,filename
							if (itemkeywordargs.Length == 2)
							{
								string filename = itemkeywordargs[1].Trim();

								RaresFile file = RareSystem.GetRaresFile(filename);

								if (file == null)
								{
									status_str = "RARE file " + filename + " not found.";
									return false;
								}
								Item item = file.GetRandomRareEntry();
								if (item != null)
								{
									AddSpawnItem(
										spawner,
							...
							{
								status_str = "RARE takes 1 arg : " + itemtypestr;
								return false;
							}
							break;
						}
