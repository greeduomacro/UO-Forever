	...
	public class XmlDialog : XmlAttachment
	{
		...
		public class SpeechEntry
		{
			...
			public string Keywords;
						  // comma separated list of keywords that can be matched to activate the entry.  If no keywords are present then it is automatically activated
			...
			public int PrePause = -1;
					   // pause in seconds before saying the speech for this entry.  -1 indicates the use of auto pause calculation based on triggering speech length.
			...
			public bool IgnoreCarried = false;
						// ignore the TriggerOnCarried/NoTriggerOnCarried settings for the dialog when activating this entry
		...
		private int m_CurrentEntryNumber = -1;
					// used to determine which entry will be subject to modification by various entry editing calls
		...
		private Mobile m_ActivePlayer;
					   // keep track of the player that is currently engaged in conversation so that other players speech can be ignored.

		private int m_SpeechPace = defSpeechPace;
					// used for automatic prepause delay calculation.  delayinsecs = speechlength/speechpace + 1
		...
		public ArrayList SpeechEntries { get { return m_SpeechEntries; } set { m_SpeechEntries = value; } }

		public Mobile ActivePlayer { get { return m_ActivePlayer; } set { m_ActivePlayer = value; } }
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime LastInteraction { get { return m_LastInteraction; } set { m_LastInteraction = value; } }
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public TimeSpan ResetTime { get { return m_ResetTime; } set { m_ResetTime = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
		public int SpeechPace { get { return m_SpeechPace; } set { m_SpeechPace = value; } }
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public int ResetRange { get { return m_ResetRange; } set { m_ResetRange = value; } }
			...
			foreach (SpeechEntry s in m_SpeechEntries)
			{
					...
					(CheckDependsOn(s, currententryid) && (IsActive || currententryid == 0)))
				{
					...
					if ((keyword == null && s.Keywords == null))
					{
						...
						if (keyword != null && s.Keywords != null)
						{
							var arglist = s.Keywords.Split(",".ToCharArray());
		...
		public override bool HandlesOnSpeech { get { return (m_Running); } }
		...
		private void CheckForReset()
		{
			...
			if (!m_HoldProcessing &&
				((DateTime.UtcNow - ResetTime > m_LastInteraction) ||
				 (AttachedTo is IEntity && m_ActivePlayer != null && !IsInRange(m_ActivePlayer, AttachedTo, ResetRange))))
			{
				Reset();
			}
		}
		...
		private void DelayedSpeech(object state)
		{
			...
			if (matchentry != null)
			{
				...
				{
					var args = CurrentEntry.Action.Split(';');
			...
			if (File.Exists(dirname))
			{
				...
				if (ds.Tables != null && ds.Tables.Count > 0)
				{
					...
					if (ds.Tables[NPCPointName] != null && ds.Tables[NPCPointName].Rows.Count > 0)
					{
						...
						try
						{
							ProximityRange = int.Parse((string)dr["ProximityRange"]);
						}
						catch
						{ }
						try
						{
							ResetRange = int.Parse((string)dr["ResetRange"]);
						}
						catch
						{ }
						try
						{
							TriggerOnCarried = (string)dr["TriggerOnCarried"];
						}
						catch
						{ }
						try
						{
							NoTriggerOnCarried = (string)dr["NoTriggerOnCarried"];
						}
						catch
						{ }
						try
						{
							m_AllowGhostTriggering = bool.Parse((string)dr["AllowGhost"]);
						}
						catch
						{ }
						try
						{
							m_SpeechPace = int.Parse((string)dr["SpeechPace"]);
						}
						catch
						{ }
						try
						{
							Running = bool.Parse((string)dr["Running"]);
						}
						catch
						{ }
						try
						{
							ResetTime = TimeSpan.FromMinutes(double.Parse((string)dr["ResetTime"]));
						}
						catch
						{ }
						try
						{
							ConfigFile = (string)dr["ConfigFile"];
						}
						catch
						{ }
		...
		private class SaveNPCTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				XmlDialog xa = XmlAttach.FindAttachment(targeted as IEntity, typeof(XmlDialog)) as XmlDialog;
		...
		private class LoadNPCTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				else
				{
					...
					XmlAttach.AttachTo(targeted as IEntity, xa);
				}
			}
		}
		...
		{
			// We don't use XmlDialog anymore -Alan
