...
namespace Server.Commands.Generic
{
	...
	public class InterfaceItemGump : BaseGridGump
	{
		...
		public void Render()
		{
			...
			{
				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, ((Mobile)m_Item.RootParent).Name );
				AddEntryButton( 20, ArrowRightID1, ArrowRightID2, 7, ArrowRightWidth, ArrowRightHeight );
				//AddEntryHeader( 20 );

				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, "Properties of parent" );
				...
				AddEntryButton( 20, ArrowRightID1, ArrowRightID2, 10, ArrowRightWidth, ArrowRightHeight );
			}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 7: // Interface the parent
				{
					if ( m_Item.RootParent is Mobile )
					{
						Mobile parent = m_Item.RootParent as Mobile;
						m_From.SendGump( new InterfaceGump( m_From, new string[]{ "Entity" }, new ArrayList( new Mobile[]{ parent } ), 0, null ) );
					}
					m_From.SendGump( new InterfaceItemGump( m_From, m_Columns, m_List, m_Page, m_Item ) );
					break;
				}
				case 8: // Properties of parent
				{
					if ( m_Item.RootParent is Mobile )
					{
						Mobile parent = m_Item.RootParent as Mobile;
						if ( m_From.AccessLevel <= parent.AccessLevel )
							m_From.SendMessage( "Access is denied" );
						else
							m_From.SendGump( new PropertiesGump( m_From, parent ) );
					}
					m_From.SendGump( new InterfaceItemGump( m_From, m_Columns, m_List, m_Page, m_Item ) );
					break;
				}
				case 9: // Go to parent mobile
				{
					if ( m_Item.RootParent is Mobile )
					{
						Mobile parent = m_Item.RootParent as Mobile;
						InvokeCommand( String.Format( "Go {0}", parent.Serial.Value ) );
					}
					m_From.SendGump( new InterfaceItemGump( m_From, m_Columns, m_List, m_Page, m_Item ) );
					break;
				}
				case 10: // Bring mobile parent to you
				{
					if ( m_Item.RootParent is Mobile )
					{
						Mobile parent = m_Item.RootParent as Mobile;
	...
	public class InterfaceMobileGump : BaseGridGump
	{
		...
		public void Render()
		{
			...
			if ( m_Mobile is IMount && ((IMount)m_Mobile).Rider != null )
			{
				Mobile rider = ((IMount)m_Mobile).Rider;

				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, rider.Name );
				AddEntryButton( 20, ArrowRightID1, ArrowRightID2, 10, ArrowRightWidth, ArrowRightHeight );
				//AddEntryHeader( 20 );

				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, "Properties of parent" );
				AddEntryButton( 20, ArrowRightID1, ArrowRightID2, 11, ArrowRightWidth, ArrowRightHeight );

				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, "Go to parent mobile" );
				AddEntryButton( 20, ArrowRightID1, ArrowRightID2, 12, ArrowRightWidth, ArrowRightHeight );

				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, "Bring parent mobile here" );
				AddEntryButton( 20, ArrowRightID1, ArrowRightID2, 13, ArrowRightWidth, ArrowRightHeight );
			}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 10: // Interface the parent
				{
					if ( m_Mobile is IMount && ((IMount)m_Mobile).Rider != null )
					{
						Mobile rider = ((IMount)m_Mobile).Rider;
						m_From.SendGump( new InterfaceGump( m_From, new string[]{ "Entity" }, new ArrayList( new Mobile[]{ rider } ), 0, null ) );
					}

					m_From.SendGump( new InterfaceMobileGump( m_From, m_Columns, m_List, m_Page, m_Mobile ) );
					break;
				}
				case 11: // Properties of parent
				{
					if ( m_Mobile is IMount && ((IMount)m_Mobile).Rider != null )
					{
						Mobile rider = ((IMount)m_Mobile).Rider;
						if ( m_From.AccessLevel <= rider.AccessLevel )
							m_From.SendMessage( "Access is denied" );
						else
							m_From.SendGump( new PropertiesGump( m_From, rider ) );
					}

					m_From.SendGump( new InterfaceMobileGump( m_From, m_Columns, m_List, m_Page, m_Mobile ) );
					break;
				}
				case 12: // Go to parent mobile
				{
					if ( m_Mobile is IMount && ((IMount)m_Mobile).Rider != null )
					{
						Mobile rider = ((IMount)m_Mobile).Rider;
						InvokeCommand( String.Format( "Go {0}", rider.Serial.Value ) );
					}

					m_From.SendGump( new InterfaceMobileGump( m_From, m_Columns, m_List, m_Page, m_Mobile ) );
					break;
				}
				case 13: // Bring mobile parent to you
				{
					if ( m_Mobile is IMount && ((IMount)m_Mobile).Rider != null )
					{
						Mobile rider = ((IMount)m_Mobile).Rider;
						rider.MoveToWorld( m_From.Location, m_From.Map);
					}
