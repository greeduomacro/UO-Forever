		...
		public void Flush( Mobile from, bool flushToLog )
		{
            ...
            if ( m_Responses.Count > 0 )
			{
				for ( int i = 0; i < m_Responses.Count; ++i )
				{
                    ...
                    //debug hang
                    if (LoggingCustom.CommandDebug) LoggingCustom.LogCommandDebug( "Respone\t");
					if ( obj is MessageEntry )
					{
						from.SendMessage( ((MessageEntry)obj).ToString() );
                        //debug hang
                        if (LoggingCustom.CommandDebug) LoggingCustom.LogCommandDebug( ((MessageEntry)obj).ToString() + "\t");
			...
			{
                //debug hang
                if (LoggingCustom.CommandDebug) LoggingCustom.LogCommandDebug( "Failure\t");
                for (int i = 0; i < m_Failures.Count; ++i)
                {
                    if (LoggingCustom.CommandDebug) LoggingCustom.LogCommandDebug( ((MessageEntry)m_Failures[i]).ToString());
                    from.SendMessage(((MessageEntry)m_Failures[i]).ToString());
                }
			}
            if (LoggingCustom.CommandDebug) LoggingCustom.LogCommandDebug( "endflush\t");
			m_Responses.Clear();
			m_Failures.Clear();
		}
	}
