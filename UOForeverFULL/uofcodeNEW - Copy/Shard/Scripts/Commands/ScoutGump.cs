...
namespace Server.Gumps
{
	public class ScoutGump : Gump
	{
		...
		{
			CommandSystem.Register( "Scout", AccessLevel.Seer, new CommandEventHandler( ScoutList_OnCommand ) );
		}
		...
		public static List<Mobile> BuildList( Mobile owner, string filter )
		{
			...
			for ( int i = 0; i < states.Count; ++i )
			{
                ...
                if (m != null && (m == owner || !m.Hidden || owner.AccessLevel >= AccessLevel.GameMaster))//owner.AccessLevel > m.AccessLevel || (m is PlayerMobile && ((PlayerMobile)m).VisibilityList.Contains( owner ) ) ) )
				{
			...
			switch ( info.ButtonID )
			{
				...
				default:
				{
					...
					if ( index >= 0 && index < m_Mobiles.Count )
					{
						...
						else if ( m == from || !m.Hidden || from.AccessLevel >= AccessLevel.GameMaster )//|| from.AccessLevel > m.AccessLevel || (m is PlayerMobile && ((PlayerMobile)m).VisibilityList.Contains( from ))) 
						{
