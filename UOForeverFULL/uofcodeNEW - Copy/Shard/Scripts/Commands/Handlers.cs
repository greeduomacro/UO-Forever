...
namespace Server.Commands
{
	public class CommandHandlers
	{
		public static void Initialize()
		{
			...
			//Register( "Go", AccessLevel.GameMaster, new CommandEventHandler( Go_OnCommand ) );

			Register( "DropHolding", AccessLevel.GameMaster, new CommandEventHandler( DropHolding_OnCommand ) );

			Register( "GetFollowers", AccessLevel.EventMaster, new CommandEventHandler( GetFollowers_OnCommand ) );

			Register( "ClearFacet", AccessLevel.Administrator, new CommandEventHandler( ClearFacet_OnCommand ) );

			Register( "Where", TestCenter.Enabled ? AccessLevel.Player : AccessLevel.Counselor, new CommandEventHandler( Where_OnCommand ) );
			...
			Register( "Cast", AccessLevel.GameMaster, new CommandEventHandler( Cast_OnCommand ) );
			...
			Register( "Save", AccessLevel.EventMaster, new CommandEventHandler( Save_OnCommand ) );
			...
			Register( "Move", AccessLevel.EventMaster, new CommandEventHandler( Move_OnCommand ) );
			Register( "Client", AccessLevel.GameMaster, new CommandEventHandler( Client_OnCommand ) );
			...
			Register( "F", AccessLevel.Player, new CommandEventHandler( FactionMessage_OnCommand ) );
            Register("C", AccessLevel.Player, new CommandEventHandler(CompanionMessage_OnCommand));
            Register("ZC", AccessLevel.Player, new CommandEventHandler(ZombieMessage_OnCommand));

            Register("PC", AccessLevel.Player, new CommandEventHandler(PokerChat_OnCommand));

			Register( "CBCast", AccessLevel.Seer, new CommandEventHandler( CBroadcastMessage_OnCommand ) );
			Register( "CBC", AccessLevel.Seer, new CommandEventHandler( CBroadcastMessage_OnCommand ) );
			Register( "CB", AccessLevel.Seer, new CommandEventHandler( CBroadcastMessage_OnCommand ) );
            
            Register("OH", AccessLevel.Seer, new CommandEventHandler(OHBroadcastMessage_OnCommand));
            Register("OHB", AccessLevel.Seer, new CommandEventHandler(OHBroadcastMessage_OnCommand));
			...
			Register( "Stats", AccessLevel.GameMaster, new CommandEventHandler( Stats_OnCommand ) );

//			Register( "ReplaceBankers", AccessLevel.Administrator, new CommandEventHandler( ReplaceBankers_OnCommand ) );

			Register( "SpeedBoost", AccessLevel.Player, new CommandEventHandler( SpeedBoost_OnCommand ) );
		}
		...
		[Description( "Enables a speed boost for the invoker (only works for staff or pseudoseers with permission).  Disable with paramaters." )]
		private static void SpeedBoost_OnCommand( CommandEventArgs e )
		{
			Mobile from = e.Mobile;
            if (from.NetState == null || from.NetState.Account == null) return;

            if (from.AccessLevel == AccessLevel.Player)
            {
                // only allow it if they are a pseudoseer with any permissions
                Mobile controllingMob = from.NetState.Account.GetPseudoSeerLastCharacter();
                if (controllingMob != null && controllingMob.Account != null)
                {
                    if (controllingMob.AccessLevel == AccessLevel.Player)
                    {
                        ...
                        else
                        {
                            ...
                            if (!PseudoSeerStone.AllowAllPossessedMobsSpeedBoost)
                            {
                                if (!PseudoSeerStone.AllowPseudoseerMobsSpeedBoost 
                                    || bc.PseudoSeerPermissions == null 
                                    || bc.PseudoSeerPermissions == String.Empty)
                                {
                                    from.SendMessage("You do not have access to that command.");
                                    return;
                                }
                                // otherwise they are a pseudoseer with permissions--allow speedboost
                            }
                            // otherwise they are possessing a mob, and all possessed mobs can have speedboost
                        }
                    }
                    // otherwise they are >= Counselor--allow speedboost
                }
                else
                {
                    from.SendMessage("You do not have access to that command.");
				...
				if ( map != null )
				{
					...
					foreach ( Item item in m.GetItemsInRange( 0 ) )
					{
						if ( item is Spawner )
						{
							...
							for ( int i = 0; !hasBankerSpawner && i < spawner.SpawnedName.Count; ++i )
								hasBankerSpawner = Insensitive.Equals( (string)spawner.SpawnedName[i], "banker" );
...
*/
		...
		private class ViewEqTarget : Target
		{
			...
			private static ItemListEntry[] GetEquip( Mobile m )
			{
				...
				for ( int i = 0; i < m.Items.Count; ++i )
				{
					...
					entries[i] = new ItemListEntry( String.Format( "{0}: {1}", item.Layer, item.GetType().Name ), item.ItemID, Math.Max( item.Hue - 1, 0 ) );
				}
		...
		private class BankTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if (targeted is PlayerMobile)
				{
					...
					foreach (var box in m.BankBoxes)
					{
						CommandLogging.WriteLine(
							from,
							"{0} {1} opening bank box {2} of {3}",
							from.AccessLevel,
							CommandLogging.Format(from),
							CommandLogging.Format(box),
		...
		public static void Echo_OnCommand( CommandEventArgs e )
		{
			...
			if ( toEcho.Length > 0 )
			{
				from.SendMessage( toEcho );
				CommandLogging.WriteLine( from, "{0} {1} commented: {2}", from.AccessLevel, CommandLogging.Format( from ), toEcho );
			}
			else
		...
		public static void Bank_OnCommand(CommandEventArgs e)
		{
			...
			if (e.Arguments.Length > 0)
			{
				...
				{
					foreach (
						var m in World.Mobiles.Values.OfType<PlayerMobile>().Where(m => m.Name.ToLower() == e.Arguments[0].ToLower()))
					{
						foreach (var box in m.BankBoxes)
						{
							CommandLogging.WriteLine(
				...
				{
					Console.WriteLine("Bank command error:" + except.Message);
					Console.WriteLine(except.StackTrace);
				}

				from.SendMessage(
					"PlayerMobile with name {0} was not found! (note that the search is NOT case-sensitive)", e.Arguments[0]);
			}
			else
			{
...
*/
		...
		[Usage( "F <text>" )]
		[Aliases( "F" )]
		[Description( "Broadcasts a message to faction members." )]
		public static void FactionMessage_OnCommand( CommandEventArgs e )
		{
			Mobile from = e.Mobile;
			...
			if ( faction == null )
				return;
			if (pl.m_NextM == null)
				pl.m_NextM = DateTime.UtcNow;
			if( e.Length >= 1 )
			{
				if ( DateTime.UtcNow >= pl.m_NextM )
				{
					List<PlayerState> members = faction.Members;

					for ( int i = 0; i < members.Count; ++i )
						members[i].Mobile.SendMessage( faction.Definition.HueBroadcast, "{0} [{1}] : {2}", from.Name, faction.Definition.FriendlyName, e.ArgString );
						
					pl.m_NextM = DateTime.UtcNow + TimeSpan.FromSeconds( 5.0 );
				}
				else
					from.SendMessage( "You must wait 5 seconds before sending another message!" );
			}
		}
        ...
        [Usage("C <text>")]
        [Aliases("C")]
        [Description("Broadcasts a message to Companions.")]
        public static void CompanionMessage_OnCommand(CommandEventArgs e)
        {
            PlayerMobile pm = e.Mobile as PlayerMobile;
            if (pm == null || pm.Companion == false) return;
            
            if (e.Arguments.Length >= 1)
            {
                string msg = "[companion]" + pm.Name + ": " + e.ArgString;
                LoggingCustom.LogCompanion(msg);
                foreach (NetState state in NetState.Instances)
                {
                    if (state.Mobile != null 
                        && state.Mobile is PlayerMobile 
                        && ((PlayerMobile)state.Mobile).Companion)
                    {
                        state.Mobile.SendMessage(0x38, msg);
                    }
                }
            }
        }
        ...
        [Usage("ZC <text>")]
        [Aliases("ZC")]
        [Description("Broadcasts a message to Zombie Participants.")]
        public static void ZombieMessage_OnCommand(CommandEventArgs e)
        {
            ZombieAvatar pm = e.Mobile as ZombieAvatar;
            if (pm == null || pm.Map != Map.ZombieLand) return;

            if (e.Arguments.Length >= 1)
            {
                string msg = "[Zombieland]" + pm.Name + ": " + e.ArgString;
                foreach (NetState state in NetState.Instances)
                {
                    if (state.Mobile is ZombieAvatar
                        && (state.Mobile).Map == Map.ZombieLand)
                    {
                        state.Mobile.SendMessage(61, msg);
                    }
                }
            }
        }
        ...
        [Usage("PC <text>")]
        [Aliases("PC")]
        [Description("Broadcasts a message to poker participants.")]
        public static void PokerChat_OnCommand(CommandEventArgs e)
        {
            var pm = e.Mobile as PlayerMobile;
            if (pm == null || pm.PokerGame == null) return;

            if (e.Arguments.Length >= 1)
            {
                string msg = "[Poker]" + pm.Name + ": " + e.ArgString;
                foreach (var player in pm.PokerGame.Players.Players)
                {
                    if (player.Mobile is PlayerMobile && player.Mobile.NetState != null && player.Chat)
                    {
                        player.Mobile.SendMessage(61, msg);
                    }
                }
            }
        }
        ...
        [Usage("OH <text>")]
        [Aliases("OH", "OHB")]
        [Description("Broadcasts an OVERHEAD message to everyone online.")]
        public static void OHBroadcastMessage_OnCommand(CommandEventArgs e)
        {
            foreach (NetState state in NetState.Instances)
            {
                if (state.Mobile != null)
                {
                    state.Mobile.LocalOverheadMessage(MessageType.Regular, e.Mobile.SpeechHue, false, "(Global)" + e.ArgString);
                }
            }
        }
